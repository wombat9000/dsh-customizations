...
Google auth provides the shared **Google accounts** Settings card, client configuration, explicit consent, and token refresh. Google Drive adds a Standard-based **Google Drive** preset that initially exposes only `request_drive_access`. Its custom card opens a private file/folder picker; confirming read access exposes session-scoped listing and text-reading tools plus their skill. Use **Manage access** to revise or revoke the selection. After an approved profile update and restart, connect Google Drive in **Settings → Plugins → Google accounts**; existing metadata-only connections need explicit consent for `drive.readonly`. Credentials stay in DSH's credential store. Grants expire on session unload, account changes, or Host restart, and do not transfer to subagents. The stock Standard preset stays unchanged. This version includes no Gmail/GCP tools, Drive editing, or `gws` execution. See the [shared auth contract](packages/dsh-google-auth/README.md) and [Drive setup and security boundary](packages/dsh-google-drive/README.md).
