...
Google auth provides the shared **Google accounts** Settings card, client configuration, scoped consent, and token refresh. Google Drive consumes that provider and adds a **Google Drive** preset with Standard coding tools and `google_drive_list_files`. After an approved profile update and restart, open **Settings → Plugins → Google accounts**, paste your downloaded Desktop OAuth client JSON in the write-only field, save it, and connect Google Drive. Future integrations declare their required scopes there and require explicit consent for missing permissions; they do not duplicate login. Credentials stay in DSH's credential store, not this repository. Select the Google Drive preset for a new session to list metadata. This version includes no Gmail/GCP tools, Workspace content editing, or `gws` execution. See the [shared auth contract](packages/dsh-google-auth/README.md) and [Drive setup](packages/dsh-google-drive/README.md).
