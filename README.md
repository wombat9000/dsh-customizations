...
Google auth provides the shared **Google accounts** Settings card, client configuration, explicit consent, and token refresh. Google Drive adds a Standard-based **Google Drive** preset with `request_drive_access` and `request_sheets_edit_access`. Its custom picker grants session-scoped reading or separate individual-spreadsheet editing access. Read grants expose listing, text-reading, and bounded Sheets range tools. Sheets edits require a local before/after preview and exact **Apply changes** approval. Use **Manage access** to revise or revoke the selection. After an approved profile update and restart, connect Google Drive in **Settings → Plugins → Google accounts**; existing metadata-only connections need explicit consent for `drive.readonly`. Credentials stay in DSH's credential store. Grants expire on session unload, account changes, or Host restart, and do not transfer to subagents. The stock Standard preset stays unchanged. Enable the Sheets API for spreadsheet reads. Editing additionally requires explicit account-wide Sheets write consent in Google accounts; DSH enforces the selected-file restriction. This version includes no Gmail/GCP tools, general Drive file mutations, or `gws` execution. See the [shared auth contract](packages/dsh-google-auth/README.md) and [Drive setup and security boundary](packages/dsh-google-drive/README.md).
