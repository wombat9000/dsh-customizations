...
const nodeHost = (source, timeout = 2000) =>
  startHost(process.execPath, ['--input-type=module', '-e', source], options, timeout)
...
test('startup exit reports failure without exposing the launch token', posix, async () => {
  const host = nodeHost(
    `console.error(${JSON.stringify(`Failed request ${launchUrl}`)}); process.exit(3)`,
  )
  try {
...
test('startup timeout rejects and the host is stopped', posix, async () => {
  ...
  try {
    await assert.rejects(host.ready, /startup timed out/)
  } finally {
    await stopHost(host.child, 50)
  }
...
test('readiness waits for the complete startup URL across output chunks', posix, async () => {
  ...
  `)
  try {
    assert.equal(await host.ready, launchUrl)
  } finally {
    await stopHost(host.child, 50)
  }
})
...
test(
  'cleanup stops descendants after the launcher has exited',
