...
export function githubFieldSessionSeed(cwd) {
  ...
  const results = [
    ...
    },
    {
      host: 'github.com',
      outcome: 'failed',
      error: { code: 'NOT_FOUND', message: 'The requested project or item was not found.' },
    },
  ]
  const calls = results.map((_, index) => ({
    type: 'tool-call',
    id: `visual-field-${index}`,
  ...
  return {
    ...
    options: {
      ...
      seed: seed.map((event, seq) => ({ seq, ...event })),
    },
  }
}
