  ...
  factory: (require) => {
    ...
    function SettingsCard({ rpc }) {
      ...
      const mutate = async (method) => {
        ...
        setNotice('')
        try {
          const value = await call(rpc, method, {
            target: status.target,
            ...(method === 'save' ? { apiKey: draft.trim() } : {}),
          })
          if (alive.current) {
            setStatus(value)
            setDraft('')
      ...
      return h(
        ...
        h(
          'div',
          { className: 'dsh-openrouter__body' },
          h(
            'p',
            { role: 'status' },
            status
          ...
          h(
            'p',
            { className: 'dsh-openrouter__hint' },
            'One credential for trusted OpenRouter integrations and the built-in OpenRouter model route. Replacing or removing it affects all consumers. The key is stored in DSH credentials, never ordinary plugin settings.',
          ),
          status && !status.writable
            ? h('p', null, 'This credential source is read-only. Change it where it is configured.')
            : null,
          h(
            'label',
            null,
          ...
          h(
            'div',
            { className: 'dsh-openrouter__actions' },
            h(
              'button',
              {
                type: 'button',
