...
test('shared OpenRouter card edits only the disposable host credential', async ({ app }) => {
  ...
  await expect(card.getByText('Shared OpenRouter key saved. No remote request was made.')).toBeVisible()
  await expect(field).toHaveValue('')
  ...
  await expect(card.getByText('Stored key removed. Effective credential status refreshed.')).toBeVisible()
