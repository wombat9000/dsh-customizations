...
let status
const calls = []
afterEach(async () => { if (root) await act(() => root.unmount()); container?.remove(); root = null; vi.restoreAllMocks() })
async function mount(overrides = {}) {
  calls.length = 0
  status = { configured: false, writable: true, source: 'provider record', target: 'record:llm-pi-ai/openrouter', ...overrides }
  const rpc = { call: vi.fn(async (channel, method, payload) => {
    expect(channel).toBe('/openrouter-integration'); calls.push({ method, payload })
...
test('shared settings saves one canonical credential and clears the password draft', async () => {
  ...
  expect(calls.at(-1)).toEqual({ method: 'save', payload: { target: 'record:llm-pi-ai/openrouter', apiKey: 'sk-or-fixture-only' } })
  await expect.element(field).toHaveValue('')
  await expect.element(page.getByText('Shared OpenRouter key saved. No remote request was made.')).toBeVisible()
...
test('transport errors do not expose request details and styles dispose', async () => {
  ...
  await act(async () => page.getByRole('button', { name: 'Save shared key' }).click())
  await expect.element(page.getByRole('alert')).toHaveTextContent('OpenRouter settings request failed.')
