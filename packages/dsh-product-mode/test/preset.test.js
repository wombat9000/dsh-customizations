...
test('published composition-only package discovers independently and final recipe retains all three custom roots', async (t) => {
  ...
  assert.deepEqual(
    ...
    ['test'],
    'no install, setup, generation or startup scripts',
  )
  ...
  for (const combined of [false, true]) {
    ...
    for (const id of [
      ...
      'cordis',
      presetId,
      ...(combined ? ['worktree-coordinator', 'project-steward'] : []),
    ]) {
  ...
  assert.equal(
    recipe.bundles.find((row) => row.name === manifest.name).source,
    '../../packages/dsh-product-mode',
  )
})
...
test('Product mode preserves exact Standard tool rows, configurations, nesting, realm boundaries and license', async () => {
  ...
  assert.ok(
    !flatten(product).some((row) =>
      /@local\/dsh-github|dsh-(tool-cordis|cordis-runtime)/.test(row.name),
    ),
    'GitHub stays host-global, with no duplicated tools or elevated Cordis access',
  )
  assert.equal(
...
test(
  'real Standard and Product standing mounts share nineteen global GitHub tools without any CLI or planning writes',
  { timeout: 15000 },
  async (t) => {
    const f = await fixture(t)
    ...
    const { ctx, subprocessCalls } = await host(
      t,
      f,
      rosterConfig(f.baseUrl, await recipePatches()),
    )
    ...
    for (const scope of [undefined, standard, product]) {
      assert.deepEqual(
        names(scope).filter((name) => name.startsWith('github_')),
        githubNames,
      )
      for (const name of githubNames)
        assert.equal(ctx.get('tools').get(name, scope), ctx.get('tools').get(name))
    }
    for (const name of [
      'planMode',
      'compaction',
      ...
      assert.equal(ctx.get(name), undefined)
    ...
    assert.equal(
      ...
      join(f.packaged, 'presets/product-mode/skills/product-planning'),
    )
    ...
    assert.equal(
      await readFile(join(loaded.resourceBase.path, 'examples.md'), 'utf8'),
      await readFile(join(presetRoot, presetId, 'skills/product-planning/examples.md'), 'utf8'),
    )
    ...
    assert.deepEqual(
      ...
      before,
      'mounting and skill reads create no drafts, schemas, issues or state files',
    )
  },
)

test(
  'copied Product preset resolves its bundled skill without the original package',
  { timeout: 15000 },
  async (t) => {
    const f = await fixture(t)
    const userRoot = join(f.directory, 'user-presets')
    await mkdir(userRoot)
    const { ctx, subprocessCalls } = await host(t, f, {
    ...
    assert.equal(
      await readFile(join(loaded.resourceBase.path, 'LICENSE.matt-pocock'), 'utf8'),
      await readFile(
        join(presetRoot, presetId, 'skills/product-planning/LICENSE.matt-pocock'),
        'utf8',
      ),
    )
    assert.equal(
      await readFile(join(userRoot, 'my-product/LICENSE.standard'), 'utf8'),
      await readFile(join(dirname(SHIPPED_PRESET_ROOT), 'LICENSE'), 'utf8'),
    )
    assert.deepEqual(subprocessCalls, [])
