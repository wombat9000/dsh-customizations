...
const cancelled = () =>
  sheetsError('cancelled', 'Google Sheets operation was cancelled before dispatch.')
// Only these local strings leave the transport. Never forward upstream text or metadata.
const diagnostics = Object.freeze({
  api_disabled:
    'The Google Sheets API is disabled for the OAuth project. Enable it in that project.',
  scopes:
    'Google Sheets OAuth permissions are missing. Grant additional permissions in Google accounts settings.',
  ...
  forbidden:
    'Google Sheets denied access. Check the account’s spreadsheet sharing permissions and access policy.',
...
function httpDiagnostic(status, bytes, size) {
  if (status === 403) {
    let error
    try {
      error = JSON.parse(bytes.toString('utf8', 0, size))?.error
    } catch {}
    ...
    const reasons = details
      .filter((d) => d?.['@type'] === 'type.googleapis.com/google.rpc.ErrorInfo')
      .map((d) => d.reason)
    const legacy = Array.isArray(error?.errors)
      ? error.errors.slice(0, 64).map((e) => e?.reason)
      : []
    if (reasons.includes('SERVICE_DISABLED') || legacy.includes('accessNotConfigured'))
      return 'api_disabled'
    if (
      reasons.includes('ACCESS_TOKEN_SCOPE_INSUFFICIENT') ||
      legacy.includes('insufficientPermissions')
    )
      return 'scopes'
    if (
      legacy.some((reason) =>
...
export class SheetsTransport {
  ...
  constructor({ withAccessToken, fetch = globalThis.fetch, requestTimeoutMs = 30_000 } = {}) {
    if (
      typeof withAccessToken !== 'function' ||
      typeof fetch !== 'function' ||
      !Number.isFinite(requestTimeoutMs) ||
      requestTimeoutMs < 1 ||
      requestTimeoutMs > 120_000
    )
      throw sheetsError('invalid', 'Invalid Google Sheets client options.')
  ...
  async request(url, { signal, body, beforeDispatch } = {}) {
    if (signal !== undefined && !(signal instanceof AbortSignal))
      throw sheetsError('invalid', 'Invalid cancellation signal.')
    ...
    try {
      return await wait(
        Promise.resolve().then(() =>
          this.#auth(async (token, authSignal) => {
            // A provider must not replay this callback, even after an HTTP failure.
            if (entered)
              throw sheetsError(
                'auth',
                'Google Sheets authentication callback replay was rejected.',
              )
            entered = true
            lifetime = authSignal
            ...
            try {
              ...
              diagnostic = 'auth'
              if (typeof token !== 'string' || !/^[\x21-\x7e]{1,16384}$/u.test(token))
                throw sheetsError('auth', 'Google Sheets authentication is unavailable.')
              diagnostic = 'request'
              if (body !== undefined && beforeDispatch) {
                ...
                if (result !== undefined) {
                  ...
                  throw sheetsError(
                    'invalid',
                    'Google Sheets dispatch guard must be synchronous and return undefined.',
                  )
                }
              }
              if (controller.signal.aborted) throw cancelled()
              ...
              const limit = response.ok ? 2_000_000 : 16_384,
                bytes = Buffer.allocUnsafe(limit)
              let size = 0,
                chunks = 0
              while (true) {
                ...
                if (
                  ...
                  size + value.byteLength > limit ||
                  ++chunks > 32_768
                )
                  throw sheetsError('response', 'Google Sheets response exceeds the safety limit.')
              ...
              try {
                ...
                throw sheetsError('response', 'Invalid Google Sheets response.')
              }
            } finally {
      ...
      if (body !== undefined && dispatched)
        throw Object.assign(
          sheetsError(
            'uncertain',
            `Google Sheets write outcome is uncertain. Inspect the sheet before preparing another edit. ${diagnostics[diagnostic]}`,
          ),
          { diagnostic },
        )
