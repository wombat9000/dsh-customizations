...
export function validatePdfOptions({
  ...
  if (
    !integer(startPage, 1, 200) ||
    (endPage !== undefined && (!integer(endPage, startPage, 200) || endPage - startPage >= 5)) ||
    !['auto', 'off', 'force'].includes(ocr) ||
    !Array.isArray(languages) ||
    languages.length < 1 ||
    languages.length > 3 ||
    new Set(languages).size !== languages.length ||
    languages.some((language) => !languagesAllowed.has(language)) ||
    !integer(maxBytes, 1, 262144) ||
    (signal !== undefined && !(signal instanceof AbortSignal))
  )
    throw new Error('Invalid PDF read options.')
...
export class PdfProcessor {
  ...
  async read(
    bytes,
    { startPage = 1, endPage, ocr = 'auto', languages = ['eng'], maxBytes = 262144, signal } = {},
  ) {
    ...
    if (
      ...
      throw new Error('PDF input must be between 5 bytes and 20 MiB.')
    if (
      Buffer.from(bytes.buffer, bytes.byteOffset, Math.min(bytes.byteLength, 1024)).indexOf(
        '%PDF-',
      ) < 0
    )
      throw new Error('PDF processing failed: the document is malformed.')
    ;({ startPage, endPage, ocr, languages, maxBytes, signal } = validatePdfOptions({
      startPage,
      endPage,
      ocr,
      languages,
    ...
    if (this.#active.size >= 2)
      throw new Error('PDF processor is busy; retry after another read finishes.')
    ...
    try {
      ...
      const run = (tool, args, options = {}) =>
        pdfProcess(tools, tool, args, { cwd: directory, signal: controller.signal, ...options })
      ...
      if (encryptedLines.length !== 1 || pageLines.length !== 1)
        throw new Error('PDF processing failed: ambiguous or malformed document metadata.')
      if (!/^Encrypted:[ \t]+no[ \t]*$/u.test(encryptedLines[0]))
        throw new Error(
          'Encrypted PDFs are not supported, including PDFs that open without a password.',
        )
      ...
      if (ocr !== 'off') {
        const installed = (await run('tesseract', ['--list-langs'], { limit: 16384 }))
          .toString('utf8')
          .split(/\r?\n/u)
          .map((line) => line.trim())
        if (languages.some((language) => !installed.includes(language)))
          throw new Error(
            'A requested OCR language is not installed. Ask an administrator to install it; no language data is downloaded automatically.',
          )
      }
      const pages = [],
        warnings =
      ...
      for (let pageNumber = startPage; pageNumber <= lastPage; pageNumber++) {
        ...
        if (ocr === 'force' || (ocr === 'auto' && text.replace(/\s/gu, '').length < 24)) {
          ...
          await run(
            ...
            { timeout: 30000, limit: 16384 },
          )
          const imageStat = await stat(image)
          if (imageStat.size > 20 * 1024 * 1024)
            throw new Error('PDF rendered image exceeds its size limit.')
          const recognized = (
            await run('tesseract', [image, 'stdout', '-l', languages.join('+'), '--psm', '3'], {
              timeout: 30000,
              limit: maxBytes,
            })
          )
        ...
        if (!text)
          warnings.push(
            `Page ${pageNumber}: no readable text was found.${ocr === 'off' ? ' Try ocr="auto" or ocr="force" for scanned content.' : ''}`,
          )
        const labeled = `[Page ${pageNumber}]\n${text}`
        outputBytes += Buffer.byteLength(labeled) + (pages.length ? 2 : 0)
        if (outputBytes > maxBytes)
          throw new Error(
            'PDF text exceeds maxBytes; request fewer pages or a larger output limit.',
          )
      ...
      return {
        ...
        pages: pages.map(({ pageNumber, method }) => ({ pageNumber, method })),
        totalPages,
        actualRange: { startPage, endPage: lastPage },
        ...(lastPage < totalPages ? { nextStartPage: lastPage + 1 } : {}),
        warnings,
        mimeType: 'text/plain',
        format: 'pdf',
      }
      ...
      if (error?.code)
        throw new Error('PDF processing could not access its private temporary storage.')
      throw error
    } finally {
      clearTimeout(timer)
      ...
      try {
        ...
        throw new Error('PDF processing could not clean up its private temporary storage.')
      } finally {
