...
const DISABLED =
  'Google Drive tools are disabled for this session. Enable Google Drive in the session toolbar first.'
...
export class SessionDriveTools {
  ...
  isEnabled(agent) {
    return !this.closed && this.entries.get(agent)?.enabled === true
  }
  revisionOf(agent) {
    return this.entries.get(agent)?.revision ?? 0
  }
  assertEnabled(agent, revision) {
    this.service.assertOwner(agent)
    const entry = this.entries.get(agent)
    if (!this.isEnabled(agent) || (revision !== undefined && entry.revision !== revision))
  ...
  set({ sessionId, ownerId, revision, enabled }, signal) {
    ...
    if (!enabled) {
      ...
      try {
        entry.dispose = this.install(entry)
      } catch (error) {
        entry.enabled = false
        entry.revision++
        entry.controller.abort()
  ...
  install(entry) {
    ...
    if (!tools || !skills || typeof agent.ctx?.effect !== 'function')
      throw new Error('Session tool and skill registries are unavailable.')
    const groups = { requests: [], drive: [], sheets: [], edits: [] }
    let active = true,
      unwatch
    const clearGroup = (group) => {
      for (const dispose of groups[group].splice(0).reverse()) dispose()
    }
    const dispose = () => {
      if (!active) return
      active = false
    ...
    const update = () => {
      ...
      try {
        sync('drive', granted, () => {
          register('drive', createListTool(this.service))
          register('drive', createReadTool(this.service))
          groups.drive.push(skills.register(READ_SKILL))
        })
        sync('sheets', granted || editable, () => {
          register('sheets', createSheetsDescribeTool(this.service))
          register('sheets', createSheetsReadTool(this.service))
