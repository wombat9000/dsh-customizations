...
const UNCERTAIN =
  'The write outcome is uncertain. Read the affected cells before preparing another change; do not retry this approval.'
...
export class SheetsRuntime {
  ...
  #explicit(owner, fileId) {
    ...
    return this.editRuntime.permissions
      .grants(owner)
      .some((grant) =>
  ...
  #edit(owner, fileId) {
    ...
    if (!this.#explicit(owner, fileId))
      throw new Error('Select this spreadsheet for session edit access first.')
  }
  #read(owner, input, action) {
    this.#owner(owner)
    const runtime = this.#explicit(owner, input.fileId) ? this.editRuntime : this.readRuntime
    if (runtime.committing.has(owner))
      throw new Error('Sheets access confirmation is still in progress.')
  ...
  describe(owner, { fileId, signal, ...extra } = {}) {
    ...
    return this.#read(owner, { fileId, signal }, (signal) =>
      this.readClient.describe({ fileId, signal }),
    )
  }
  read(owner, { fileId, range, signal, ...extra } = {}) {
    if (Object.keys(extra).length) throw new Error('Invalid Sheets read options.')
    return this.#read(owner, { fileId, signal }, (signal) =>
      this.readClient.read({ fileId, range, signal }),
    )
  }
  #notify(owner) {
    try {
      ...
      /* Notification failure must not change a write outcome. */
    }
  }
  #key(owner, callId) {
    return `${owner.session.id}:${callId}`
  }
  #valid(record) {
    this.#edit(record.owner, record.fileId)
    if (record.generation !== this.auth.getAccessGeneration() || Date.now() >= record.expiresAt)
  ...
  prepare(owner, { callId, fileId, range, changes, signal, ...extra } = {}) {
    ...
    if (
      ...
      !validIdentity(callId) ||
      (signal !== undefined && !(signal instanceof AbortSignal))
    )
      throw new Error('Invalid Sheets preview options.')
    ...
    if (records.filter(([, record]) => ACTIVE.has(record.state)).length >= 20)
      throw new Error('Too many pending Sheets previews.')
    ...
    const record = {
      ...
      generation: this.auth.getAccessGeneration(),
    ...
    void this.editRuntime.permissions
      .withAuthorized(
        owner,
        { fileId, signal: record.controller.signal },
        async (signal, check, file) => {
          if (file.mimeType !== SHEET) throw new Error('Select a Google spreadsheet.')
          this.#valid(record)
      ...
      .catch((error) => {
        ...
        if (record.state === 'preparing')
          this.#finish(record, 'failed', { state: 'failed', message })
      })
    if (signal?.aborted) cancel()
    return result
  }
  #resolve({ sessionId, callId, requestId } = {}) {
    ...
    if (
      !record ||
      record.owner !== owner ||
      (requestId !== undefined && requestId !== record.requestId)
    )
      throw new Error(EXPIRED)
    return record
  }
  status(input) {
    ...
    if (ACTIVE.has(record.state)) {
      ...
      try {
        ...
        record.result = {
          state: record.state,
          ...(record.state === 'uncertain' ? { message: UNCERTAIN } : {}),
        }
      }
    }
    return {
      ...
      ...(record.preview ? { preview: copy(record.preview) } : {}),
      ...(record.result ? { result: copy(record.result) } : {}),
    }
  }
  deny(input) {
    const record = this.#resolve(input)
    if (!input.requestId || !['preparing', 'pending'].includes(record.state))
      throw new Error(EXPIRED)
    this.#valid(record)
  ...
  async #apply(record) {
    ...
    try {
      ...
      await this.editRuntime.permissions.withAuthorized(
        record.owner,
        { fileId: record.fileId, signal: record.controller.signal },
        async (signal, check, file) => {
      ...
      this.#valid(record)
      const result = await this.writeClient.apply({
        proposal,
        signal: record.permissionSignal,
        beforeDispatch: () => {
          this.#valid(record)
          dispatched = true
        },
