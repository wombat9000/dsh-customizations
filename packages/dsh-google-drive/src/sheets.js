...
const formatSchema = {
  ...
  textFormat: {
    fontFamily: (v) => text(v, 100),
    fontSize: (v) => integer(v, 1, 400),
    bold: boolean,
    italic: boolean,
...
function format(v, schema = formatSchema, strict = true) {
  ...
  if (schema === color)
    for (const v of Object.values(out)) if (v !== null && (v < 0 || v > 1)) fail()
  if (schema === colorStyle) {
    ...
    if (
      strict &&
      out.rgbColor?.alpha !== undefined &&
      out.rgbColor.alpha !== null &&
      out.rgbColor.alpha !== 1
    )
      fail()
  }
...
function url(fileId, params, write = false) {
  const u = new URL(
...
function valueOf(v, effective = false) {
  ...
  const k = ks[0]
  if (k === 'errorValue') {
    keys(v[k], ['type', 'message'])
    return { errorValue: { type: text(v[k].type, 100), message: text(v[k].message ?? '', 1000) } }
  }
  return {
...
function normalize(raw, fileId, r) {
  ...
  for (const grid of sheet.data ?? []) {
    const sr = grid.startRow ?? 0,
      sc = grid.startColumn ?? 0
    integer(sr, r.start.row, r.end.row)
    integer(sc, r.start.col, r.end.col)
    if (
...
function patch(target, input, prefix, masks) {
  for (const [k, v] of Object.entries(input)) {
    ...
    if (v === null) {
      ...
      target[k] ??= {}
      patch(target[k], v, path, masks)
      if (!Object.keys(target[k]).length) delete target[k]
    } else {
      target[k] = v
      masks.push(path)
    }
...
function changedFormat(current, input) {
  ...
  for (const [k, v] of Object.entries(input)) {
    if (v === null) {
      ...
      const nested = changedFormat(current[k] ?? {}, v)
...
export class GoogleSheetsClient {
  ...
  async describe(options) {
    ...
    return freeze({
      fileId,
      title: text(raw.properties?.title ?? '', 1000),
      tabs: raw.sheets
        .slice(0, 100)
        .filter((s) => !s.properties?.sheetType || s.properties.sheetType === 'GRID')
        .map((s) => ({
          ...tabOf(s.properties),
          index: integer(s.properties.index ?? 0, 0, 10000),
        })),
