...
export class DriveDocumentReader {
  ...
  async readText({ fileId, maxBytes = 262_144, signal, ...pdfOptions } = {}) {
    if (!Number.isInteger(maxBytes) || maxBytes < 1 || maxBytes > 1_048_576)
      throw new Error('Invalid Google Drive read limit.')
    if (signal !== undefined && !(signal instanceof AbortSignal))
      throw new Error('Invalid cancellation signal.')
    if (this.#disposed || signal?.aborted) throw cancelled()
    if (
      Object.keys(pdfOptions).some(
        (key) => !['startPage', 'endPage', 'ocr', 'languages'].includes(key),
      )
    )
      throw new Error('Invalid Google Drive read options.')
    ...
    if (metadata.mimeType === 'application/pdf')
      return this.#readPdf(metadata, { fileId, maxBytes, signal, ...pdfOptions })
    if (Object.keys(pdfOptions).length)
      throw new Error('Page ranges and OCR options require a PDF file.')
    const exportMime =
      metadata.mimeType === 'application/vnd.google-apps.document' ? 'text/plain' : undefined
    if (!exportMime && !TEXT_MIMES.includes(metadata.mimeType))
      throw new Error('Unsupported Google Drive MIME type.')
    ...
    return this.#transport.request({
      ...
      prepare: () => {
        ...
        url.search = new URLSearchParams(
          exportMime ? { mimeType: exportMime } : { alt: 'media' },
        ).toString()
  ...
  async #readPdf(metadata, options) {
    ...
    if (
      ...
      BigInt(metadata.size) > BigInt(PDF_LIMIT)
    )
      throw new Error('PDF exceeds the 20 MiB download limit.')
    if (this.#reads.size >= 2)
      throw new Error('PDF reader is busy. Retry after an active read finishes.')
