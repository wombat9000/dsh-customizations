...
export class GoogleDriveClient {
  ...
  constructor(options = {}) {
    ...
    this.#documents = new DriveDocumentReader({
  ...
  async listFiles(options = {}) {
    return this.pickerList(options)
  }
  ...
  async pickerList({
    ...
    if (
      !['my-drive', 'shared-with-me'].includes(view) ||
      query !== undefined ||
      (parentId !== undefined && !validId(parentId)) ||
    ...
    if (search)
      filters.push(`name contains '${search.replaceAll('\\', '\\\\').replaceAll("'", "\\'")}'`)
  ...
  async listFolder({ folderId, ...options } = {}) {
    if (
      !validId(folderId) ||
      Object.keys(options).some((key) => !['pageSize', 'pageToken', 'signal'].includes(key))
    )
      throw new Error('Invalid Google Drive folder options.')
  ...
  async getMetadata({ fileId, signal } = {}) {
    ...
    if (result.id !== fileId || result.trashed !== false || !Array.isArray(result.parents))
      throw new Error('Invalid Google Drive metadata.')
    return result
  }

  async readText(options = {}) {
    return this.#documents.readText(options)
  }
        ...
        url.search = new URLSearchParams(
          fileId
            ...
            : {
                ...
                spaces: 'drive',
                ...(pageToken ? { pageToken } : {}),
                q: query ? `trashed = false and (${query})` : 'trashed = false',
              },
        ).toString()
        return {
          url: url.toString(),
...
function projectMetadata(result, { fileId, pageSize }) {
  ...
  if (
    ...
    result.files.length > pageSize ||
    (result.nextPageToken !== undefined && !text(result.nextPageToken, 4096))
  )
    throw new Error('Invalid Google Drive response.')
  const files = result.files.map((file) => {
    if (!file || !text(file.id, 1024) || !text(file.name, 4096) || !text(file.mimeType, 256))
      throw new Error('Invalid Google Drive response.')
    ...
    if (text(file.webViewLink, 4096)) {
      try {
        ...
        if (
          ...
          !link.password &&
          !link.port &&
          ['drive.google.com', 'docs.google.com'].includes(link.hostname)
        )
          output.webViewLink = link.href
      } catch {
    ...
    if (fileId && file.parents === undefined) output.parents = []
      ...
      file.parents.length <= 100 &&
      file.parents.every((id) => validId(id))
    )
      output.parents = [...file.parents]
    return output
  })
  if (fileId) return files[0]
  return {
    files: files.filter((file) => file.trashed !== true),
