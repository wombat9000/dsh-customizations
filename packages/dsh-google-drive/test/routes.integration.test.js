...
async function fixture(t) {
  ...
  const client = {
    async pickerList(args) {
      ...
      return {
        files: [folder, file],
        ...(args.pageToken ? {} : { nextPageToken: 'google-private-cursor' }),
      }
    },
    async getMetadata({ fileId }) {
      return fileId === 'folder'
        ...
        : fileId === 'file'
          ? file
          : (() => {
              throw new Error('PRIVATE_GOOGLE_ERROR')
            })()
    },
  }
  const googleAuth = {
  ...
  const server = createServer((req, res) => {
    ...
    if (!['status', 'browse', 'grant', 'deny', 'manage', 'revoke'].includes(action)) {
      res.writeHead(404)
      res.end()
      return
    }
...
test('actual HTTP routes match client transport from pending through browse, grant, manage and revoke', async (t) => {
  ...
  assert.deepEqual(
    first.files.map((item) => item.name),
    ['Reports', 'Budget'],
  )
  ...
  assert.equal(browseCalls.at(-1).parentId, 'folder')
  assert.equal(browseCalls.at(-1).search, 'Budget')
  assert.deepEqual(
    audit,
    [],
    'requests and local browse/search must not append custom session events',
  )
  const granted = await plugin.api('grant', {
    ...request,
    selected: [{ id: 'folder', recursive: true }],
  })
  assert.equal(plugin.validStatus(granted), true)
  assert.equal(granted.state, 'granted')
  assert.deepEqual(granted.grants, [
    { id: folder.id, name: folder.name, mimeType: folder.mimeType, recursive: true },
  ])
  ...
  assert.equal(plugin.validStatus(revoked), true)
  assert.deepEqual(revoked.grants, [])
...
test('HTTP boundary rejects cross-origin and malformed requests without leaking provider details', async (t) => {
  ...
  assert.equal(
    (
      await post('grant', {
        ...request,
        selected: [{ id: 'file', recursive: false, permission: 'write' }],
      })
    ).status,
    409,
  )
  ...
  assert.equal(denied.result.value.state, 'denied')
...
test('failed grant settles dead pending request so Manage can mint a usable identity', async (t) => {
  ...
  const pending = await plugin.api('status', identity)
  const failed = await post('grant', {
    ...identity,
    requestId: pending.requestId,
    selected: [{ id: 'missing', recursive: false }],
  })
  ...
  assert.notEqual(managed.requestId, pending.requestId)
  assert.equal(
    (await plugin.api('browse', { ...identity, requestId: managed.requestId })).files.length,
    2,
  )
})
