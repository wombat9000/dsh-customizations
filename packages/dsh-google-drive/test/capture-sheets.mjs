...
const theme = await readFile(
  join(dirname(cli.resolve('@deepseek-ai/dsh-client-ui-theme/package.json')), 'lib/client.js'),
  'utf8',
)
...
const react = await readFile(
  join(dirname(require.resolve('react/package.json')), 'umd/react.production.min.js'),
  'utf8',
)
const reactDOM = await readFile(
  join(dirname(require.resolve('react-dom/package.json')), 'umd/react-dom.production.min.js'),
  'utf8',
)
...
try {
  for (const [name, themeName, width, height] of [
    ['dark', 'dark', 1200, 1000],
    ['light', 'light', 1200, 1000],
    ['narrow', 'dark', 390, 844],
    ...
    await page.setContent(
      `<html><head><style>${css}body{margin:0;background:var(--dsw-alias-bg-layer-2);font-family:system-ui}</style></head><body ${themeName === 'dark' ? 'data-ds-dark-theme' : ''}><div id="app"></div></body></html>`,
    )
    ...
    await page.evaluate(() => {
      const plugin = window.module.factory(() => window.React),
      ...
      const header = {
        backgroundColor: { red: 0.12, green: 0.28, blue: 0.24 },
        textFormat: { bold: true, foregroundColor: { red: 1, green: 1, blue: 1 } },
      }
      const before = {
        fileId: 'synthetic-budget',
        range: 'Budget!A1:C3',
        tab,
        cells: [
          { cell: 'A1', userEnteredValue: { stringValue: 'Category' } },
          { cell: 'B1', userEnteredValue: { stringValue: 'Planned' } },
          { cell: 'C1', userEnteredValue: { stringValue: 'Actual' } },
          { cell: 'A2', userEnteredValue: { stringValue: 'Hosting' } },
          { cell: 'B2', userEnteredValue: { numberValue: 120 } },
          { cell: 'C2', userEnteredValue: { numberValue: 145 } },
          { cell: 'A3', userEnteredValue: { stringValue: 'Draft note' } },
          { cell: 'B3', userEnteredValue: { numberValue: 200 } },
          { cell: 'C3', userEnteredValue: { formulaValue: '=SUM(C2)' } },
        ],
      }
      const after = {
        ...before,
        cells: before.cells.map((c, i) =>
          i < 3
            ? { ...c, userEnteredFormat: header }
            : i === 6
              ? { cell: c.cell }
              : i === 5
                ? {
                    ...c,
                    userEnteredFormat: {
                      backgroundColor: { red: 1, green: 0.88, blue: 0.8 },
                      numberFormat: { type: 'CURRENCY', pattern: '€#,##0.00' },
                    },
                  }
                : i === 8
                  ? { cell: c.cell, userEnteredValue: { formulaValue: '=SUM(C2:C2)*1.2' } }
                  : c,
        ),
      }
      const status = {
      ...
      window.root.render(
        h(plugin.PreviewDialog, {
          status,
          busy: false,
          error: '',
          act() {
            throw new Error('Screenshots cannot write')
          },
          close() {},
        }),
      )
        ...
        window.root.render(
          h(plugin.Picker, {
            entry: {
              ...
              mode: 'edit',
              status: { state: 'pending', mode: 'edit', requestId: 'synthetic', grants: [] },
              request: async (method, body) => {
                assertMethod(method)
                return {
                  files:
                    body.view === 'shared-with-me'
                      ? [
                          {
                            id: 'shared-budget',
                            name: 'Partner budget',
      ...
      function assertMethod(method) {
        if (method !== 'edit-browse') throw new Error('Screenshot fixture permits browsing only')
      }
    })
    ...
    await page.locator('.gs-body').evaluate((e) => {
