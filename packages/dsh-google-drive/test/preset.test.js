...
function rosterConfig(baseUrl, patches) {
  ...
  const rows = composeEntries(
    patches.map((path) => loadOverlayPatches('drive-test', path)),
...
test('Drive adds no preset and personal-web preserves all custom roots', async (t) => {
  ...
  for (const combined of [false, true]) {
    const config = rosterConfig(
      f.baseUrl,
      combined ? await recipePatches() : [webPatch, drivePatch],
    )
    ...
    if (combined)
      assert.deepEqual(
        roots,
        ['dsh-worktree', 'dsh-project-steward', 'dsh-product-mode'].map((name) => ({
          path: join(repo, 'packages', name, 'presets'),
          trust: 'system',
        })),
      )
    const roster = await discoverPresets(
      [{ path: SHIPPED_PRESET_ROOT, trust: 'system' }, ...roots],
      f.baseUrl,
    )
    for (const id of [
      ...shipped.map((row) => row.id),
      ...(combined ? ['worktree-coordinator', 'project-steward', 'product-mode'] : []),
    ]) {
    ...
    assert.equal(
      roster.some((row) => row.id === 'google-drive'),
      false,
    )
  }
})
...
test('Drive insertion preserves arbitrary custom roster configuration', async (t) => {
  ...
  const config = {
    ...
    roots: [
      { path: '/custom/first', trust: 'user' },
      { path: '/custom/second', trust: 'system' },
    ],
  }
  const rows = composeEntries([
