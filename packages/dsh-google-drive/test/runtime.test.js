...
function fixture(t, mode = 'read') {
  ...
  const agent = {
    session: {
      id: 'session',
      append() {
        assert.fail('Drive must not append custom session events')
      },
    },
  }
...
test('both access modes forward picker view and preserve read versus individual edit boundaries', async (t) => {
  for (const mode of ['read', 'edit']) {
    ...
    client.pickerList = async (options) => {
      calls.push(options)
      return { files: [folder, sheet, file('notes')] }
    }
    ...
    for (const view of ['my-drive', 'shared-with-me']) {
      ...
      assert.deepEqual(
        result.files.map((item) => item.id),
        mode === 'edit' ? ['folder', 'sheet'] : ['folder', 'sheet', 'notes'],
      )
    }
...
test('pending requests expire and late browser confirmations remain inert', async (t) => {
  ...
  await assert.rejects(
    runtime.grant({ ...input, selected: [{ id: 'notes', recursive: false }] }),
    /active/,
  )
  assert.deepEqual(runtime.resources(agent), [])
})
...
test('runtime pending picker is private; grant audit precedes returned permission result', async (t) => {
  ...
  assert.deepEqual(
    runtime.resources(agent).map((value) => value.id),
    ['notes'],
  )
  await assert.rejects(
    runtime.grant({ ...input, selected: [{ id: 'notes', recursive: false }] }),
...
test('runtime rejects copied agent identities, child sessions and replaced root', async (t) => {
  ...
  const { input, done } = await request()
  await assert.rejects(
    runtime.request({ session: agent.session }, { callId: 'copy', reason: 'read' }),
    /exact live/,
  )
  const child = { session: { id: 'child', append() {} } }
  agentsById.set('child', child)
  await assert.rejects(
    runtime.request(child, { callId: 'child-call', reason: 'read' }),
    /exact live/,
  )
...
test('runtime reconnect cancels pending UI and revokes prior grants', async (t) => {
  ...
  assert.deepEqual(runtime.resources(agent), [])
  await assert.rejects(
    runtime.grant({ ...second.input, selected: [{ id: 'notes', recursive: false }] }),
    /active/,
  )
...
test('public service gates reads and observer exposure until grant transition commits', async (t) => {
  ...
  const googleAuth = {
    withAccessToken: async (_id, operation) =>
      operation('synthetic-token', new AbortController().signal),
  ...
  service = new GoogleDriveService({
    googleAuth,
    agents: { get: (id) => (id === agent.session.id ? agent : undefined), roots: () => [agent] },
  ...
  service.browser('session-set', {
    ...
    ownerId: sessionStatus.ownerId,
    revision: sessionStatus.revision,
    enabled: true,
  })
  let observing = false
  ...
  const input = {
    sessionId: agent.session.id,
    callId: 'call',
    requestId: service.browser('status', { sessionId: agent.session.id, callId: 'call' }).requestId,
  }
  observing = true
  const granted = service.browser('grant', {
    ...input,
    selected: [{ id: 'notes', recursive: false }],
  })
  await entered.promise
...
for (const mode of ['read', 'edit']) {
  test(`${mode} request and manage reject a toolbar epoch changed during auth lookup`, async (t) => {
    ...
    f.runtime.deny(input)
    await done
    const status = await f.googleAuth.status()
    const gate = deferred(),
      entered = deferred()
    f.googleAuth.status = async () => {
      entered.resolve()
      return gate.promise
    }
  ...
  test(`${mode} old access cards cannot mutate a later toolbar epoch`, async (t) => {
    ...
    await assert.rejects(
      f.runtime.grant({ ...input, selected: [{ id: 'notes', recursive: false }] }),
      /toolbar/,
    )
...
test('runtime request rejects disabled prompting and unauthenticated Drive', async (t) => {
  ...
  policy('ask')
  googleAuth.status = async () => ({
    connected: true,
    integrations: [{ id: 'google-drive', authorized: false }],
  })
