...
test('choose, search, recursive review and grant send only selected IDs', async () => {
  ...
  await click(page.getByRole('checkbox', { name: 'Reports (folder)' }))
  expect(page.getByRole('list', { name: 'Selected access' }).element().textContent).toContain('folder and all descendants (recursive)')
...
test('folder navigation, pagination and session changes keep request identity scoped', async () => {
  const fixture = await mount((method, body) => method !== 'browse' ? pending : body.parentId ? { files: [file], ...(body.pageToken ? {} : { nextPageToken: 'page2' }) } : { files: [folder] })
  await click(page.getByRole('button', { name: 'Choose files and folders' }))
  await click(page.getByRole('button', { name: 'Open Reports' }))
...
test('stale search responses cannot replace current results and failed grants keep selection', async () => {
  ...
  await expect.element(page.getByRole('checkbox', { name: 'Reports (folder)' })).not.toBeInTheDocument()
