  ...
  await act(async () =>
    root.render(
      h(
        React.Fragment,
        null,
        h(plugin.Card, { sessionId: 'session', callId: 'call', picker, request }),
      ...
      await act(async () =>
        root.render(
          h(
            React.Fragment,
            null,
            h(plugin.Card, { sessionId, callId: 'call', picker, request }),
            h(plugin.Overlay, { picker }),
          ),
        ),
      )
...
test('choose, search, recursive review and grant send only selected IDs', async () => {
  const fixture = await mount((method) =>
    method === 'browse'
      ? { files: [folder, file] }
      : method === 'grant'
        ? { state: 'granted', grants: [{ ...folder, recursive: true }] }
        : pending,
  )
  await click(page.getByRole('button', { name: 'Choose files and folders' }))
  await expect.element(page.getByRole('dialog')).toBeVisible()
  expect(fixture.calls.find((call) => call.method === 'browse').body.parentId).toBeUndefined()
  ...
  expect(page.getByRole('list', { name: 'Selected access' }).element().textContent).toMatch(
    /descendants|subfolders|future/i,
  )
  await act(async () => page.getByRole('searchbox').fill('Budget'))
  await click(page.getByRole('button', { name: 'Search', exact: true }))
  expect(fixture.calls.filter((call) => call.method === 'browse').at(-1).body).toEqual({
    sessionId: 'session',
    callId: 'call',
    requestId: 'opaque',
    view: 'my-drive',
  ...
  expect(fixture.calls.find((call) => call.method === 'grant').body).toEqual({
    sessionId: 'session',
    callId: 'call',
    requestId: 'opaque',
  ...
  const fixture = await mount((method) =>
    method === 'browse'
      ...
      : method === 'manage'
        ...
        : method === 'revoke'
          ? { state: 'none', grants: [] }
          : { state: 'granted', grants },
  )
  await click(page.getByRole('button', { name: 'Manage access' }))
  await expect.element(page.getByRole('dialog')).toBeVisible()
  expect(fixture.calls.find((call) => call.method === 'manage').body).toEqual({
    sessionId: 'session',
    callId: 'call',
  })
  await click(page.getByRole('button', { name: 'Revoke all access' }))
  expect(fixture.calls.find((call) => call.method === 'revoke').body).toEqual({
    sessionId: 'session',
    callId: 'call',
  })
})
test('folder navigation, pagination and session changes keep request identity scoped', async () => {
  const fixture = await mount((method, body) =>
    method !== 'browse'
      ? pending
      : body.parentId === 'folder'
        ? { files: [file], ...(body.pageToken ? {} : { nextPageToken: 'page2' }) }
        : { files: [folder] },
  )
  await click(page.getByRole('button', { name: 'Choose files and folders' }))
...
test('review removal updates selection without granting and search-result folders reset navigation', async () => {
  const fixture = await mount((method) =>
    method === 'browse' ? { files: [folder, file] } : pending,
  )
  await click(page.getByRole('button', { name: 'Choose files and folders' }))
