...
async function mount(handler, sessionId = 'a') {
  ...
  const render = async (id) =>
    act(async () => root.render(h(plugin.SessionToggle, { sessionId: id, api })))
...
test('strict session status rejects malformed and ambiguous authority', () => {
  for (const value of [
    ...
    { available: false, enabled: false, ownerId: 'x' },
    status(false, -1),
    status(false, 1.5),
    status(false, Number.MAX_SAFE_INTEGER + 1),
    status(false, 0, ''),
    { ...status(), enabled: 'false' },
  ])
    expect(plugin.validSessionStatus(value)).toBe(false)
...
test('default OFF, exact one same-origin POST enables; OFF warning explains boundaries', async () => {
  const fetch = vi.spyOn(window, 'fetch').mockImplementation(async (url, init) => ({
    ok: true,
    json: async () => ({
      ok: true,
      value: url.endsWith('session-set') ? status(true, 1) : status(),
    }),
  }))
  container = document.createElement('div')
  document.body.append(container)
  ...
  expect(container.querySelector('[tabindex]').getAttribute('aria-label')).toMatch(
    /no file access.*OFF revokes.*cannot undo dispatched writes/,
  )
})
...
test.each(['network', 'stale'])(
  '%s mutation failure re-reads; retry never repeats a mutation',
  async (failure) => {
    let reads = 0,
      recover
    const fixture = await mount((method) => {
      if (method === 'session-set') {
        if (failure === 'stale') return status(true, 0, 'old-owner')
...
test('failed recovery locks switch until successful read-only retry', async () => {
  let healthy = true
  await mount((method) => {
    if (!healthy || method === 'session-set') throw new Error('offline')
    return status()
  })
  healthy = false
  await click()
...
test('OFF sends exact revision and blank reset aborts without assuming unavailable', async () => {
  let blank = false,
    resolveRead
  const calls = [],
    request = async (method, body, signal) => {
      calls.push({ method, body, signal })
      if (blank)
        return new Promise((resolve) => {
          resolveRead = resolve
  ...
  const render = () =>
    act(async () =>
      root.render(h(plugin.SessionToggle, { sessionId: 'a', api: request, useSession })),
    )
  await render()
  await click()
