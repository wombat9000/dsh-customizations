    ...
    root.render(
      h(plugin.Picker, {
        entry: {
          ...
          sessionId: 's',
          callId: 'c',
          status: { state: 'pending', requestId: 'r', grants: [] },
          request,
          onChanged() {},
        },
        close() {},
...
for (const mode of ['read', 'edit'])
  test(`${mode}: tabs reset navigation, preserve combined selection and global search`, async () => {
    const calls = await mount(mode, (method, body) =>
      method.endsWith('grant')
    ...
    await expect
      .element(page.getByRole('tabpanel', { name: 'Search results across all of Drive' }))
      .toBeVisible()
    ...
    await click(
      page.getByRole('button', {
        name: mode === 'edit' ? 'Allow editing for this session' : 'Allow read access',
      }),
    )
    expect(calls.at(-1).method).toBe(mode === 'edit' ? 'edit-grant' : 'grant')
    expect(calls.at(-1).body.selected).toEqual([
      { id: 'mine', recursive: false },
      { id: 'shared', recursive: false },
    ])
  })
test('tabs use roving focus and ArrowLeft/Right, Home and End activation', async () => {
  await mount('read', () => ({ files: [] }))
  const my = page.getByRole('tab', { name: 'My Drive', exact: true }),
    sharedTab = page.getByRole('tab', { name: 'Shared with me' })
  for (const [key, target] of [
    ['ArrowRight', sharedTab],
    ['ArrowRight', my],
    ['ArrowLeft', sharedTab],
    ['Home', my],
    ['End', sharedTab],
...
test('aborts and discards stale pagination across tabs, search and folder navigation', async () => {
  const deferred = []
  const calls = await mount('read', (method, body) =>
    body.pageToken
      ? new Promise((resolve) => deferred.push(resolve))
      : { files: [folder, body.view === 'shared-with-me' ? shared : mine], nextPageToken: 'next' },
  )
  for (const transition of [
    () => click(page.getByRole('tab', { name: 'Shared with me' })),
    async () => {
      await act(async () => page.getByRole('searchbox').fill('global'))
      await click(page.getByRole('button', { name: 'Search', exact: true }))
    },
    ...
    await act(async () =>
      deferred.at(-1)({
        files: [{ ...mine, id: 'stale', name: 'Stale result' }],
        nextPageToken: 'stale-page',
      }),
    )
    await expect
      .element(page.getByRole('checkbox', { name: 'Stale result' }))
