...
const after = {
  ...
  cells: [
    ...
    {
      cell: 'B2',
      userEnteredValue: { formulaValue: '=B1*3' },
      effectiveValue: { numberValue: 999999 },
    },
  ],
}
const pending = {
  ...
  await act(async () =>
    root.render(
      h(
        React.Fragment,
        null,
        h(plugin[component], { sessionId: 's', callId: 'c', picker, request }),
        h(plugin.Overlay, { picker }),
      ),
    ),
  )
  return {
    ...
    async switchSession(sessionId) {
      await act(async () =>
        root.render(h(plugin[component], { sessionId, callId: 'c', picker, request })),
      )
    },
  }
}
test('edit picker navigates folders but grants only native spreadsheets', async () => {
  ...
  const fixture = await mount('EditCard', (method) =>
    method === 'edit-browse'
      ? {
          files: [
  ...
  expect(fixture.calls.find((c) => c.method === 'edit-grant').body).toEqual({
    sessionId: 's',
    callId: 'c',
    requestId: 'opaque',
    selected: [{ id: 'sheet', recursive: false }],
  })
})
test('preview displays exact clear/formula/format details and applies identity only once', async () => {
  const fixture = await mount('PreviewCard', (method) =>
    method === 'preview-apply' ? { ...pending, state: 'applied' } : pending,
  )
  ...
  expect(fixture.calls.find((c) => c.method === 'preview-apply').body).toEqual({
    sessionId: 's',
    callId: 'c',
    requestId: 'opaque',
  })
...
test('lost apply response becomes terminal uncertain without retry', async () => {
  await mount('PreviewCard', (method) => {
    if (method === 'preview-apply') throw new Error('lost')
    return pending
  })
  await click(page.getByRole('button', { name: 'Review changes' }))
  await click(page.getByRole('button', { name: 'Apply changes', exact: true }))
...
test('cancel proposal sends deny identity, never a write', async () => {
  const fixture = await mount('PreviewCard', (method) =>
    method === 'preview-deny' ? { ...pending, state: 'denied' } : pending,
  )
  await click(page.getByRole('button', { name: 'Review changes' }))
  await click(page.getByRole('button', { name: 'Cancel proposal' }))
  expect(fixture.calls.find((c) => c.method === 'preview-deny').body).toEqual({
    sessionId: 's',
    callId: 'c',
    requestId: 'opaque',
  })
...
test('preparing proposal can be cancelled without any apply', async () => {
  const fixture = await mount('PreviewCard', (method) => ({
    state: method === 'preview-deny' ? 'cancelled' : 'preparing',
    requestId: 'opaque',
  }))
  await click(page.getByRole('button', { name: 'Cancel proposal' }))
  expect(fixture.calls.find((c) => c.method === 'preview-deny').body).toEqual({
...
test.each([
  [100, 2],
  [10, 20],
])('all 200 changes remain reviewable for %i rows and %i columns', async (rows, columns) => {
  const cells = Array.from({ length: rows * columns }, (_, i) => ({
    cell: `${String.fromCharCode(65 + (i % columns))}${Math.floor(i / columns) + 1}`,
    userEnteredValue: { stringValue: 'before' },
  }))
  ...
  const proposal = {
    ...
    preview: {
      ...pending.preview,
      fileName: 'Annual operating budget',
      before: { ...before, cells },
      after: {
        ...after,
        cells: cells.map((c, i) => ({
          ...c,
          userEnteredValue: { stringValue: i === 199 ? long : 'after' },
        })),
      },
    },
  }
...
test('exact values distinguish types, empty strings, clearing and whitespace', async () => {
  ...
  const snapshot = (list) => ({
    ...before,
    cells: list.map((value, i) => ({ cell: `A${i + 1}`, userEnteredValue: value })),
  })
  await mount('PreviewCard', () => ({
    ...pending,
    preview: { ...pending.preview, before: snapshot(values), after: snapshot(next) },
  }))
  await click(page.getByRole('button', { name: 'Review changes' }))
  const dialog = page.getByRole('dialog').element()
  expect(dialog.textContent).toContain('1 cleared')
  const details = dialog.querySelectorAll('.gs-detail')
  expect(details[0].textContent).toContain('Number')
  expect(details[0].textContent).toContain('"1"Text')
  expect(details[1].textContent).toContain('Boolean')
  expect(details[1].textContent).toContain('"TRUE"Text')
  expect(details[2].textContent).toContain('""Empty string')
  expect(details[2].textContent).toContain('Empty cell · Clear cell value')
  expect(details[3].textContent).toContain('""Empty string')
  expect(details[3].textContent).not.toContain('Clear cell value')
  expect(details[4].querySelectorAll('.gs-value')[1].textContent).toBe(
    JSON.stringify(next[4].stringValue),
  )
...
test('empty RGB objects and cleared formats have exact visible descriptions', async () => {
  const a = [
    { cell: 'A1' },
    { cell: 'A2', userEnteredFormat: { backgroundColor: {} } },
    { cell: 'A3' },
  ]
  const b = [
    { cell: 'A1', userEnteredFormat: { backgroundColor: {} } },
    { cell: 'A2' },
    { cell: 'A3', userEnteredFormat: { backgroundColorStyle: { rgbColor: {} } } },
  ]
  await mount('PreviewCard', () => ({
    ...pending,
    preview: { ...pending.preview, before: { ...before, cells: a }, after: { ...after, cells: b } },
  }))
  await click(page.getByRole('button', { name: 'Review changes' }))
...
test('real GoogleSheetsClient.prepare fixture exposes every typed and format transition', async () => {
  ...
  expect(detail('B1')).toContain('ACCENT1')
  expect(detail('B1')).toContain('→ Default / unset')
  expect(detail('C1')).toContain('ACCENT2 → Default / unset')
  expect(detail('C1')).toContain('Rgb Color › Blue')
  expect(detail('D1')).toContain('CURRENCY → NUMBER')
  expect(detail('D1')).toContain('Number Format › Pattern')
  expect(detail('D1')).toContain('Empty string')
  expect(detail('E1')).toContain('Clear cell value')
  expect(detail('E1')).toContain('Bold')
  expect(detail('A2')).toContain('Default / unset')
  expect(detail('B2')).toContain('Default / unset')
  expect(detail('C2')).toContain('"=1+1"Text')
  expect(detail('D2')).toContain('"=1+1"Formula')
  expect(detail('E2')).toContain('Boolean')
  expect(detail('E2')).toContain('"false"Text')
  expect(detail('E2')).toContain('NONE')
})
test('style projection excludes CSS injection and unbounded font sizes', async () => {
  ...
  const style = plugin.cellStyle({
    ...
    textFormat: { fontFamily: 'x; background:url(https://bad)', fontSize: 999999 },
    position: 'fixed',
  })
