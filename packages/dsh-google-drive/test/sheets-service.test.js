...
const sheet = {
  ...
  sheets: [
    {
      properties: { sheetId: 0, title: 'Tab', gridProperties: { rowCount: 100, columnCount: 20 } },
    },
  ],
}
function fixture(
  t,
  { enabled = true, authStatus, sheetsResponse = () => Response.json(sheet) } = {},
) {
  const registrations = new Set()
  const register = (value) => {
    registrations.add(value)
    return () => registrations.delete(value)
  }
  ...
  const service = new GoogleDriveService({
    agents: {
      get: (id) => (id === 'root' ? agent : id === 'child' ? child : undefined),
      roots: () => [agent],
    },
    approval: { overrideOf: () => 'ask' },
    googleAuth: {
      withAccessToken: (id, fn) => {
        tokens.push(id)
        return fn('synthetic-token')
      },
      ...
      status:
        ...
        (async () => ({
          connected: true,
          integrations: [
            { id: 'google-drive', authorized: true },
            { id: 'google-sheets-edit', authorized: true },
          ],
        })),
    },
  ...
  const toggle = (enabled) =>
    service.browser('session-set', {
      sessionId: 'root',
      ...service.browser('session-status', { sessionId: 'root' }),
      enabled,
    })
  ...
  async function request(edit = false) {
    const callId = `call-${++serial}`,
      input = { sessionId: 'root', callId }
    const done = service[edit ? 'requestEdit' : 'request'](agent, {
      callId,
  ...
  async function grant(edit = false, id = 'book') {
    ...
    await service.browser(edit ? 'edit-grant' : 'grant', {
      ...r.input,
      selected: [{ id, recursive: false }],
    })
    assert.equal((await r.done).state, 'granted')
    return r.input
  }
...
test('Sheets tools pass exact caller, call ID and cancellation, rejecting unknown arguments', async () => {
  const calls = [],
    ...
    exec = { agent, callId: 'exact-call', signal }
  const service = Object.fromEntries(
    ['describeSheets', 'readSheet', 'proposeSheetEdit', 'requestEdit'].map((method) => [
      method,
      (owner, args) => {
  ...
  const cases = [
    [createSheetsDescribeTool(service), { fileId: 'book' }],
    [createSheetsReadTool(service), { fileId: 'book', range: 'Tab!A1' }],
    [
      createSheetsProposeTool(service),
      { fileId: 'book', range: 'Tab!A1', changes: [{ cell: 'A1', value: 2 }] },
    ],
    [createSheetsRequestTool(service), { reason: 'Edit book' }],
  ]
  for (const [tool, args] of cases) {
    ...
    const accessor = { ...args }
    Object.defineProperty(accessor, Object.keys(args)[0], {
      get() {
        assert.fail('must not invoke getter')
      },
      enumerable: true,
    })
    await assert.rejects(tool.execute(accessor, exec))
    if ('fileId' in args)
      for (const fileId of ['', '../secret', 'x'.repeat(201), 4])
        await assert.rejects(tool.execute({ ...args, fileId }, exec))
    if ('range' in args)
      for (const range of ['', 'A1', 'x'.repeat(301), null])
        await assert.rejects(tool.execute({ ...args, range }, exec))
    if ('changes' in args)
      for (const changes of [[], Array(201).fill({}), {}, null])
        await assert.rejects(tool.execute({ ...args, changes }, exec))
    const aborted = new AbortController()
    aborted.abort()
    await assert.rejects(tool.execute(args, { ...exec, signal: aborted.signal }))
    assert.deepEqual(JSON.parse(await tool.execute(args, exec)), { state: 'synthetic' })
    assert.equal(calls.at(-1).owner, agent)
    assert.equal(calls.at(-1).args.signal, signal)
    assert.equal(tool.parameters.additionalProperties, false)
  }
  assert.equal(calls.length, 4)
  assert.equal(calls[2].args.callId, exec.callId)
  assert.equal(calls[3].args.callId, exec.callId)
})
test('assembled service routes read grants to read scope and never authorizes editing', async (t) => {
  ...
  assert.equal(
    (await f.service.readSheet(f.agent, { fileId: 'book', range: 'Tab!A1' })).cells.length,
    1,
  )
  assert.throws(() =>
    f.service.proposeSheetEdit(f.agent, {
      fileId: 'book',
      range: 'Tab!A1',
  ...
  assert.throws(() =>
    f.service.readSheet({ session: f.agent.session }, { fileId: 'book', range: 'Tab!A1' }),
  )
  assert.ok(f.tokens.every((id) => id === 'google-drive'))
  assert.ok(f.calls.every((call) => call.method !== 'POST'))
})
for (const httpStatus of [400, 401, 403, 404, 429, 500])
  test(`Sheets HTTP ${httpStatus} preserves read and edit grants and never dispatches writes`, async (t) => {
    const f = fixture(t, {
      sheetsResponse: () =>
        Response.json(
          {
    ...
    for (const operation of [
      () => f.service.describeSheets(f.agent, { fileId: 'book' }),
      ...
      assert.deepEqual(
        f.service.browser('status', read).grants.map((item) => item.id),
        ['text'],
      )
      assert.deepEqual(
        f.service.browser('edit-status', edit).grants.map((item) => item.id),
        ['book'],
      )
      assert.deepEqual(
        (await f.service.listFiles(f.agent, {})).files.map((item) => item.id),
        ['text'],
      )
    }
    assert.ok(f.calls.every((call) => call.method !== 'POST'))
...
test('assembled edit-only grant permits bounded reads but does not grant arbitrary Drive access', async (t) => {
  const f = fixture(t)
  await f.grant(true)
  assert.equal(f.service.hasAccess(f.agent), false)
  assert.equal(f.service.hasEditAccess(f.agent), true)
  await f.service.readSheet(f.agent, { fileId: 'book', range: 'Tab!A1' })
  assert.ok(f.tokens.every((id) => id === 'google-drive'))
  await assert.rejects(f.service.listFiles(f.agent, {}))
  await assert.rejects(f.service.readText(f.agent, { fileId: 'book' }))
  await assert.rejects(f.service.readSheet(f.agent, { fileId: 'unselected', range: 'Tab!A1' }))
  f.service.release(f.agent)
  assert.equal(f.service.hasEditAccess(f.agent), false)
  await assert.rejects(async () =>
    f.service.readSheet(f.agent, { fileId: 'book', range: 'Tab!A1' }),
  )
})
for (const id of ['folder', 'text'])
  test(`assembled edit picker rejects ${id} resources`, async (t) => {
    const f = fixture(t),
      r = await f.request(true)
    await assert.rejects(
      f.service.browser('edit-grant', { ...r.input, selected: [{ id, recursive: false }] }),
    )
    assert.equal(f.service.hasEditAccess(f.agent), false)
    f.service.release(f.agent)
    await r.done
...
test('opening unrelated read picker preserves pending edit preview; edit revoke cancels it', async (t) => {
  const f = fixture(t),
    editInput = await f.grant(true)
  const done = f.service.proposeSheetEdit(f.agent, {
    fileId: 'book',
    range: 'Tab!A1',
    changes: [{ cell: 'A1', value: 2 }],
    callId: 'preview',
  })
  ...
  for (let i = 0; i < 100; i++) {
    await Promise.resolve()
    if (f.service.browser('preview-status', input).state !== 'preparing') break
  }
...
for (const edit of [false, true])
  test(`OFF during ${edit ? 'edit' : 'read'} auth await prevents a picker even after re-enable`, async (t) => {
    let resume
    const f = fixture(t, {
      authStatus: () =>
        new Promise((resolve) => {
    ...
    const done = f.service[edit ? 'requestEdit' : 'request'](f.agent, {
      callId: 'waiting',
      reason: 'Read',
    })
    f.toggle(false)
    ...
    assert.throws(() =>
      f.service.browser(edit ? 'edit-status' : 'status', { sessionId: 'root', callId: 'waiting' }),
    )
...
test('OFF retains inert permission records and cancels a pending preview', async (t) => {
  ...
  assert.equal(f.service.hasEditAccess(f.agent), false)
  await assert.rejects(async () =>
    f.service.browser('edit-grant', { ...access, selected: [{ id: 'book', recursive: false }] }),
  )
})

test('assembled service rejects arbitrary browser dispatch and releases auth listeners', (t) => {
  const f = fixture(t)
  for (const name of ['apply', 'batchUpdate', 'constructor', '__proto__', 'dispose'])
    assert.throws(() => f.service.browser(name, {}))
  f.service.dispose()
