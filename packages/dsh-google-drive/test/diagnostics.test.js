...
const failure = (status, reason) =>
  `Google request failed (HTTP ${status})${reason ? `: ${reason}` : ''}. Try again or reconnect.`
function fixture(t, fetch, options = {}) {
  const client = new GoogleDriveClient({
    withAccessToken: (operation) => operation('private-token'),
    fetch,
    ...options,
  })
...
test('Drive HTTP diagnostics expose only status and exact allowlisted identifiers', async (t) => {
  for (const [body, reason] of [
    [
      {
        error: {
          errors: [{ reason: 'accessNotConfigured', message: secret }],
    ...
    [
      { error: { details: [{ reason: 'SERVICE_DISABLED', metadata: { url: secret } }] } },
      'SERVICE_DISABLED',
    ],
...
test('Drive bounds error reads and hides malformed, stream, and network errors', async (t) => {
  ...
  await assert.rejects(network.listFiles(), {
    message: 'Google network request failed. Try again.',
  })
})
...
test('Drive abort and disposal interrupt stalled error bodies', async (t) => {
  for (const mode of ['abort', 'dispose', 'timeout']) {
    ...
    const client = fixture(
      ...
      { requestTimeoutMs: mode === 'timeout' ? 10 : 1000 },
    )
