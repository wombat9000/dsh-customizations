...
test('safe folder and literal picker search reject raw queries and path injection', async (t) => {
  ...
  assert.equal(
    ...
    "trashed = false and (name contains 'a\\' or name contains \\'\\\\')",
  )
  for (const fileId of ['../id', 'https://evil.invalid', "id' or true", '', 'a'.repeat(257)])
    await assert.rejects(client.getMetadata({ fileId }), /Invalid/)
  ...
  for (const view of ['my-drive', 'shared-with-me']) {
    for (const parentId of [undefined, 'root', 'nested-folder']) {
      for (const search of [undefined, '', "report'\\\\name"]) {
        await client.pickerList({
          view,
          parentId,
          search,
          pageToken: 'next&token=literal',
          pageSize: 50,
        })
...
test('bounded metadata requires exact ID, nontrash and parents', async (t) => {
  for (const value of [
    { ...file, parents: [], trashed: false },
    { ...file },
    { ...file, id: 'other', parents: [], trashed: false },
    { ...file, parents: [], trashed: true },
    { ...file, parents: [42], trashed: false },
  ]) {
    const { client, calls } = fixture(t, { fetch: () => Response.json(value) })
    if (value.id === 'id' && value.trashed === false && value.parents?.length === 0)
      assert.deepEqual(await client.getMetadata({ fileId: 'id' }), value)
...
test('read-only text download and Docs export use fixed bounded endpoints', async (t) => {
  for (const mimeType of ['text/plain', 'application/vnd.google-apps.document']) {
    ...
    const { client, calls } = fixture(t, {
      fetch: () =>
        ++count === 1
          ? Response.json({ ...file, mimeType, parents: [], trashed: false })
          : new Response('hello'),
    })
    ...
    assert.equal(
      url.pathname,
      mimeType === 'text/plain' ? '/drive/v3/files/id' : '/drive/v3/files/id/export',
    )
    assert.equal(
      url.searchParams.get(mimeType === 'text/plain' ? 'alt' : 'mimeType'),
      mimeType === 'text/plain' ? 'media' : 'text/plain',
    )
...
test('unsupported content MIME fails closed and oversized or malformed text fails', async (t) => {
  for (const mimeType of [
    ...
    const { client, calls } = fixture(t, {
      fetch: () => Response.json({ ...file, mimeType, parents: [], trashed: false }),
    })
  ...
  for (const body of ['123456', new Uint8Array([0xff])]) {
    ...
    const { client } = fixture(t, {
      fetch: () =>
        ++count === 1
          ? Response.json({ ...file, parents: [], trashed: false })
          : new Response(body),
    })
...
function fixture(t, options = {}) {
  ...
  const client = new GoogleDriveClient({
    ...
    fetch: (url, init) => {
      calls.push({ url, init })
      return (options.fetch ?? (() => Response.json({ files: [file] })))(url, init)
    },
  })
...
test('Drive module exports no OAuth or configuration API', () => {
  ...
  assert.deepEqual(Object.getOwnPropertyNames(GoogleDriveClient.prototype).sort(), [
    'constructor',
    'dispose',
    'getMetadata',
    'listFiles',
...
test('projects bounded metadata, excludes trash and unsafe links', async (t) => {
  const links = [
    ...
    'https://drive.google.com.attacker.invalid/file',
  ...
  const { client } = fixture(t, {
    fetch: () =>
      Response.json({
        ...
        secret: 'omit',
        files: [
          ...links.map((webViewLink) => ({ ...file, webViewLink, arbitrary: 'omit' })),
          { ...file, trashed: true },
        ],
      }),
  })
...
test('bounds response shape and sanitizes transport, body, timeout and auth errors', async (t) => {
    ...
    await assert.rejects(client.listFiles(), (error) => {
      assert.ok(
        [
          ...
          'Google operation was cancelled.',
  ...
  for (const result of [
    ...
    { files: [], nextPageToken: 'x'.repeat(4097) },
  ]) {
  ...
  await assert.rejects(client.listFiles(), {
    message: 'Connect Google Drive in Settings → Plugins → Google accounts, then retry.',
  })
