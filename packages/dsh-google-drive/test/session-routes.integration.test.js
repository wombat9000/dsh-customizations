...
async function fixture(t) {
  ...
  const service = {
    assertOwner(agent) {
      if (!agent || live.get(agent.session.id) !== agent || !roots.has(agent))
        throw new Error('Not a live root')
    },
    hasAccess: () => false,
    hasEditAccess: () => false,
    observe: () => () => {},
    revokeSession: (agent) => revoked.push(agent),
    release: () => {},
  }
  const manager = new SessionDriveTools({ service, agents })
  const handlers = {
    'session-status': (input) => manager.status(input),
    'session-set': (input, signal) => manager.set(input, signal),
  }
  let port
  ...
  vm.runInNewContext(source, {
    window: {
      ...
      fetch: (url, options) =>
        fetch(origin + url, { ...options, headers: { ...headers, ...options.headers } }),
    },
  })
...
test('real session HTTP routes and client transport expose only exact root-scoped request tools', async (t) => {
  ...
  assert.deepEqual([...f.a.tools.keys()].sort(), [
  ...
  assert.equal(
    f.a.skills.size,
    0,
    'enabling alone does not expose read skills or grant file access',
  )
  assert.equal(f.b.tools.size, 0)
  assert.equal(f.child.tools.size, 0)
...
function postWith(f, status, enabled) {
  return f.post('session-set', mutation(status, enabled))
}
...
test('session routes reject caller authority, malformed types and missing fields before dispatch', async (t) => {
  ...
  const invalidStatus = [
    ...
    { sessionId: 1 },
    { sessionId: 'a', callId: 'call' },
    { sessionId: 'a', enabled: true },
    { sessionId: 'a', ownerId: initial.ownerId },
  ]
  ...
  const invalidSet = [
    ...
    {},
    { ...valid, sessionId: '' },
    { ...valid, sessionId: 'x'.repeat(201) },
    { ...valid, ownerId: '' },
    { ...valid, ownerId: 'x'.repeat(201) },
    { ...valid, ownerId: {} },
    { ...valid, revision: -1 },
    { ...valid, revision: 0.5 },
  ...
  for (const key of Object.keys(valid))
    invalidSet.push(Object.fromEntries(Object.entries(valid).filter(([name]) => name !== key)))
  ...
  assert.equal(
    (await f.post('session-set', JSON.stringify({ ...valid, extra: 'x'.repeat(33000) }))).status,
    400,
  )
  assert.equal(f.a.tools.size, 0)
  assert.equal(
    (await f.post('session-status', { sessionId: 'a' })).result.value.revision,
    initial.revision,
  )
})
...
test('toolbar HTTP boundary keeps origin/header checks and sanitizes failures', async (t) => {
    ...
    for (const action of ['session-status', 'session-set']) {
      assert.equal(
        (
          await f.post(
            action,
            action === 'session-status' ? { sessionId: 'a' } : mutation(initial, true),
            { headers },
          )
        ).status,
        403,
        `${action}: ${JSON.stringify(headers)}`,
      )
    }
  }
  ...
  f.handlers['session-set'] = () => {
    throw new Error('PRIVATE_FIXTURE_TOKEN_AND_PROVIDER_DETAILS')
  }
...
test('absent/child sessions stay unavailable; old incarnation cannot enable a replacement owner', async (t) => {
  ...
  for (const sessionId of ['archived', 'child']) {
    ...
    assert.equal(
      (await f.post('session-set', { sessionId, ownerId: 'unknown', revision: 0, enabled: true }))
        .status,
      409,
    )
  }
  const old = (await f.post('session-status', { sessionId: 'a' })).result.value
  f.manager.release(f.a.agent)
  f.roots.delete(f.a.agent)
