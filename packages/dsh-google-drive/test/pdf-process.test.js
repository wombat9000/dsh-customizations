...
print(json.dumps({"as":resource.getrlimit(resource.RLIMIT_AS),"cpu":resource.getrlimit(resource.RLIMIT_CPU),"fsize":resource.getrlimit(resource.RLIMIT_FSIZE),"nofile":resource.getrlimit(resource.RLIMIT_NOFILE),"core":resource.getrlimit(resource.RLIMIT_CORE),"secret":os.environ.get("DSH_PDF_TEST_SECRET"),"path":os.environ.get("PATH")}))`,
      ...
      if (process.platform === 'linux') {
        await assert.rejects(
          pdfProcess(tools, 'probe', ['-I', '-c', 'x=bytearray(700*1024*1024)'], { cwd: dir }),
          /processing limits/u,
        )
      }
      await assert.rejects(
        pdfProcess(tools, 'probe', ['-I', '-c', 'open("large", "wb").write(b"x"*(34*1024*1024))'], {
          cwd: dir,
        }),
        /processing limits/u,
      )
    } finally {
      delete process.env.DSH_PDF_TEST_SECRET
...
test(
  'native runner bounds stdout/stderr, times out, and awaits cancellation close',
  native,
  async () => {
    const dir = await mkdtemp(join(tmpdir(), 'dsh-pdf-runner-'))
    try {
      await assert.rejects(
        pdfProcess(tools, 'probe', ['-I', '-c', 'print("x"*100000)'], { cwd: dir, limit: 32 }),
        /output limit/u,
      )
      await assert.rejects(
        pdfProcess(
          tools,
          'probe',
          ['-I', '-c', 'import sys;sys.stderr.write("private-path"*100000)'],
          { cwd: dir },
        ),
        (error) =>
          /diagnostic limit/u.test(error.message) && !error.message.includes('private-path'),
      )
      await assert.rejects(
        pdfProcess(tools, 'probe', ['-I', '-c', 'import time;time.sleep(10)'], {
          cwd: dir,
          timeout: 40,
        }),
        /time limit/u,
      ...
      const promise = pdfProcess(
        tools,
        'probe',
        [
          '-I',
          '-c',
          'import os,signal,time;signal.signal(signal.SIGTERM,signal.SIG_IGN);os.fork();time.sleep(10)',
        ],
