...
function fixture(t) {
  ...
  const client = {
    async getMetadata({ fileId }) {
      ...
      if (!nodes.has(fileId)) throw Error('not found')
      return { ...nodes.get(fileId) }
    },
    async listFolder({ folderId }) {
      calls.push(['folder', folderId])
      return { files: [...nodes.values()].filter((file) => file.parents.includes(folderId)) }
    },
    async pickerList(args) {
      calls.push(['picker', args])
      return { files: [...nodes.values()], nextPageToken: 'private-google-token' }
    },
    async readText() {
      return { text: 'content', mimeType: 'text/plain' }
    },
  }
  const core = new DrivePermissions({
    client,
    getAccountGeneration: () => generation,
    isOwnerLive: (value) => !dead.has(value),
    onChange: (value) => changes.push(value),
  })
  t.after(() => core.dispose())
  const approve = async (selection = { folderIds: ['root'], fileIds: ['explicit'] }) =>
    core.approve(owner, core.request(owner).requestId, selection)
  return {
    core,
    client,
    owner,
...
test('selection is one-shot, owned, private picker separate from agent root', async (t) => {
  ...
  assert.deepEqual(
    (await core.listFiles(owner)).files.map((file) => file.id),
    ['explicit', 'root'],
  )
...
test('picker cursors bind view, search, parent and pending request without granting access', async (t) => {
    ...
    for (const search of [undefined, 'global search']) {
      ...
      for (const change of [
        ...
        await assert.rejects(
          core.pickerList(owner, requestId, { ...options, ...change, pageToken }),
          /permission/,
        )
      await assert.rejects(
        core.pickerList(owner, core.request(owner).requestId, { ...options, pageToken }),
        /permission/,
      )
  ...
  for (const view of ['', 'shared', null, 1, {}, []])
    await assert.rejects(core.pickerList(owner, requestId, { view }), /Invalid/)
...
test('future descendants are allowed and moved-out descendants denied', async (t) => {
  ...
  assert.deepEqual(
    (await core.listFiles(owner, { folderId: 'nested' })).files.map((file) => file.id),
    ['child', 'future'],
  )
...
test('selection rejects shortcuts, folder mismatch, duplicate and malicious IDs', async (t) => {
  ...
  await assert.rejects(
    core.approve(owner, request.requestId, { fileIds: ['explicit'] }),
    /permission/,
  )
})
...
test('opaque cursors are bound to owner, request, parent, search and revision', async (t) => {
  ...
  await assert.rejects(
    core.pickerList(owner, b, { search: 'note', pageToken: page.nextPageToken }),
    /permission/,
  )
  await assert.rejects(
    core.pickerList(owner, a, { search: 'other', pageToken: page.nextPageToken }),
    /permission/,
  )
  await assert.rejects(
    core.pickerList(other, a, { search: 'note', pageToken: page.nextPageToken }),
    /permission/,
  )
  await core.pickerList(owner, a, { search: 'note', pageToken: page.nextPageToken })
  await approve()
  await assert.rejects(
    core.pickerList(owner, a, { search: 'note', pageToken: page.nextPageToken }),
    /permission/,
  )
...
test('replace atomically removes deselected files, empty replacement revokes all', async (t) => {
  ...
  await assert.rejects(
    core.approve(owner, failed, { fileIds: ['root'], replace: true }),
    /selection/,
  )
...
test('read permission forwards only supplied PDF options and retains structured pages', async (t) => {
  ...
  const result = {
    ...
    format: 'pdf',
    mimeType: 'text/plain',
    pages: [{ pageNumber: 4, text: 'OCR text', method: 'ocr' }],
    totalPages: 9,
    actualRange: { startPage: 4, endPage: 4 },
    nextStartPage: 5,
    warnings: [],
  }
  client.readText = async (args) => {
    calls.push(args)
    return result
  }
  const options = {
  ...
  assert.deepEqual(read, {
    ...result,
    file: { id: 'explicit', name: 'explicit', mimeType: 'text/plain' },
  })
  await core.readText(owner, { fileId: 'explicit' })
  for (const key of ['startPage', 'endPage', 'ocr', 'languages'])
    assert.equal(Object.hasOwn(calls[1], key), false)
  await assert.rejects(
    core.readText(owner, { fileId: 'explicit', processor: 'arbitrary' }),
    /Invalid/,
  )
...
test('deferred PDF processing receives abort on revocation, replacement and owner lifetime end', async (t) => {
  for (const mode of ['caller', 'revoke', 'replace', 'account', 'release', 'dispose']) {
    await t.test(mode, async (t) => {
      ...
      const controller = new AbortController()
      const reading = core.readText(owner, {
        fileId: 'explicit',
        startPage: 1,
        ocr: 'force',
      ...
      if (mode === 'caller') controller.abort()
        ...
        await core.approve(owner, core.request(owner).requestId, {
          fileIds: ['explicit'],
          replace: true,
        })
      ...
      stalled.resolve({
        text: 'late OCR secret',
        pages: [{ pageNumber: 1, text: 'late OCR secret', method: 'ocr' }],
      })
...
test('fresh ancestry after deferred OCR rejects moved, trashed and shortcut resources', async (t) => {
  for (const mode of ['moved', 'trashed', 'shortcut', 'parent']) {
    ...
    stalled.resolve({
      text: 'OCR secret',
      pages: [{ pageNumber: 1, text: 'OCR secret', method: 'ocr' }],
    })
    await rejected
  }
})
