...
test('registers verified keyed tool and additive root overlay seats with one owned store', () => {
  ...
  plugin.apply({
    ...
    slots: {
      inject: (name, fn) => fn(),
      register: (options, component) => rows.push({ options, component }),
    },
  })
  assert.equal(record.id, '@local/dsh-google-drive')
  assert.deepEqual(Array.from(plugin.inject), ['slots'])
  const header = rows.find((row) => row.options.name === 'conversation.session.header.utilities')
  assert.equal(header.options.id, 'google-drive-session-toggle')
  assert.equal(header.options.order, 20)
  assert.equal(header.component, plugin.SessionToggle)
  const injected = header.options.inject('session')
  assert.equal(injected.sessionId, 'session')
  assert.equal(injected.api, plugin.api)
  const card = rows.find((row) => row.options.key === 'request_drive_access')
  const overlay = rows.find((row) => row.options.name === 'shell.overlay')
  assert.equal(card.options.name, 'tool.call.toolview')
  assert.equal(overlay.options.id, 'google-drive-access')
  const picker = card.options.inject().picker
  assert.equal(picker, overlay.options.inject().picker)
  picker.open({ owner: {} })
  effects[0]()
  assert.equal(picker.getSnapshot(), null)
})
test('all browser routes use same-origin JSON POST and abort signal', async () => {
  const calls = []
  const { plugin } = load(async (...args) => {
    calls.push(args)
    return { ok: true, json: async () => ({ ok: true, value: {} }) }
  })
  const signal = {},
    body = { sessionId: 's', callId: 'c', requestId: 'opaque' }
  for (const method of ['status', 'browse', 'manage', 'grant', 'deny', 'revoke']) {
    ...
    assert.equal(options.headers['X-DSH-Google-Drive'], '1')
...
test('transport never exposes raw server or network details', async () => {
  for (const fetch of [
    ...
    async () => ({ ok: false, json: async () => ({ ok: false, error: { message: 'SECRET' } }) }),
    async () => ({
      json: async () => {
        throw new Error('SECRET')
      },
...
test('pending status requires opaque identity and picker cleanup is owner scoped', () => {
  ...
  const store = plugin.createPickerStore(),
    owner = {},
    other = {}
  let changes = 0
  const off = store.subscribe(() => changes++)
  store.open({ owner })
  store.closeOwner(other)
  assert.equal(store.getSnapshot().owner, owner)
