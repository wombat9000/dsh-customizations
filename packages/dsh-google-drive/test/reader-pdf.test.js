...
const result = {
  text: '[Page 1]\nExtracted',
  pages: [{ pageNumber: 1, method: 'embedded' }],
  totalPages: 1,
  actualRange: { startPage: 1, endPage: 1 },
  warnings: [],
  format: 'pdf',
  mimeType: 'text/plain',
}
...
function fixture(t, { metadata = file, process, fetch } = {}) {
  ...
  const transport = new GoogleTransport({
    withAccessToken: async (operation) => {
      ...
      try {
        return await operation('private-token', lifetime.signal)
      } finally {
        lifetime.abort()
      }
    },
    fetch:
      ...
      ((_url, init) => {
        calls.push(init)
        return new Response(new Uint8Array([37, 80, 68, 70, 45, 255]))
      }),
  })
...
test('PDF receives raw bounded bytes and operation signal survives successful authentication', async (t) => {
  ...
  assert.deepEqual(
    await reader.readText({ fileId: 'pdf', maxBytes: 100, startPage: 1, ocr: 'off' }),
    { ...result, file },
  )
