...
async function fixture(t) {
  ...
  const registry = (values) => ({
    register(value) {
      ...
      return () => values.delete(value.name)
    },
  })
  const registries = { tools: registry(tools), skills: registry(skills) }
  const agent = {
    session: { id: 'root', append: (...args) => audit.push(args) },
    ctx: {
      get: (key) => registries[key],
      effect(factory) {
        let cleanup = factory()
        return () => {
          const fn = cleanup
  ...
  let entered = { stringValue: 'before' },
    stallWrite = false,
    stallRead = false,
    port
  const service = new GoogleDriveService({
    agents: { get: (id) => (id === 'root' ? agent : undefined), roots: () => [agent] },
    ...
    fetch: async (url, opts) => {
      ...
      const waitForAbort = () =>
        new Promise((_, reject) => {
          const abort = () => {
            events.emit('upstream-aborted')
            reject(new Error('PRIVATE_PROVIDER_FAILURE'))
          }
      ...
      if (u.hostname === 'sheets.googleapis.com') {
        ...
        if (stallRead) {
          const pending = waitForAbort()
          events.emit('read-stalled')
          return pending
        }
                  ...
                  rowData: [
                    {
                      values: [
                        {
                          ...(entered
                            ? { userEnteredValue: entered, effectiveValue: entered }
                            : {}),
                          formattedValue: entered?.stringValue ?? '',
                        },
                      ],
                    },
                  ],
  ...
  const runtime = Object.fromEntries(
    allActions.map((action) => [action, (args, signal) => service.browser(action, args, signal)]),
  )
  ...
  async function post(action, input, options = {}) {
    const response = await fetch(`${origin}/api/plugins/google-drive/${action}`, {
      method: 'POST',
      ...options,
  ...
  const access = service.requestEdit(agent, {
    callId: identity.callId,
    reason: 'Edit synthetic budget',
  })
  async function grant() {
    ...
    assert.equal(
      (await post('edit-grant', { ...input, selected: [{ id: 'book', recursive: false }] })).status,
      200,
    )
  ...
  async function prepare({ stalled = false, signal } = {}) {
    ...
    const done = service.proposeSheetEdit(agent, {
      callId: 'preview',
      fileId: 'book',
      range: 'Tab!A1',
      changes: [{ cell: 'A1', value: 'after' }],
      signal,
    })
    await observed
...
test('edit browse routes carry both views through to Google without granting edit access', async (t) => {
  ...
  for (const view of ['my-drive', 'shared-with-me']) {
    ...
    assert.equal(
      new URL(f.calls.at(-1).url).searchParams.get('q'),
      view === 'my-drive'
        ? "trashed = false and ('root' in parents)"
        : 'trashed = false and (sharedWithMe = true)',
    )
    assert.equal(
      (await f.post('edit-browse', { ...input, view, parentId: 'folder', search: 'Budget' }))
        .status,
      200,
    )
    assert.equal(
...
test(
  'real HTTP edit grant and preview apply sends one exact batch then reads back without custom history',
  { timeout: 5000 },
  async (t) => {
    const f = await fixture(t)
    await f.grant()
    ...
    assert.equal(
      p.status.result.value.preview.before.cells[0].userEnteredValue.stringValue,
      'before',
    )
    ...
    assert.equal(result.status, 200)
    assert.equal(result.result.value.state, 'applied')
    const outcome = await p.done
    assert.equal(outcome.state, 'applied')
    assert.equal(outcome.snapshot.cells[0].userEnteredValue.stringValue, 'after')
    const writes = f.calls.filter((c) => c.method === 'POST')
    assert.equal(writes.length, 1)
    const index = f.calls.indexOf(writes[0])
    assert.ok(
      f.calls.slice(index + 1).some((c) => new URL(c.url).hostname === 'sheets.googleapis.com'),
    )
...
test(
  'every new route rejects cross-origin, missing header, wrong method and replacement payloads',
  { timeout: 10000 },
  async (t) => {
    const f = await fixture(t)
    for (const action of actions) {
      for (const headers of [
        ...
        const result = await f.post(action, f.identity, { headers })
        assert.equal(result.status, 403, action)
        assert.ok(!JSON.stringify(result).includes('PRIVATE_'))
      }
      assert.equal((await f.post(action, f.identity, { method: 'GET' })).status, 405, action)
      for (const field of ['proposal', 'requests', 'accessToken'])
        assert.equal((await f.post(action, { ...f.identity, [field]: {} })).status, 400, action)
      assert.equal(
        (await f.post(action, { ...f.identity, sessionId: 'other' })).status,
        409,
        action,
      )
    }
...
test(
  'action-specific IDs reject mismatched access and preview identities without writes',
  { timeout: 5000 },
  async (t) => {
    const f = await fixture(t)
    const pending = await f.post('edit-status', f.identity)
    for (const action of ['edit-browse', 'edit-grant', 'edit-deny']) {
      const result = await f.post(action, {
        ...f.identity,
        requestId: 'wrong',
        ...(action === 'edit-grant' ? { selected: [{ id: 'book', recursive: false }] } : {}),
      })
    ...
    assert.equal(
      (await f.post('preview-status', p.input)).status,
      400,
      'status accepts no requestId replacement',
    )
...
test(
  'browser denial while preparation stalls aborts upstream read and settles without a write',
  { timeout: 5000 },
  async (t) => {
    const f = await fixture(t)
    await f.grant()
    const p = await f.prepare({ stalled: true })
    assert.equal(p.status.result.value.state, 'preparing')
...
test(
  'toolbar OFF cancels pending picker; ON cannot revive old access cards',
  { timeout: 5000 },
  async (t) => {
    const f = await fixture(t)
...
test(
  'toolbar OFF during preview preparation aborts reads and retains sanitized status',
  { timeout: 5000 },
  async (t) => {
    const f = await fixture(t)
    await f.grant()
...
test(
  'toolbar OFF after write dispatch preserves uncertain tool and HTTP outcome without retry',
  { timeout: 5000 },
  async (t) => {
    const f = await fixture(t)
    await f.grant()
    const p = await f.prepare()
    f.stallWrites()
    const dispatched = once(f.events, 'write-dispatched'),
      aborted = once(f.events, 'upstream-aborted')
    ...
    const status = await f.post('preview-status', {
      sessionId: p.input.sessionId,
      callId: p.input.callId,
    })
    assert.equal(status.status, 200)
...
test(
  'browser disconnect after dispatched POST settles tool as uncertain without leaking provider content',
  { timeout: 5000 },
  async (t) => {
    const f = await fixture(t)
    await f.grant()
