...
try {
  for (const variant of [
    { name: 'dark', width: 980, theme: 'dark' },
    { name: 'light', width: 980, theme: 'light' },
    { name: 'narrow', width: 390, theme: 'dark' },
  ]) {
    ...
    await page.addScriptTag({ content: dom })
    await page.evaluate(() => {
    ...
    await page.evaluate(() => {
      const h = React.createElement,
        plugin = window.module.factory(() => React)
      let enabled = false,
        revision = 0
      window.calls = []
      ...
      ReactDOM.createRoot(document.getElementById('app')).render(
        h(
          ...
          h(
            'header',
            null,
            h('strong', null, 'Quarterly planning'),
            h(
              'nav',
              { 'aria-label': 'Session utilities' },
              h('button', null, 'Session log'),
              h('button', null, 'Recap'),
              h(plugin.SessionToggle, { sessionId: 'synthetic-session', api }),
            ),
          ),
          h(
            'main',
            null,
            h('strong', null, 'Google Drive is optional for each session'),
            h(
              'p',
              null,
              'Enable Drive and Sheets tools without granting file access. Select files separately when requested.',
            ),
            h(
              'p',
              null,
              'Turning OFF revokes session grants and cancels requests and previews. It cannot undo dispatched writes.',
            ),
          ),
        ),
      )
    ...
    await page.screenshot({
      path: join(root, `docs/screenshots/session-toggle-${variant.name}.png`),
    })
    await toggle.click()
    ...
    assert.equal(
      await page.evaluate(
        () => window.calls.filter((call) => call.method === 'session-set').length,
      ),
      1,
    )
    if (variant.name === 'dark')
      await page.screenshot({
        path: join(root, 'docs/screenshots/session-toggle-enabled-dark.png'),
      })
...
console.log(
  'Captured four synthetic session toggle screenshots; actual component, native theme, no external requests.',
)
