...
try {
  for (const variant of [
    ...
    await page
      .getByRole('button', { name: 'Allow read access', exact: true })
      .waitFor({ state: 'visible' })
    ...
    const footer = await page
      ...
      .boundingBox()
    assert.ok(
      footer.y >= 0 && footer.y + footer.height <= variant.height,
      'confirmation stays within viewport',
    )
    ...
    assert.ok(
      ...
      'primary button text contrast is at least 4.5:1',
    )
    ...
    if (variant.name === 'picker-landscape') {
      await page.getByRole('button', { name: 'Review selection (2)', exact: true }).click()
      await page.getByRole('list', { name: 'Selected access' }).waitFor({ state: 'visible' })
      const expandedFooter = await page
        .getByRole('button', { name: 'Allow read access', exact: true })
        .boundingBox()
      assert.equal(
        expandedFooter.y,
        footer.y,
        'landscape review cannot push confirmation offscreen',
      )
      await scrollRegion.evaluate((node) => {
        node.scrollTop = node.scrollHeight
      })
    ...
    await page
      .getByRole('button', { name: 'Review selection (3)', exact: true })
      .waitFor({ state: 'visible' })
    ...
    await page.screenshot({
      path: join(output, `${variant.name}-shared.png`),
      animations: 'disabled',
    })
