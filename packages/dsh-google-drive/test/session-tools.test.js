...
function fixture(t, failAt = Infinity) {
  ...
  const set = (enabled) =>
    manager.set({ sessionId: 'root', ...manager.status({ sessionId: 'root' }), enabled })
  return {
    manager,
    agent,
    registered,
...
for (const failAt of [1, 2, 3, 5])
  test(`registration failure ${failAt} fails closed and cleans partial groups`, (t) => {
    const f = fixture(t, failAt)
    if (failAt <= 2) assert.throws(() => f.set(true), /Registration failed/)
    else {
      f.set(true)
      assert.throws(() => f.grant(), /Registration failed/)
    }
...
test('executors combine caller cancellation with the enable lifetime', async (t) => {
  ...
  await tool.execute(
    { reason: 'Read notes' },
    { agent: f.agent, callId: 'one', signal: caller.signal },
  )
...
test('aborted toggle does not register or advance the revision', (t) => {
  const f = fixture(t),
    initial = f.manager.status({ sessionId: 'root' }),
