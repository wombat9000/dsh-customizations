...
async function fixture(t) {
  ...
  return { ctx, standingKey, standing, plugin, a, b, child, names, skillNames, change, prepare, registry, skills, observers }
}
...
test('real registry views and skill snapshots isolate granted roots sharing one preset', async t => {
  ...
  for (const owner of [f.a.value, f.b.value, f.child.value]) {
    ...
    assert.deepEqual(f.names(owner), ['request_drive_access'])
  ...
  const expected = ['google_drive_list_files', 'google_drive_read_file', 'request_drive_access']
  ...
  assert.deepEqual(await f.skillNames(f.a.value), ['google-drive-read'])
  for (const key of [f.b.value, f.child.value, f.standingKey]) {
    assert.deepEqual(f.names(key), ['request_drive_access'])
  ...
  assert.deepEqual(await f.skillNames(f.b.value), ['google-drive-read'])
  f.change(f.a.value, false)
  assert.deepEqual(f.names(f.a.value), ['request_drive_access'])
  assert.deepEqual(await f.skillNames(f.a.value), [])
  assert.deepEqual(f.names(f.b.value), expected)
  assert.deepEqual(await f.skillNames(f.b.value), ['google-drive-read'])
  assert.deepEqual(f.names(f.child.value), ['request_drive_access'])
...
test('real agent fiber disposal and preset plugin disposal remove progressive registrations', async t => {
  ...
  f.change(f.a.value, true)
  assert.equal(f.names(f.a.value).length, 3)
  await f.a.scope.dispose()
  assert.deepEqual(f.names(f.a.value), ['request_drive_access'])
  assert.deepEqual(await f.skillNames(f.a.value), [])
  assert.equal(f.observers.get(f.a.value).size, 0)
  assert.equal(f.names(f.b.value).length, 3)
