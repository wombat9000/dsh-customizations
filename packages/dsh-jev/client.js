window.__ModuleLoader__.load({
  ...
  factory: require => {
    ...
    async function call(rpc, method, payload = {}) {
      let result
      try { result = await rpc.call('/jev-integration', method, payload) }
      catch { throw new Error('Jev settings request failed.') }
    ...
    function SettingsCard({ rpc }) {
      ...
      const load = async (resetModel = false) => {
        try {
          ...
          if (alive.current) { setStatus(value); if (resetModel) setModel(value.model) }
        } catch { if (alive.current) setError('Could not read Jev settings.') }
      }
      React.useEffect(() => {
        alive.current = true
        const style = document.createElement('style'); style.textContent = css; style.dataset.pluginCss = 'jev/settings'; document.head.appendChild(style)
        void load(true)
      ...
      const save = async () => {
        ...
        try {
          ...
          if (alive.current) { setStatus(value); setModel(value.model); setNotice('Jev model saved. No evaluation was sent.') }
        } catch (failure) { if (alive.current) setError(failure.message) }
        finally { if (alive.current) setBusy(false) }
      }
      return h('details', { className: 'dsh-jev', 'aria-label': 'Jev settings' },
        h('summary', null, 'Jev'),
        h('div', { className: 'dsh-jev__body' },
          h('p', { role: 'status' }, status ? `Shared OpenRouter key: ${status.credential?.configured ? 'Configured' : 'Not configured'}` : 'Checking shared OpenRouter key…'),
          h('p', { className: 'dsh-jev__hint' }, 'Manage the shared key in the OpenRouter plugin card. Jev evaluates typed questions; it is not a chat model. Consumers choose when to send data through OpenRouter to TypeSafe.'),
          h('label', null, 'Jev model ID', h('input', { value: model, disabled: busy || !status, onChange: event => setModel(event.target.value), spellCheck: false })),
          h('p', { className: 'dsh-jev__hint' }, 'Default: typesafe/jev-1.13. Use ~typesafe/jev-latest only if you want the release to change automatically.'),
          h('button', { type: 'button', disabled: busy || !status || !model.trim(), onClick: () => { void save() } }, 'Save Jev model'),
          h('button', { type: 'button', disabled: busy, onClick: () => { void load() } }, 'Refresh credential status'),
          error || status?.credential?.error ? h('p', { role: 'alert' }, error || status.credential.error) : null,
          notice ? h('p', { role: 'status' }, notice) : null))
    }
    function apply(ctx) {
      ctx.slots.inject('settings.plugin.item', () => ctx.slots.register({
        name: 'settings.plugin.item', key: 'jev', inject: () => ({ rpc: ctx.get('connection').rpc }),
      }, SettingsCard))
    }
