...
const SKILL_CANDIDATE = {
  name: 'trivy-audit',
  description:
    'Audit a local repository with Trivy for dependency vulnerabilities and infrastructure-as-code misconfigurations. Use when the user requests a vulnerability audit, dependency CVE review, security scan, Trivy scan, or remediation and rescan workflow.',
  whenToUse:
    'Use for repository-focused vulnerability and IaC audits when the pre-installed Trivy CLI is available.',
  invocation: {
    modelInvocable: true,
    userInvocable: true,
  },
...
export function registerTrivyTool(ctx, runtime) {
  ctx.tools.register(
    defineTool({
      name: 'trivy_scan',
      description:
        'Scan a file or directory inside the active workspace with the pre-installed Trivy CLI for dependency vulnerabilities and IaC misconfigurations. Trivy must be available on DSH’s effective PATH; the plugin never installs it.',
      parameters: {
        target: {
          type: 'string',
          description:
            'Workspace-relative file or directory to scan. Defaults to the workspace root. Targets outside the active workspace are rejected.',
        },
        scanners: {
          type: 'array',
          items: { type: 'string', enum: ['vulnerability', 'misconfiguration'] },
        ...
        ignoreUnfixed: {
          type: 'boolean',
          description:
            'Exclude vulnerabilities without a published fixed version. Defaults to false.',
        },
      },
      output: {
