...
const user = {
  ...
  active: true,
  isMe: true,
  url: 'https://linear.app/acme/settings/members/ada',
}
const project = {
  id: 'project-id',
  slugId: 'platform',
...
test('runtime enforces the bound workspace before reads', async () => {
  const runtime = runtimeWith({
    organization: Promise.resolve({ id: 'other', name: 'Other', urlKey: 'other' }),
  })
...
test('search paginates and joins issue metadata without lazy relation reads', async () => {
  ...
  for (const key of ['state', 'team', 'assignee', 'project', 'cycle']) {
    Object.defineProperty(found, key, {
      get() {
        relationReads += 1
  ...
  const result = await runtimeWith(client).searchIssues({
    query: 'timeout',
    limit: 5,
    cursor: 'cursor-1',
  })
...
test('state type selectors resolve every match without invalid ID comparisons', async () => {
  ...
  assert.equal(
    filter.or.some((branch) => branch.id !== undefined),
    false,
  )
})
...
test('human-readable selectors never enter UUID-only Linear comparators', async () => {
  ...
  const client = {
    ...
    async teams(variables) {
      filters.team = variables.filter
      return page([team])
    },
    async users(variables) {
      filters.user = variables.filter
      return page([user])
    },
    ...
    async cycles(variables) {
      filters.cycle = variables.filter
      return page([cycle])
    },
    async issueLabels(variables) {
      filters.labels = variables.filter
      return page([label])
    },
  }
...
test('full project URLs and overview URLs resolve by canonical browser slug', async () => {
  ...
  for (const filter of filters) {
    assert.equal(
      filter.or.some((branch) => branch.slugId?.eq === candidate.slugId),
      true,
    )
  }
})
...
test('project URL validation rejects malformed paths and other workspaces before lookup', async () => {
  ...
  await assert.rejects(
    resolveProject(client, 'https://linear.app/relational-life-tech/issue/ENG-123', {
      organizationUrlKey: 'relational-life-tech',
    }),
    /must contain.*project.*project-slug/iu,
  )
  await assert.rejects(
    resolveProject(client, 'https://linear.app/other-workspace/project/platform/overview', {
      organizationUrlKey: 'relational-life-tech',
    }),
    /belongs to workspace.*other-workspace.*connected workspace.*relational-life-tech/iu,
  )
  await assert.rejects(
    resolveProject(client, 'https://example.com/relational-life-tech/project/platform', {
      organizationUrlKey: 'relational-life-tech',
    }),
...
test('issue comments paginate and batch-join authors', async () => {
  ...
  const result = await runtimeWith(client).getIssueComments({
    issue: 'ENG-123',
    limit: 10,
    cursor: 'comments-1',
  })
...
test('project, cycle, and user listings return compact paginated catalogs', async () => {
  ...
  assert.deepEqual(projectVariables, {
    first: 5,
    filter: {
      name: { containsIgnoreCase: 'plat' },
      status: { id: { in: ['status-id'] } },
    },
    includeArchived: false,
    orderBy: 'updatedAt',
  })
...
test('testing an environment credential binds only its workspace identity', async () => {
  ...
  const ctx = {
    get(name) {
      ...
      if (name === 'credentials')
        return {
          async describe() {
            return { configured: true, writable: false, source: 'env' }
          },
        }
      if (name === 'settings')
        return {
  ...
  const live = {
    workspace: { id: 'org-id', name: 'Acme', urlKey: 'acme' },
    viewer: { id: 'user-id', name: 'Ada' },
    teams: [team],
  }
  registerLinearSettingsRpc(ctx, {
    runtime: { workspace: async () => live },
    settings: () => config,
