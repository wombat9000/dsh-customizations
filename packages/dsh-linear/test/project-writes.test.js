...
const lead = {
  ...
  active: true,
  isMe: true,
  url: 'https://linear.app/acme/member/ada',
}
...
test('project update preparation builds a concrete diff and immutable patch', async () => {
  ...
  const prepared = await writes.prepareUpdate({
    project: 'Platform',
    expectedUpdatedAt: '2026-03-20T15:00:00Z',
    status: 'Completed',
...
test('project update executes only when updatedAt remains unchanged', async () => {
  ...
  const client = selectors({
    ...
    async project() {
      projectReads += 1
      return projectReads === 1 ? current : updated
    },
  })
  const writes = new LinearProjectWrites(runtime(client))
...
test('project status report is concurrency-checked and returns its author', async () => {
  ...
  const update = {
    ...
    health: 'onTrack',
    url: 'https://linear.app/acme/project/platform/updates/update-id',
...
test('write tools ask once with prepared preview and cannot execute after rejection', async () => {
  ...
  const writes = {
    async prepareCreate() {
      return Object.freeze({
        kind: 'create-project',
        reason: 'Resolved create preview',
        input: Object.freeze({}),
      })
    },
  ...
  const exec = {
    ...
    arguments: { name: 'Platform', teams: ['ENG'] },
    token,
    callId: 'call-1',
    rootCallId: 'call-1',
...
test('an approved write consumes its exact prepared action once', async () => {
  ...
  const writes = {
    async prepareCreate(args) {
      return Object.freeze({
        kind: 'create-project',
        reason: `Create ${args.name}`,
        input: Object.freeze({ name: args.name }),
      })
    },
    async executeCreate(prepared) {
