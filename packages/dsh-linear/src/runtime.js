...
export function publicLinearError(error, operation = 'request') {
  if (error?.type === 'Ratelimited' || error?.status === 429)
...
export class LinearRuntime {
  ...
  async request(operation, task) {
    try {
      ...
      if (error instanceof Error && error.message.startsWith('Linear ') && error.type === undefined)
        throw error
