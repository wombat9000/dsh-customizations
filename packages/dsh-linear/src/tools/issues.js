...
function renderIssue(issue) {
  ...
  if (issue.description !== undefined && issue.description.length > 0)
...
export function registerIssueTools(ctx, runtime, config) {
  ctx.tools.register(
    defineTool({
      ...
      description:
        'Full-text search issues in the connected Linear workspace. Use linear_list_issues for structured filters.',
      parameters: {
        ...
        includeArchived: {
          type: 'boolean',
          description: 'Whether archived issues may be returned.',
        },
      },
      output: {
        schema: {
        ...
        render: (_args, value) => [
          {
            ...
            text: renderPage(
              ...
              `Found ${value.totalCount} issue(s):`,
            ),
          },
        ],
      },
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        'List Linear issues using deterministic team, state, assignee, priority, project, cycle, label, and date filters.',
      parameters: {
        team: { type: 'string', description: 'Team ID, key, or exact name. Required with states.' },
        states: {
          type: 'array',
          items: { type: 'string' },
          description: 'State IDs, exact names, or types.',
        },
        assignee: {
          type: 'string',
          description: 'User ID, email, exact name, “me”, or “unassigned”.',
        },
        priorities: {
          type: 'array',
          items: { type: 'integer' },
          description: 'Priorities: 0 none, 1 urgent, 2 high, 3 medium, 4 low.',
        },
        project: {
          type: 'string',
          description:
            'Project UUID, API slug ID, exact name, full project URL, browser path slug, or “none”.',
        },
        cycle: {
          type: 'string',
          description: 'Cycle ID, number, exact name, current, next, previous, or “none”.',
        },
        labels: {
          type: 'array',
          items: { type: 'string' },
          description: 'Label IDs or exact names; any matching label is accepted.',
        },
        updatedAfter: {
          type: 'string',
          description: 'Only issues updated on or after this ISO date/timestamp.',
        },
        createdAfter: {
          type: 'string',
          description: 'Only issues created on or after this ISO date/timestamp.',
        },
        orderBy: {
          type: 'string',
          enum: ['updatedAt', 'createdAt'],
          description: 'Pagination order. Defaults to updatedAt.',
        },
        ...PAGE_PARAMETERS,
        includeArchived: {
          type: 'boolean',
    ...
    defineTool({
      ...
      parameters: {
        ...
        orderBy: {
          ...
          description: 'Pagination order. Defaults to createdAt.',
        },
        ...PAGE_PARAMETERS,
        includeArchived: {
          type: 'boolean',
          description: 'Whether archived comments may be returned.',
        },
      },
      output: {
        schema: {
