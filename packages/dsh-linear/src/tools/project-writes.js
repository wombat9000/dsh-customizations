...
export function registerProjectWriteTools(ctx, writes, config) {
    ...
    defineTool({
      ...
      parameters: {
        ...
        teams: {
          ...
          required: true,
          items: { type: 'string' },
          description: 'One or more team IDs, keys, or exact names.',
        },
        description: {
          type: 'string',
          description: 'Short project description, up to 2,000 characters.',
        },
      ...
      output: {
        ...
        render: (_args, project) => [
          { type: 'text', text: projectText(project, 'Created project') },
        ],
      },
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        'Update selected Linear project fields after an approval diff and optimistic-concurrency check.',
      parameters: {
        project: {
          type: 'string',
          required: true,
          description:
            'Project UUID, API slug ID, exact name, full project URL, or browser path slug.',
        },
        expectedUpdatedAt: {
          type: 'string',
          description: 'Optional updatedAt timestamp from a prior read.',
        },
        ...
        teams: {
          type: 'array',
          items: { type: 'string' },
          description: 'Replacement non-empty team set.',
        },
        clearFields: {
          type: 'array',
          items: {
            type: 'string',
            enum: ['description', 'content', 'lead', 'startDate', 'targetDate'],
          },
      ...
      output: {
        schema: PROJECT_SCHEMA,
        render: (_args, project) => [
          { type: 'text', text: projectText(project, 'Updated project') },
        ],
      },
      timeoutMs: config.timeoutMs,
      isConcurrencySafe: () => false,
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        'Post a Linear project status report after one-shot approval and a concurrency check.',
      parameters: {
        project: {
          type: 'string',
          required: true,
        ...
        health: {
          ...
          description: 'Reported project health.',
        },
      ...
      presentCall: (args) => ({
        card: 'generic',
        title: `Post update to ${args.project}`,
        kind: 'edit',
      }),
    }),
  )
