...
export function registerCatalogTools(ctx, runtime, config) {
  ctx.tools.register(
    defineTool({
      ...
      description:
        'List and discover Linear projects, optionally scoped to a team and filtered by name or project status.',
      parameters: {
        ...
        includeArchived: {
          type: 'boolean',
          description: 'Whether archived projects may be returned.',
        },
      },
      output: {
        schema: {
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        'Get detailed read-only information for one Linear project by UUID, API slug ID, exact name, full project URL, or browser-visible project slug.',
      parameters: {
        project: {
          type: 'string',
          required: true,
          description:
            'Project UUID, API slug ID, exact name, full linear.app project URL, or browser-visible project path slug.',
        },
      },
      output: {
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        'List Linear cycles across the workspace or for one team, including active, future, past, next, and previous views.',
      parameters: {
        team: { type: 'string', description: 'Optional team ID, key, or exact name.' },
        status: {
          type: 'string',
          enum: ['all', 'active', 'future', 'past', 'next', 'previous'],
          description: 'Cycle view. Defaults to all.',
        },
        orderBy: {
          type: 'string',
          enum: ['updatedAt', 'createdAt'],
        ...
        includeArchived: {
          type: 'boolean',
          description: 'Whether archived cycles may be returned.',
        },
      },
      output: {
        schema: {
      ...
      isConcurrencySafe: () => true,
      execute: (args, exec) =>
        runtime.listCycles({ ...args, status: args.status ?? 'all' }, exec.signal),
  ...
  ctx.tools.register(
    defineTool({
      ...
      description:
        'List or find users in the connected Linear workspace or one team for assignee discovery.',
      parameters: {
        query: {
          type: 'string',
          description: 'Case-insensitive name, display-name, or email fragment.',
        },
        team: { type: 'string', description: 'Optional team ID, key, or exact name.' },
        active: { type: 'boolean', description: 'Filter by active account status.' },
        includeDisabled: {
          type: 'boolean',
          description: 'Whether disabled users may be returned.',
        },
        orderBy: {
          type: 'string',
          enum: ['updatedAt', 'createdAt'],
