...
function git(cwd, args, { signal, accept = [0] } = {}) {
  ...
  return new Promise((resolveResult, reject) => {
    ...
    const timer = setTimeout(
      () => stop(new Error(`Git operation exceeded ${TIMEOUT_MS}ms`)),
      TIMEOUT_MS,
    )
    ...
    child.once('error', (error) => {
      failure ??= new Error(`Unable to start Git: ${error.message}`, { cause: error })
    })
...
async function context(cwd, signal) {
  ...
  const common = singleLine(
    (await git(directory, ['rev-parse', '--git-common-dir'], { signal })).stdout,
  )
  const commonDir = await canonical(resolve(directory, common))
  const bare =
    singleLine((await git(directory, ['rev-parse', '--is-bare-repository'], { signal })).stdout) ===
    'true'
  const current = bare
    ? directory
    : await canonical(
        singleLine((await git(directory, ['rev-parse', '--show-toplevel'], { signal })).stdout),
      )
      ...
      while (next < rows.length) {
        ...
        try {
          ...
          const actual = singleLine(
            (await git(path, ['rev-parse', '--git-common-dir'], { signal })).stdout,
          )
          if ((await canonical(resolve(path, actual))) !== info.commonDir)
            throw new Error('Repository changed')
          const top = singleLine(
            (await git(path, ['rev-parse', '--show-toplevel'], { signal })).stdout,
          )
          ...
          if (
            filters.stdout
              .split('\0')
              .some((entry) => entry && entry.slice(entry.indexOf('\n') + 1).trim())
          )
            throw new Error('External status filter configured')
          changes = parseStatus(
            (
              await git(
                path,
...
async function refuseCheckoutFilters(cwd, signal) {
  ...
  for (const entry of result.stdout.split('\0')) {
    ...
    if (separator < 0 || entry.slice(separator + 1).trim() !== '') {
      throw new Error(
        'Worktree creation refuses configured checkout filters (filter.*.smudge/process); these can execute external commands',
      )
    }
  }
}
...
export async function createWorktree(cwd, name, { signal } = {}) {
  ...
  if (!current || current.bare)
    throw new Error('Worktree creation requires a non-bare current checkout')
  ...
  if (info.worktrees.some((row) => row.path === target))
  ...
  try {
    ...
    if ((await realpath(target)) !== target)
      throw new Error('Worktree target escaped its managed root')
    ...
    await git(
      current.path,
      [...checkoutConfig, 'worktree', 'add', '--no-checkout', '-b', branch, '--', target, head],
      { signal },
    )
    await validateManagedRoot(info.repository, root)
    if ((await realpath(target)) !== target)
    ...
    if (!worktree || worktree.branch !== branch || worktree.head !== head)
      throw new Error('Created worktree registration could not be verified')
    ...
    try {
      ...
      /* Preserve anything Git or another actor has populated. */
    }
    throw error
  } finally {
    if (hooks) {
      try {
        ...
        /* Never recursively delete an unexpected directory. */
      }
    }
  }
}
...
export async function resolveWorktree(cwd, path, { signal } = {}) {
  ...
  if (!worktree)
    throw new Error('Selected directory is not a registered worktree of this repository')
  if (worktree.isOriginal || worktree.isCurrent)
    throw new Error(
      'Dispatch requires a linked worktree other than the original or current checkout',
    )
  ...
  const actual = singleLine(
    (await git(selected, ['rev-parse', '--git-common-dir'], { signal })).stdout,
  )
  if ((await canonical(resolve(selected, actual))) !== info.commonDir)
    throw new Error('Selected worktree belongs to a different Git common directory')
  const top = singleLine((await git(selected, ['rev-parse', '--show-toplevel'], { signal })).stdout)
  if ((await canonical(top)) !== selected)
