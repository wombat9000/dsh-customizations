...
test('themed layout is responsive and keeps details keyboard accessible', async () => {
  ...
  try {
    ...
    rich.selected.run.task =
      'Review the Worktrees tab redesign, including accessibility and responsive layout.'
    f.set(rich)
    ...
    for (const theme of ['dark', 'light']) {
      ...
      expect(
        getComputedStyle(f.container.querySelector('.wt-layout')).gridTemplateColumns.split(' ')
          .length,
      ).toBe(2)
  ...
  worktrees: [
    {
      ...
      workerStatus: 'busy',
      changes: { count: 1 },
      latestAssignment: 'Review feature',
    },
  ],
  selected: {
    path: '/repo/a',
    changes: { count: 1, files: [{ path: 'file.txt', status: ' M' }] },
    runs: [
      { id: 'new', mode: 'read-only', status: 'running' },
      { id: 'old', mode: 'write', status: 'completed' },
    ],
...
async function mount() {
  ...
  let jobs = {},
    ...
    deferred
  const rpc = {
    call: vi.fn(async (_channel, method, args) => {
      if (method === 'capability')
        return { ok: true, value: { sessionId: args.sessionId, state: 'ready' } }
      if (deferred) await deferred.promise
      const result = structuredClone(value)
      if (args.runId === 'old' && result.selected)
        result.selected.run = { id: 'old', task: 'Implement feature', report: 'Tests passed' }
      return result.state === 'error'
        ? {
            ok: false,
            error: { code: 'worktrees/read-failed', message: result.message, details: {} },
          }
  ...
  await act(async () =>
    root.render(React.createElement(registration.Component, registration.options.inject('a'))),
  )
  return {
    container,
    rpc,
...
test('registered slot renders separate statuses, details, selection and in-place refresh', async () => {
  ...
  try {
    ...
    expect(f.container.textContent).toContain('Tests passed')
    expect(
      [...f.container.querySelectorAll('button')].map((b) => b.textContent).join(' '),
    ).not.toMatch(/Dispatch|Cancel|Merge|Delete|Create/)
  } finally {
    await f.close()
  }
})
...
test('refresh keeps layout stable; errors, unavailable and empty states remain explicit', async () => {
  ...
  try {
    ...
    expect(getComputedStyle(f.container.querySelector('.wt-refresh-status')).visibility).toBe(
      'visible',
    )
    ...
    for (const [value, text] of [
      [
        { sessionId: 'a', state: 'error', message: 'private host details' },
        'Worktrees could not refresh. Try again.',
      ],
      ...
      [
        {
          ...snapshot(),
          selected: { path: '/repo/a', changes: { count: 0, files: [] }, runs: [], run: null },
        },
        'No recorded runs',
      ],
    ]) {
