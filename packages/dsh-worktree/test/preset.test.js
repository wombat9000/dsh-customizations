...
function patchedRoster() {
  ...
  const rows = composeEntries(
    [loadOverlayPatches('preset-test', webPatch), loadOverlayPatches('preset-test', ownPatch)],
...
test('bundle registers its packaged preset root and keeps the stock default', async (t) => {
  ...
  const packaged = config.roots.find(
    (root) => root.trust === 'system' && root.path.endsWith('/presets'),
  )
  assert.ok(packaged)
...
test('coordinator retains every Standard row and only adds its tool contribution', async () => {
  ...
  for (const original of baseline) {
    ...
    if (original.id === 'tool-jobs')
      assert.deepEqual(copied.config, {
        ...original.config,
        completionDelivery: 'wakeup',
        maxConsecutiveWakes: 10,
      })
    else if (!original.group && !['persona', 'skill-filesystem'].includes(original.id))
      assert.deepEqual(copied.config, original.config, `${original.id} config changed`)
  }
  const tools = actual.filter((row) => row.name === '@local/dsh-worktree/tools')
  assert.equal(tools.length, 1)
  assert.equal(tools[0].disabled, undefined)
  assert.ok(
    !actual.some((row) => row.name === '@local/dsh-worktree'),
    'shared service belongs in host, not preset',
  )
  const normalized = structuredClone(coordinator).filter(
    (row) => row.name !== '@local/dsh-worktree/tools',
  )
  normalized.find((row) => row.id === 'persona').config = structuredClone(
    standard.find((row) => row.id === 'persona').config,
  )
  const skillConfig = standard.find((row) => row.id === 'skill-filesystem').config
  if (skillConfig === undefined)
    delete normalized.find((row) => row.id === 'skill-filesystem').config
  ...
  assert.match(
    persona,
    /When only background workers remain, END your turn with a brief progress update/,
  )
  assert.match(persona, /Yielding the turn is not reporting task completion/)
  assert.match(
    persona,
    /Do not use job_output\(wait: true\) merely to supervise workers; collect their results after completion notifications/,
  )
  ...
  assert.match(
    persona,
    /Before writing or changing a Cordis composition, load the editing-cordis-compositions skill/,
  )
  assert.match(persona, /not grants of Creator runtime tools or broader permissions/)
  assert.match(
    persona,
    /execution rules .* and cordis_inspect_\*, cordis_define, and cordis_run workflow apply only to dynamic plugins/,
  )
  assert.match(
    persona,
    /static packaged plugin development and review, use repository API contracts, imports, TypeScript\/JSX where supported, and normal build\/test workflows/,
  )
...
test('Creator skills resolve from a relocated preset and load official bodies on demand', async (t) => {
  ...
  assert.throws(
    () => createRequire(relocated).resolve('@deepseek-ai/dsh-agent-presets/package.json'),
    /Cannot find module/,
  )
  ...
  const config = interpolate(
    ctxBase.extend({ baseUrl: relocated }),
  ...
  const officialRoot = join(
    dirname(local.resolve('@deepseek-ai/dsh-agent-presets/package.json')),
    'presets/cordis/skills',
  )
  ...
  const absent = new FileSystemSkillProvider(ctx, control, {
    customSkillDirs: [join(directory, 'missing-skills')],
    includeDefaultRoots: false,
    watch: false,
  })
...
test('saved user default overrides bundle default without rewriting settings', async (t) => {
  ...
  const saved =
    '# User preference must survive bundle installation.\nagent-presets:\n  default: minimal\n'
  ...
  await ctx
    .plugin(AgentPresets, {
      ...interpolate({ baseUrl }, patchedRoster().config),
...
test('patch replacement and precedence remain explicit', () => {
  const base = [
    {
      ...
      config: { default: 'standard', roots: [] },
    },
  ]
  const rootsOnly = composeEntries([
    [{ insert: base }],
    [{ id: 'agent-presets', config: { roots: [] } }],
  ])
  assert.equal(
    rootsOnly[0].config.default,
    undefined,
    'Cordis replaces config; it does not merge roots',
  )
  ...
  assert.ok(
    !replaced.roots.some((root) => root.path === '/deployment-presets'),
    'bundle cannot append to earlier custom roots',
  )
})

test(
  'full Standard and coordinator standing mounts coexist on an isolated host',
  { timeout: 15000 },
  async (t) => {
    const { directory, baseUrl } = await profile(t)
    const ctx = new Context()
    ...
    assert.throws(
      () =>
        createRequire(pathToFileURL(`${userRoot}/`)).resolve('@local/dsh-worktree/package.json'),
      /Cannot find module/,
    )
    await ctx
      .plugin(AgentPresets, {
    ...
    assert.deepEqual(
      [...ctx.get('tools').view(copiedKey).visible.keys()].sort(),
      [...ctx.get('tools').view(coordinator).visible.keys()].sort(),
    )
    for (const name of ['cordis-plugin-development', 'editing-cordis-compositions']) {
    ...
    for (const key of [standard, coordinator, copiedKey]) {
      ...
      for (const name of worktreeNames) {
        ...
        assert.equal(
          ctx.get('tools').get(name, key),
          ctx.get('tools').get(name),
          'legacy rows do not shadow host definitions',
        )
      }
    }
    ...
    for (const name of ['cordis-plugin-development', 'editing-cordis-compositions']) {
      assert.ok(
        ...
        `missing scoped skill ${name}`,
      )
      assert.ok((await skills.get(name, { scope: coordinator, cwd: directory })).content)
    }
    assert.deepEqual(
      await skills.list({ cwd: directory }),
      [],
      'skills must not leak into the host scope',
    )
...
test('legacy tool row mounts without registering tools or publishing a service', async (t) => {
  ...
  t.after(() => ctx.fiber.dispose())
  ctx.baseUrl = baseUrl
  for (const Plugin of [Loader, SessionProjections, SystemPrompt, Tools])
  ...
  assert.equal(
    await roster.standingKeyFor('tool-contribution'),
    key,
    'standing composition is reused',
  )
  assert.deepEqual(
    [...ctx.get('tools').view(key).visible.keys()],
    [],
    'legacy row is inert; shared service owns tools',
  )
