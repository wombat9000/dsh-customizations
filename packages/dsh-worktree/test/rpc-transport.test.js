...
function transport(handler) {
  ...
  let connection
  load(browserSource, { __DSH_TRANSPORT__: { fetch: send } }).apply({
    provide: (name, value) => {
...
function fixture({ raw = false } = {}) {
  ...
  const ctx = {
    agents: {
      get: (id) => {
        if (lookupFail) throw new Error('PRIVATE lookup details')
        return live && id === 'a' ? agent : undefined
      },
    },
  ...
  const manager = {
    ...
    git: {
      inspectWorktrees: async () => {
        ...
        return {
          repository: '/repo',
          worktrees: [{ path: '/repo/a', branch: 'feature', changes: { count: 0, files: [] } }],
        }
      },
    },
  }
  return {
    ...transport((raw ? createSnapshotHandler : createSnapshotRpcHandler)(ctx, manager)),
    disable: () => {
      enabled = false
    },
    unload: () => {
...
async function read(f) {
  let state
  const reader = plugin.createReader(f.rpc, 'a', (next) => {
    state = next
  })
...
test('mounted service and installed host/browser RPC register the tab and load the snapshot', async (t) => {
  ...
  ctx.get('worktreeWorkers').manager.git = {
    inspectWorktrees: async () => ({
      repository: '/repo',
      worktrees: [{ path: '/repo/a', branch: 'feature', changes: { count: 0, files: [] } }],
    }),
...
test('negative control: original raw host result fails the installed browser parser', async () => {
  ...
  await assert.rejects(
    f.rpc.call(CHANNEL, 'capability', { sessionId: 'a' }),
    /invalid server-response result/,
  )
...
test('malformed envelopes, payloads, server failures and transport failures fail honestly', async () => {
  const mutations = [
    async (response) => {
      ...
      return Response.json({ ...body, result: { state: 'ready' } })
    },
    async (response) => {
      const body = await response.json()
      return Response.json({ ...body, rpcId: 'wrong' })
    },
    async (response) => {
      const body = await response.json()
      return Response.json({ ...body, result: { ok: true, value: null } })
    },
    async (response) => {
      const body = await response.json()
      return Response.json({
        ...body,
        result: { ok: false, error: { code: 'internal', message: 'PRIVATE', details: {} } },
      })
    },
