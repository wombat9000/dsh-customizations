...
async function fixture(t) {
  ...
  ctx.on('agent/inbox/claimed', (event) => {
    if (event.agent === agent) claimed.push(event.message)
  })
...
test(
  'idle completion opens one real followup and retains its report',
  { timeout: 10000 },
  async (t) => {
...
test(
  'busy completion injects without scheduling a duplicate followup',
  { timeout: 10000 },
  async (t) => {
    const f = await fixture(t)
    const gate = f.hold()
    ...
    assert.equal(
      f.agent.session.snapshotEvents().filter((event) => event.type === 'turn/start').length,
      1,
    )
  },
)
...
test(
  'ten idle wakes exhaust the budget and later completions stay queued',
  { timeout: 10000 },
  async (t) => {
    const f = await fixture(t)
    for (let i = 0; i < 10; i++) {
...
test(
  'only a claimed human message resets the exhausted wake budget',
  { timeout: 10000 },
  async (t) => {
    const f = await fixture(t)
    for (let i = 0; i < 10; i++) {
    ...
    await f.agent.whenIdle()
    assert.ok(
      f.claimed.some((msg) => noticeText(msg).includes(queued)),
      'queued completion survives until the next turn',
    )
    assert.ok(
      f.claimed.some(
        (msg) => msg.source.kind === 'user' && noticeText(msg) === 'Continue coordination',
      ),
    )
