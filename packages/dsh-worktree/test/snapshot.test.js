...
function fixture() {
  ...
  const tools = new Map([
    [a, markIntegrationTool({})],
    [b, {}],
  ])
  const ctx = {
    agents: { get: (id) => agents.get(id) },
    tools: { get: (_name, owner) => tools.get(owner) },
  }
  const history = new WeakMap([
    ...
    [
      b,
      new Map([['secret', { path: '/repo/a', jobId: 'secret', task: 'SECRET', report: 'SECRET' }]]),
    ],
  ])
  const manager = {
    history,
    cwd: (owner) => {
      assert.equal(agents.get(owner.session.id), owner)
      return '/repo'
    },
    ...
    git: {
      inspectWorktrees: async () => ({
        ...
        truncated: false,
        worktrees: [
          { path: '/repo/a', branch: 'a', changes: { count: 0, files: [], truncated: false } },
        ],
      }),
    },
  }
...
test('RPC adapter preserves private projection and wraps domain errors without host details', async () => {
  ...
  for (const args of [null, {}, { sessionId: 'a', path: [] }]) {
    assert.deepEqual(await call('snapshot', args), {
      ok: false,
      error: {
  ...
  f.ctx.agents.get = () => {
    throw new Error('SECRET host lookup')
  }
...
test('snapshot isolates reports, assignments and ids; busy may be global', async () => {
  ...
  assert.deepEqual(
    result.selected.runs.map((r) => r.id),
    ['new', 'old'],
  )
...
test('path selection never supplies a host cwd; invalid methods and input fail closed', async () => {
  ...
  f.manager.git.inspectWorktrees = async (value) => {
    cwd = value
    return read()
  }
  ...
  for (const [method, args] of [
    ['dispatch', { sessionId: 'a' }],
    ['snapshot', { sessionId: 'a', cwd: '/etc' }],
    ['snapshot', { sessionId: 'a', path: [] }],
  ]) {
...
test('coalesces bounded reads and discards results after capability removal or agent replacement', async () => {
  ...
  let resolve,
    signal,
    reads = 0
  f.manager.git.inspectWorktrees = (_cwd, options) => {
    reads++
...
test('empty history, bounded history and Git errors have explicit snapshots', async () => {
  ...
  f.manager.git.inspectWorktrees = async () => {
