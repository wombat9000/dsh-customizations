...
const templates = join(
  repo,
  'packages/dsh-project-steward/presets/project-steward/skills/project-steward/templates/v1',
)
...
async function fresh(t, source = repo, env = process.env) {
  ...
  const listed = spawnSync(
    ...
    { cwd: sourcePath, env, encoding: 'utf8' },
  )
  assert.equal(listed.status, 0, listed.stderr)
  for (const relative of new Set(
    listed.stdout.split('\0').filter((path) => path && !path.split('/').includes('node_modules')),
  )) {
    ...
    try {
      await lstat(join(sourcePath, relative))
    } catch (error) {
...
test('fresh source fixture runs dependency-free checks with no pnpm executable or cache', async (t) => {
  ...
  const env = {
    PATH: emptyPath,
    HOME: emptyHome,
    DSH_HOME: join(emptyHome, '.dsh'),
    XDG_CACHE_HOME: join(emptyHome, '.cache'),
  }
  ...
  // The package roster grows independently of this dependency-free setup check.
  assert.match(result.stdout, /Checked 1 profile recipe\(s\) and [1-9]\d* package reference\(s\)/)
...
test('v1 templates explicitly distinguish placeholders and verification evidence', async () => {
  ...
  for (const phrase of [
    'Unverified proposal',
    'Verified definition only',
    'Executed successfully',
    'Not run',
    '--offline --frozen-lockfile --ignore-scripts',
  ])
    assert.ok(setup.includes(phrase))
})
