...
export class GoogleOAuthClient {
  ...
  configured() {
    return (
      text(this.config.clientId, 1024) &&
      (this.config.clientSecret === undefined ||
        this.config.clientSecret === '' ||
        text(this.config.clientSecret, 4096))
    )
  }

  assertActive(epoch = this.epoch) {
    if (this.disposed || this.disconnecting || epoch !== this.epoch)
  ...
  async readCredentials() {
    try {
      ...
      throw fail('Google credentials could not be read.')
    }
  }
  ...
  async saveCredentials(record, epoch, flow) {
    return this.serialize(async () => {
      ...
      try {
        await this.credentials.set(record, isValid)
      } catch {
        throw fail('Google credentials could not be saved.')
      }
    })
  }
  ...
  tokenRecord(result, previous, requested) {
    ...
    return {
      ...
      scopes: granted,
      ...(previous?.account ? { account: accountCopy(previous.account) } : {}),
    }
  }
  ...
  async begin({ scopes: required, publishCallback } = {}) {
    ...
    flow.server = createServer(
      ...
      (req, res) => {
        ...
        if (url.searchParams.getAll('code').length !== 1 || !text(code, 4096))
          return reply(400, 'Invalid callback.')
        flow.accepted = true
        trackResponse()
        ...
        void (async () => {
          ...
          try {
            ...
            if (!includes(record.scopes, required)) {
              safeError =
                'Google did not grant the required scopes. Connect again and approve the requested access.'
            ...
            if (previous && previous.account.id !== identity.sub) {
              safeError =
                'Google account does not match. Disconnect first to connect a different account.'
              throw fail(safeError)
            }
            record.account = {
              id: identity.sub,
              ...(identity.email_verified === true && text(identity.email, 320)
    ...
    try {
      ...
      if (publishCallback !== undefined) {
        ...
        const match =
          typeof lease?.origin === 'string' &&
          /^http:\/\/127\.0\.0\.1:([1-9][0-9]{0,4})\/?$/.exec(lease.origin)
        if (
          !match ||
          match[0] !== lease.origin ||
  ...
  async accessTokenOperation(required, epoch) {
    ...
    if (!includes(record.scopes, required))
      throw fail(
        'Required Google scopes are missing. Connect Google and approve the requested access.',
      )
    if (
    ...
    if (!this.refresh) {
      const promise = (async () => {
        const result = await this.request(TOKEN, {
          method: 'POST',
          body: this.tokenParams({
    ...
    this.assertActive(epoch)
    if (!includes(updated.scopes, required))
      throw fail(
        'Required Google scopes are missing. Connect Google and approve the requested access.',
      )
  ...
  async disconnect({ revoke = false } = {}) {
    ...
    try {
      record = await this.serialize(async () => {
        ...
        try {
          ...
          throw fail('Google credentials could not be removed.')
        }
        return value
      })
    } finally {
    ...
    if (revoke && text(token, 16_384)) {
      try {
        ...
        throw fail(
          'Local Google credentials were removed, but revocation failed. Revoke access in your Google account.',
        )
      }
    }
  }
