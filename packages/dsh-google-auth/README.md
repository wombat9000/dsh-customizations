...
- A local browser must run on the same host as DSH. The callback binds `127.0.0.1`; a remote browser or Docker loopback does not reach it automatically.
...
4. Review an integration's required scopes, then choose its **Connect** action.
5. Follow **Continue with Google** and approve the requested access. The card shows the connected account and each integration's permission status.
...
The auth provider belongs in the host composition and injects `credentials`, `webServer`, and `settings`. It serves the empty `google-auth` settings namespace so DSH displays the Google accounts card. Client configuration and tokens stay outside that settings document.
...
| `status()` | Returns connection/account facts and registered integration scopes, authorization status, and missing scopes. Never returns credentials. |
...
The automated suites use synthetic tokens, mock Google responses, real ephemeral loopback callbacks, real DSH service/route lifecycle fixtures, and browser component tests. They do not authenticate a real account or deploy a live profile. A real consent and Drive listing test remains a post-deployment step.
