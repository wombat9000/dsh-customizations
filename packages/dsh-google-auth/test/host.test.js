...
const clientJson = JSON.stringify({
  installed: {
    ...
    auth_uri: 'https://attacker.invalid',
  },
})
const tokens = {
  ...
  scopes: [scope],
  account: { id: 'account-1', email: 'fixture@example.invalid' },
}
const deferred = () => {
  let resolve
...
function fixture(overrides = {}) {
  const records = new Map([
    [
      CLIENT_KEY,
      { kind: 'grant', payload: { version: 1, clientId, clientSecret: 'FIXTURE-SECRET' } },
    ],
  ])
  const reads = [],
    calls = [],
  ...
  const credentials = {
    ...
    async modifyRecord(key, fn) {
      const value = await fn(records.get(key))
      ...
      const client = {
        ...
        async status() {
          return {
            ...
            pending: false,
            grantedScopes: [scope],
            account: { ...tokens.account, secret: 'HIDDEN' },
            accessToken: 'HIDDEN',
            ...overrides.status,
          }
        },
        async getAccessToken(args) {
          ...
          return overrides.token ? overrides.token(args) : 'FIXTURE-ACCESS'
        },
        async begin(args) {
          calls.push(['begin', args])
          return overrides.begin
            ? overrides.begin(args)
            : { authorizationUrl: 'https://accounts.google.com/fixture', expiresAt: 42 }
        },
        cancel() {
          calls.push(['cancel'])
          return overrides.cancel ?? true
        },
        dispose() {
          calls.push(['dispose'])
        },
...
const register = (service, id = 'drive', scopes = [scope]) =>
  service.registerIntegration({ id, label: id, scopes })

test('non-secret access generation invalidates observers on reconnect, reset and disposal', async () => {
  const f = fixture()
  register(f.service, 'drive')
  const seen = []
  const removeThrowing = f.service.onAccessChange(() => {
...
test('registry canonicalizes email aliases for missing permission comparisons and rejects sparse scopes', async () => {
  ...
  for (const scopes of [new Array(1), [scope, , scope]])
    assert.throws(() => register(f.service, 'sparse', scopes), /explicit Google permission scopes/)
})
...
test('actual Drive fetch aborts on shared auth disconnect and cannot return old account metadata', async (t) => {
  ...
  const drive = new GoogleDriveClient({
    withAccessToken: (operation) => f.service.withAccessToken('google-drive', operation),
    fetch: async (_url, init) => {
      ...
      return Response.json({
        files: [{ id: 'old-account', name: 'PRIVATE OLD METADATA', mimeType: 'text/plain' }],
      })
    },
  })
  t.after(() => {
...
test('unregister aborts only matching operations while other integrations remain active', async () => {
  const f = fixture(),
    driveStarted = deferred(),
    calendarStarted = deferred(),
    release = deferred()
  const remove = register(f.service)
  register(f.service, 'calendar', [otherScope])
  let driveSignal, calendarSignal
  const drive = f.service.withAccessToken('drive', async (_token, signal) => {
    driveSignal = signal
  ...
  const calendar = f.service.withAccessToken('calendar', async (_token, signal) => {
...
test('host operation completion and callback errors revoke signals and clean tracking', async () => {
  ...
  await assert.rejects(
    f.service.withAccessToken('drive', (_token, signal) => {
      failedSignal = signal
      throw failure
    }),
    (error) => error === failure,
  ...
  assert.equal(f.service.accessOperations.size, 0)
  assert.deepEqual(
    await f.service.withAccessToken('drive', (token, signal) => {
  ...
  await assert.rejects(
    f.service.withAccessToken('unknown', () => assert.fail('unknown operation ran')),
    /not registered/,
  )
  assert.equal(f.service.accessOperations.size, 0)
})

for (const action of ['configure', 'clearConfig', 'begin', 'dispose'])
  test(`${action} cancels account-bound operations and suppresses late results`, async () => {
    const f = fixture(),
      started = deferred(),
      release = deferred()
    register(f.service)
    let signal
    const result = f.service.withAccessToken('drive', async (_token, authSignal) => {
      signal = authSignal
      started.resolve()
      await release.promise
      return 'STALE'
...
test('integration registration is pure, validated, bounded, immutable and uniquely owned', () => {
  ...
  for (const value of [
    null,
    {},
    { id: '../drive', label: 'x', scopes: [scope] },
    { id: 'x', label: '', scopes: [scope] },
    { id: 'x', label: 'x', scopes: [] },
    { id: 'x', label: 'x', scopes: ['https://attacker.invalid'] },
  ])
    assert.throws(() => f.service.registerIntegration(value))
  remove()
  remove()
  register(f.service)
...
test('status derives per-integration missing permissions and projects only account leaves', async () => {
  ...
  assert.deepEqual(await f.service.status(), {
    ...
    requiredScopes: [otherScope, scope].sort(),
    missingScopes: [otherScope],
    integrations: [
      { id: 'drive', label: 'drive', scopes: [scope], authorized: true, missingScopes: [] },
      {
        id: 'calendar',
        label: 'calendar',
        scopes: [otherScope, scope].sort(),
        authorized: false,
        missingScopes: [otherScope],
      },
    ],
...
test('tokens require exact IDs and account consent rejects caller arguments', async () => {
  ...
  for (const id of [
    ...
    null,
    'unknown',
    { integrationId: 'drive', scopes: [otherScope] },
    [scope],
  ])
    await assert.rejects(f.service.getAccessToken(id), /not registered/)
  ...
  for (const args of [['drive'], [undefined], [{ scopes: [scope] }], [{}]])
    await assert.rejects(f.service.begin(...args), /accepts no/)
...
test('account consent unions and deduplicates all current integrations while token checks stay exact', async () => {
  ...
  assert.deepEqual(
    status.integrations.map((item) => item.authorized),
    [true, false],
  )
  ...
  register(f.service, 'calendar', [otherScope])
  assert.equal(
    f.calls.filter((call) => call[0] === 'begin').length,
    1,
    'later registration never starts consent',
  )
  await f.service.begin()
  assert.deepEqual(f.calls.at(-1), [
    'begin',
    { scopes: [otherScope, driveScope, sheetsScope].sort() },
  ])
})

for (const phase of ['load', 'begin'])
  for (const change of ['remove', 'replace', 'add']) {
    test(`account scope snapshot rejects ${change} during ${phase}`, async () => {
      ...
      if (phase === 'load') {
        ...
        f.credentials.readRecord = async (key) => {
          entered.resolve()
          await gate.promise
          return read(key)
        }
      ...
      if (phase === 'begin' && change !== 'add')
        assert.equal(f.calls.at(-1)[0], 'cancel', 'removal interrupts begin immediately')
      gate.resolve({ authorizationUrl: 'https://accounts.google.com/fixture' })
      await rejected
      if (phase === 'load')
        assert.equal(
          f.calls.some((call) => call[0] === 'begin'),
          false,
        )
...
for (const change of ['remove', 'cancel'])
  test(`access observer ${change} cannot start stale account consent`, async () => {
    const f = fixture()
    const remove = register(f.service)
    f.service.onAccessChange(() => {
      if (change === 'remove') remove()
      else void f.service.cancel()
    })
...
test('configuration is write-only, separates client and grant keys, and invalidates generations', async () => {
  ...
  assert.deepEqual(f.records.get(CLIENT_KEY), {
    kind: 'grant',
    payload: { version: 1, clientId, clientSecret: 'FIXTURE-SECRET' },
  })
  ...
  for (const value of [
    'bad',
    '{}',
    JSON.stringify({ web: { client_id: clientId } }),
    'x'.repeat(32769),
  ])
    await assert.rejects(f.service.configure(value), /Desktop OAuth/)
...
test('a client configuration read already in flight cannot publish an obsolete client', async () => {
  ...
  f.credentials.readRecord = async () => {
    entered.resolve()
    await release.promise
    return oldRecord
  }
...
test('credential adapter rejects wrong clients and malformed grant envelopes', async () => {
  const f = fixture(),
    adapter = credentialAdapter(f.credentials, clientId)
  for (const record of [
    undefined,
    { kind: 'secret', payload: { version: 1, clientId, tokens } },
    { kind: 'grant', payload: { version: 2, clientId, tokens } },
    {
      kind: 'grant',
      payload: { version: 1, clientId: 'other.apps.googleusercontent.com', tokens },
    },
  ]) {
  ...
  const entered = deferred(),
    release = deferred()
  f.credentials.modifyRecord = async (key, fn) => {
    entered.resolve()
    await release.promise
...
test('credential storage errors and invalid client grants never leak provider secrets', async () => {
  ...
  for (const operation of [() => adapter.get(), () => adapter.set(tokens), () => adapter.delete()])
    await assert.rejects(operation(), (error) => !error.message.includes(secret))
  const f = fixture()
  register(f.service)
  for (const record of [
    undefined,
    { kind: 'secret' },
    { kind: 'grant', payload: { version: 1, clientId: 'invalid', clientSecret: secret } },
  ]) {
