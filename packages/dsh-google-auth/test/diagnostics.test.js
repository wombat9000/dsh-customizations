...
const failure = (status, code) =>
  `Google request failed (HTTP ${status})${code ? `: ${code}` : ''}. Try again or reconnect.`
...
test('auth HTTP diagnostics expose only status and exact allowlisted codes', async (t) => {
  for (const [body, code] of [
    ...
    [
      { error: { details: [{ reason: 'PERMISSION_DENIED', metadata: { url: secret } }] } },
      'PERMISSION_DENIED',
    ],
  ...
  const client = fixture(t, () =>
    Response.json({ error: 'invalid_token', error_description: secret }, { status: 400 }),
  )
  await assert.rejects(
    client.request('https://oauth2.googleapis.com/revoke', {}, undefined, true),
    { message: failure(400) },
  )
})
...
test('auth bounds error reads and sanitizes malformed, stream, and network failures', async (t) => {
  ...
  await assert.rejects(network.request(endpoint), {
...
test('auth preserves cancellation semantics while reading stalled error bodies', async (t) => {
  for (const mode of ['abort', 'dispose', 'timeout']) {
    ...
    const rejected = assert.rejects(pending, {
      message: 'Google request failed. Try again or reconnect.',
    })
    await reading
