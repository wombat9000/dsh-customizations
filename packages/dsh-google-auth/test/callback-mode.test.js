...
function fixture({ mode = false, publisher, save } = {}) {
  const records = new Map([
    [
      CLIENT_KEY,
      { kind: 'grant', payload: { version: 1, clientId: 'fixture.apps.googleusercontent.com' } },
    ],
    [
      CREDENTIAL_KEY,
      {
  ...
  const service = new GoogleAuthService({
    credentials: {
      async readRecord() {
        ...
        return {
          kind: 'grant',
          payload: { version: 1, clientId: 'fixture.apps.googleusercontent.com' },
        }
      },
    },
    createClient: () => ({
...
test('direct mode never consults publication and disabling prevents future forwarded links', async (t) => {
  ...
  assert.equal(
    new URL(new URL(first.authorizationUrl).searchParams.get('redirect_uri')).hostname,
    '127.0.0.1',
  )
...
for (const publisher of [
  undefined,
  {
    available: () => false,
    publish() {
      assert.fail('unavailable publisher invoked')
    },
  },
]) {
...
for (const phase of ['startup', 'waiting'])
  for (const external of [false, true]) {
    test(`${external ? 'external settings' : 'mode write'} cancels publication during ${phase} and retains account`, async (t) => {
      ...
      const outcome =
        phase === 'startup' ? assert.rejects(pending, /cancel|changed|could not start/i) : pending
      await entered.promise
...
test('failed mode persistence preserves enabled mode without a direct fallback', async (t) => {
  ...
  t.after(() => f.service.dispose())
  await assert.rejects(f.service.setCallbackMode(false), {
    message: 'Could not save Google callback settings.',
  })
