...
test('authorization links reject untrusted origins, credentials, paths, fragments and protocols', () => {
  const { plugin } = load()
  assert.equal(
    plugin.authorizationUrl('https://accounts.google.com/o/oauth2/v2/auth?state=fixture'),
  ...
  for (const value of [
    ...
    assert.throws(() => plugin.authorizationUrl(value), /invalid authorization link/)
})
test('transport errors hide raw exception contents', async () => {
  ...
  await assert.rejects(
    api('status'),
    (error) => /Cannot reach DSH/.test(error.message) && !error.message.includes('SECRET'),
  )
})
test('unreadable and rejected replies produce actionable errors', async () => {
  ...
  await assert.rejects(
    load(async () => ({
      ok: false,
      json: async () => ({ ok: false, error: { message: 'Use the local loopback GUI.' } }),
    })).api('connect'),
    /local loopback GUI/,
  )
})
