...
const clientJson = JSON.stringify({
  installed: { client_id: 'fixture.apps.googleusercontent.com', client_secret: 'FIXTURE-ONLY' },
})

function requester(base) {
  return (action, body = {}, headers = {}, method = 'POST') =>
    fetch(`${base}/api/plugins/google-auth/${action}`, {
      method,
...
test('local request guard requires exact loopback host, origin, JSON and CSRF headers', () => {
  ...
  for (const remoteAddress of ['::1', '::ffff:127.0.0.1'])
    assert.equal(allowedRequest({ ...valid, socket: { remoteAddress } }, 1234), true)
  for (const host of ['localhost:1234', '[::1]:1234'])
    assert.equal(
      allowedRequest(
        { ...valid, headers: { ...valid.headers, host, origin: `http://${host}` } },
        1234,
      ),
      true,
    )
  for (const headers of [
    ...
    { host: 'attacker.invalid:1234', origin: 'http://attacker.invalid:1234' },
    { host: '127.0.0.1:9999', origin: 'http://127.0.0.1:9999' },
    { 'x-dsh-google-auth': undefined },
    { 'x-dsh-google-auth': '0' },
    ...
    assert.equal(
      allowedRequest({ ...valid, headers: { ...valid.headers, ...headers } }, 1234),
      false,
...
test('HTTP bodies are strict, account consent accepts only an empty object and projections never expose tokens', async (t) => {
  ...
  const service = {
    async status() {
      return {
        ...
        missingScopes: [],
        expiresAt: 42,
        account: { id: 'account-1', email: 'fixture@example.invalid', accessToken: 'PRIVATE' },
        accessToken: 'PRIVATE',
    ...
    async begin(...args) {
      ...
      if (failConnect) throw Error('PRIVATE invalid_grant')
      return {
        authorizationUrl: 'https://accounts.google.com/fixture',
        expiresAt: 42,
        accessToken: 'PRIVATE',
      }
    },
    ...
    async cancel() {
      throw Error('PRIVATE commit in progress')
    },
    async disconnect() {
      calls.push(['disconnect'])
      return { refreshToken: 'PRIVATE' }
    },
    async clearConfig() {
      calls.push(['clear'])
      return {}
    },
    async setCallbackMode(value) {
      calls.push(['mode', value])
      if (!value) throw Error('PRIVATE https://callback.invalid/token')
      return { secret: 'PRIVATE' }
    },
  }
  const server = createServer((req, res) =>
    settingsHandler(service, req.url.split('/').at(-1), server.address().port)(req, res),
  )
  ...
  for (const body of [
    ...
    null,
    [],
    { integrationId: 'drive', scopes: [scope] },
    { scopes: [scope] },
    { integrationId: 'drive', scope },
    { integrationId: 'drive', clientId: 'x' },
    { integrationId: 'drive', __extra: true },
    { integrationId: '../drive' },
    { integrationId: 1 },
    '{',
    'x'.repeat(1025),
  ...
  assert.deepEqual(await connect.json(), {
    ok: true,
    value: { authorizationUrl: 'https://accounts.google.com/fixture', expiresAt: 42 },
  })
  assert.deepEqual(calls, [['begin']])
  for (const action of ['status', 'cancel', 'disconnect', 'clear-config'])
    assert.equal((await post(action, { scopes: [scope] })).status, 400)
  for (const body of [
    {},
    { clientJson: 1 },
    { clientJson, scopes: [scope] },
    { clientJson: 'x'.repeat(32769) },
  ])
    assert.equal((await post('configure', body)).status, 400)
  ...
  for (const body of [
    ...
    { useSandbox: 1 },
    { useSandbox: null },
    { useSandbox: true, scope },
    { useSandbox: true, scopes: [scope] },
    { useSandbox: true, callbackUrl: 'https://attacker.invalid' },
    { useSandbox: true, redirectUri: 'http://localhost:42' },
    { useSandbox: true, integrationId: 'drive' },
  ]) {
    assert.equal((await post('callback-mode', body)).status, 400, JSON.stringify(body))
  }
  assert.equal(calls.length, beforeMode)
  assert.deepEqual(await (await post('callback-mode', { useSandbox: true })).json(), {
    ok: true,
    value: {},
  })
  ...
  assert.deepEqual(await modeError.json(), {
    ok: false,
    error: {
      message:
  ...
  for (const headers of [
    ...
    assert.equal((await post('disconnect', {}, headers)).status, 403)
...
test('real Cordis SettingsFile/WebServer lifecycle serves Google namespace; Drive consumes shared auth only', async (t) => {
  ...
  assert.equal(
    ctx
      .get('settings')
      .describe()
      .some((item) => item.ns === 'google-drive'),
    false,
  )
  ...
  auth.getAccessToken = async (...args) => {
    ids.push(args)
    throw Error('fixture authentication unavailable')
  }
  ...
  for (const action of ['getAccessToken', 'token', 'refresh'])
    assert.equal((await post(action)).status, 404)
