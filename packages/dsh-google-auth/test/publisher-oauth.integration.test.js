...
for (const outcome of ['success', 'denied']) {
  test(`real adapter preserves forwarded ${outcome} response before lease release`, async (t) => {
    ...
    const publisher = new SandboxCallbackPublisher({
      bridgeDir: '/unused-fixture',
      resolveBridge: () => 'fixture',
      loadBridge: async () => ({ BridgeClient }),
    })
    let saved
    const exchanges = []
    const client = new GoogleOAuthClient({
      ...
      credentials: {
        get: async () => saved,
        set: async (value, valid) => {
          if (valid()) saved = value
        },
        delete: async () => {
          saved = undefined
        },
      ...
      fetch: async (url, options) => {
        ...
        const data = url.includes('userinfo')
          ? { sub: 'fixture-account' }
          : {
    ...
    callback.searchParams.set(
      outcome === 'success' ? 'code' : 'error',
      outcome === 'success' ? 'fixture-code' : 'access_denied',
    )
    const response = await fetch(callback)
    assert.equal(response.status, outcome === 'success' ? 200 : 400)
    assert.match(
      await response.text(),
      outcome === 'success' ? /You can close this tab/ : /not authorized/,
    )
