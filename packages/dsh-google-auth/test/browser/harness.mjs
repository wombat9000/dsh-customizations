...
export const integration = (extra = {}) => ({
  id: 'google-drive',
  label: 'Google Drive',
  scopes: ['https://www.googleapis.com/auth/drive.metadata.readonly'],
  authorized: false,
  missingScopes: ['https://www.googleapis.com/auth/drive.metadata.readonly'],
  ...extra,
})
...
export async function mountSettings({ status = {}, overrides = {} } = {}) {
    ...
    Object.entries({
      status: async () =>
        ok({
          ...
          missingScopes: [
            ...new Set(state.integrations.flatMap((item) => item.missingScopes)),
          ].sort(),
        }),
      configure: async () => clear(true),
      'clear-config': async () => clear(false),
      'callback-mode': async ({ useSandbox }) => {
      ...
      connect: async () => {
        state.pending = true
        return ok({
          authorizationUrl: 'https://accounts.google.com/o/oauth2/v2/auth?state=browser-fixture',
  ...
  const fetch = vi.spyOn(window, 'fetch').mockImplementation(async (url, options) => {
    expect(url).toMatch(
      /^\/api\/plugins\/google-auth\/(status|configure|clear-config|callback-mode|connect|cancel|disconnect)$/,
    )
    ...
    if (method === 'callback-mode') {
      expect(Object.keys(body)).toEqual(['useSandbox'])
      expect(typeof body.useSandbox).toBe('boolean')
  ...
  try {
    await act(async () =>
      root.render(React.createElement(registration.Component, registration.options.inject())),
    )
  } catch (error) {
