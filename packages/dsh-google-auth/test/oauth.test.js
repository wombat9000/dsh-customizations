...
const fresh = () => ({
  ...
  scopes: [...granted],
  account: { id: 'subject-one', email: 'test@example.invalid' },
})
      ...
      return options.fetch
        ...
        : json(
            url.includes('userinfo')
              ? { sub: 'subject-one', email: 'test@example.invalid', email_verified: true }
              : token(),
          )
    },
  })
...
async function login(client, options = required) {
  const flow = await callback(client, options)
...
function publisherFixture(t) {
  ...
  state.publish = async (port, signal) => {
    ...
    t.after(stop)
    return {
      origin: `http://127.0.0.1:${state.publicPort}/`,
      async dispose() {
        state.disposals++
        await stop()
      },
...
test('forwarded callback uses public Host and exact token redirect through real TCP relay', async (t) => {
  const relay = publisherFixture(t),
    { client, calls } = fixture(t)
  const { url, auth, started } = await callback(client, {
    ...required,
    publishCallback: relay.publish,
  })
  assert.notEqual(relay.internalPort, relay.publicPort)
  assert.equal(url.port, String(relay.publicPort))
  ...
  assert.equal(relay.disposals, 0)
  ...
  assert.equal(relay.disposals, 1)
  client.cancel()
...
test('missing lease disposer, malformed publisher and publication rejection fail closed', async (t) => {
  for (const publishCallback of [
    null,
    1,
    async () => undefined,
    async () => ({ origin: 'http://127.0.0.1:1234' }),
    async () => {
      throw new Error('private forwarding details')
    },
  ]) {
    const { client, calls } = fixture(t)
    await assert.rejects(
      client.begin({ ...required, publishCallback }),
      /Invalid Google callback publisher|could not start/,
    )
    await client.cleanup()
    assert.equal((await client.status()).pending, false)
    assert.equal(calls.length, 0)
...
test('cancel timeout and dispose reject pending publication and collect late abort-ignoring lease', async (t) => {
  for (const action of ['cancel', 'timeout', 'dispose']) {
    ...
    if (action === 'timeout') await settled(client)
      ...
      teardown = client[action]()
      if (action === 'cancel') assert.equal(teardown, true)
    }
    await rejected
...
test('every forwarded terminal outcome disposes the lease once and flushes callback bodies', async (t) => {
  for (const action of [
    ...
    const relay = publisherFixture(t)
    const { client } = fixture(t, {
      timeoutMs: action === 'timeout' ? 40 : 300_000,
      ...(action === 'failure'
        ? {
            fetch: () => {
    ...
    if (action === 'denial') {
      ...
      assert.equal(response.body, 'Google sign-in was not authorized.')
...
test('forwarding cleanup rejection is observed and reported without private details', async (t) => {
  ...
  await client
    .begin({
      ...
      publishCallback: async () => ({
        ...
        dispose() {
          disposals++
          return Promise.reject(new Error('private forwarding secret'))
        },
      }),
    })
    .catch(() => {})
  ...
  assert.equal(disposals, 1)
  assert.equal(
    (await client.status()).error,
    'Google callback forwarding cleanup failed. Restart before connecting again.',
  )
})
  ...
  const rejected = assert.rejects(
    client.begin({
      ...
      publishCallback: async () => {
        ...
        throw new Error('private late rejection')
      },
    }),
    /could not start/,
  )
...
test('timeout includes credential reads before listener and publication', async (t) => {
  ...
  const { client } = fixture(t, {
    timeoutMs: 20,
    credentials: { get: () => read.promise, async set() {}, async delete() {} },
  })
...
test('scope options are mandatory and Google-only; invalid requests have no browser or HTTP effects', async (t) => {
  ...
  for (const options of [
    ...
    {},
    { scopes: 'openid' },
    { scopes: ['https://evil.invalid/auth/a'] },
    { scopes: ['https://www.googleapis.com/auth/a?x'] },
    { scopes: ['openid email'] },
    { scopes: [null] },
    { scopes: Array(1) },
  ]) {
...
test('incremental consent explicitly unions old grants and preserves offline token for same account', async (t) => {
  const { client, record } = fixture(t, {
    record: fresh(),
    fetch: (url) =>
      json(url.includes('userinfo') ? { sub: 'subject-one' } : token({ refresh_token: undefined })),
  })
...
test('explicit consent grants do not invent previous scopes omitted by Google', async (t) => {
  const { client, record } = fixture(t, {
    ...
    fetch: (url) =>
      json(
        url.includes('userinfo') ? { sub: 'subject-one' } : token({ scope: `${B} openid email` }),
      ),
  })
  await login(client, { scopes: [B] })
...
test('email/profile aliases normalize across requests, consent, stored grants and refresh', async (t) => {
  ...
  assert.deepEqual(normalizeScopes(['email', email, 'profile', profile, 'openid']), [
    email,
    profile,
    'openid',
  ])
  ...
  for (const scope of ['openid email profile', `openid ${email} ${profile}`]) {
    const { client, record } = fixture(t, {
      fetch: (url) => json(url.includes('userinfo') ? { sub: 'subject-one' } : token({ scope })),
    })
  ...
  for (const scope of [undefined, `openid ${email} ${profile}`, 'openid email profile']) {
    const { client, record } = fixture(t, {
      record: { ...fresh(), expiresAt: 0, scopes: ['openid', 'email', 'profile'] },
...
test('delayed credential reads cannot reuse refresh tokens rotated by another caller', async (t) => {
  ...
  const first = client.getAccessToken(required)
  await entered.promise
  ...
  assert.equal(record.refreshToken, 'rotated-refresh')
})
...
test('queued token consumers retain admission epoch and missing scopes do not poison later callers', async (t) => {
  ...
  await missing
  assert.equal(await valid, 'synthetic-access')
...
test('userinfo binds accounts, ignores JWT claims and includes only verified email', async (t) => {
  for (const identity of [{ sub: 'different' }, {}, { sub: '' }]) {
    ...
    const { client, record } = fixture(t, {
      record: old,
      fetch: (url) =>
        json(url.includes('userinfo') ? identity : token({ id_token: 'untrusted.jwt.synthetic' })),
    })
    await login(client)
    assert.equal(record(), old)
    assert.match(
      (await client.status()).error,
      identity.sub ? /Disconnect first/ : /sign-in failed/,
    )
  }
  for (const email_verified of [false, 'true', undefined]) {
    const { client, record } = fixture(t, {
      fetch: (url) =>
        json(
          url.includes('userinfo')
            ? { sub: 'subject-one', email: 'unverified@example.invalid', email_verified }
            : token(),
        ),
    })
    await login(client)
...
test('shared refresh preserves omitted scopes/account and validates narrowed grants per caller', async (t) => {
  for (const narrowed of [false, true]) {
    const gate = deferred()
    const { client, calls, record } = fixture(t, {
      record: { ...fresh(), scopes: [...granted, B], expiresAt: 0 },
      fetch: async () => {
        await gate.promise
        return json(
          token({ refresh_token: undefined, ...(narrowed ? { scope: `${B} openid email` } : {}) }),
        )
      },
    })
    ...
    gate.resolve()
    await check
    assert.equal(await b, 'synthetic-access')
...
test('callback rejects method/path/host/state/code ambiguity without consuming flow', async (t) => {
  ...
  for (const mutate of [
    ...
    (u) => u.searchParams.append('state', 'duplicate'),
    (u) => u.searchParams.append('code', 'duplicate'),
    (u) => u.searchParams.set('code', 'x'.repeat(4097)),
  ]) {
    const bad = new URL(url)
    mutate(bad)
    assert.ok((await fetch(bad)).status >= 400)
  }
  const raw = (path, host) =>
    new Promise((resolve, reject) => {
...
test('denial is sanitized and listener closes', async (t) => {
  const { client, calls } = fixture(t)
  const { url } = await callback(client)
  url.searchParams.set('error', 'private-provider-detail')
...
test('cancel, timeout and disposal close listeners and prevent late exchange writes', async (t) => {
  for (const action of ['cancel', 'timeout', 'dispose']) {
    const gate = deferred(),
      entered = deferred()
    const { client, writes } = fixture(t, {
      timeoutMs: action === 'timeout' ? 30 : 300_000,
      fetch: async () => {
        entered.resolve()
        await gate.promise
        return json(token())
  ...
  const { client } = fixture(t)
  const pending = client.begin(required)
  client.dispose()
...
test('disconnect invalidates refresh, serializes deletion and blocks operations during a write', async (t) => {
  ...
  const pending = client.getAccessToken(required)
  const rejected = assert.rejects(pending, /cancelled/)
  await writing.promise
  const disconnect = client.disconnect()
  await assert.rejects(client.begin(required), /cancelled/)
  await assert.rejects(client.getAccessToken(required), /cancelled/)
  finish.resolve()
...
test('reconnect aborts old refresh; token consumers cannot bypass pending consent', async (t) => {
  const gate = deferred()
  const { client, writes } = fixture(t, {
    record: { ...fresh(), expiresAt: 0 },
    fetch: async () => {
      await gate.promise
      return json(token())
    },
  })
  const refresh = client.getAccessToken(required)
  const rejected = assert.rejects(refresh, /Google/)
  await new Promise((resolve) => setImmediate(resolve))
  await client.begin(required)
  await assert.rejects(client.getAccessToken(required), /Finish or cancel/)
  gate.resolve()
  await rejected
  assert.equal(writes.length, 0)
})
  ...
  for (const stage of ['token', 'identity']) {
    const { client, writes } = fixture(t, {
      fetch: (url) => {
        ...
        return json(
          { error: 'invalid_grant', error_description: 'private-token https://private.invalid/' },
          400,
        )
      },
    })
    ...
    assert.equal(
      (await client.status()).error,
      'Google request failed (HTTP 400): invalid_grant. Try again or reconnect.',
    )
    assert.equal(writes.length, 0)
  }
  const { client } = fixture(t, {
    credentials: {
      ...
      async set() {
        throw new Error('Google request failed (HTTP 400): invalid_grant. Try again or reconnect.')
      },
      async delete() {},
    },
  })
...
test('transport fixes endpoints, bounds size and time including abort-ignoring fetch and bodies', async (t) => {
  ...
  for (const url of [
    ...
    'https://openidconnect.googleapis.com/v1/userinfo#fragment',
...
test('disconnect is local by default; explicit revoke cannot leak errors or restore credentials', async (t) => {
  ...
  const remote = fixture(t, { record: fresh(), fetch: () => new Response(null) })
  ...
  await assert.rejects(
    broken.client.disconnect({ revoke: true }),
    /Local Google credentials were removed/,
  )
...
test('userinfo failure and cancellation preserve previous account without committing', async (t) => {
  for (const cancel of [false, true]) {
    const old = fresh(),
      entered = deferred(),
      gate = deferred()
    const { client, record, writes } = fixture(t, {
      record: old,
      fetch: async (url) => {
...
test('fresh token and disconnected reads never initiate OAuth or HTTP', async (t) => {
  ...
  assert.equal(await ready.client.getAccessToken(required), 'synthetic-access')
  ...
  assert.deepEqual(await empty.client.status(), {
    configured: true,
    connected: false,
    pending: false,
    grantedScopes: [],
...
test('invalid tokens, missing offline access and adapter errors stay sanitized', async (t) => {
  ...
  await login(missing.client)
  assert.equal(missing.record(), null)
  assert.match((await missing.client.status()).error, /sign-in failed/)
  const optional = fixture(t, { clientSecret: undefined })
  await login(optional.client)
  ...
  await assert.rejects(broken.client.status(), { message: 'Google credentials could not be read.' })
  await broken.client.disconnect()
})
