...
export function fieldPresentation(prepared) {
  ...
  const content = pick(prepared.targets.item.content, [
    '__typename',
    'id',
    'number',
    'title',
  ...
  return {
    ...
    targets: {
      ...
      item: {
        id: prepared.targets.item.id,
        content,
        project: pick(prepared.targets.item.project, ['id']),
      },
    },
    change: {
...
export function createGitHubPresentation({ agents, grants, caller }) {
  ...
  function request(exec, scope, exactPreview) {
