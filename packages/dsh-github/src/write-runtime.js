...
const existingIssue = {
  repositoryOwner: str('Explicit issue repository owner, independent of project owner.'),
...
export const WRITE_OPERATIONS = Object.freeze({
  createProject: {
    ...
    properties: {
      ...owner,
      title: str('Exact new project title, 1–256 characters.'),
      templateOwner: str('Explicit source template owner; requires templateNumber.'),
      templateNumber: num('Explicit source template project number; requires templateOwner.'),
      includeDraftIssues: {
        type: 'boolean',
        description: 'Copy source draft issues. Template copies only; default false.',
      },
    },
    description:
      'Create one project with a title, optionally copying an explicit template. Approves the source, destination and copy behavior; other metadata edits require another call.',
  },
  updateProject: {
    ...
    properties: {
      ...project,
      title: str('Replacement title, 1–256 characters.'),
      description: str('Replacement short description, up to 1,024 characters. Empty clears it.'),
      readme: str('Replacement README, up to 20,000 characters. Empty clears it.'),
    },
    description:
      'Update selected project title, short description or README after an exact before/after preview.',
  },
  linkProjectRepository: {
    name: 'github_link_project_repository',
    required: ['owner', 'projectNumber', 'repositoryOwner', 'repo'],
    properties: {
      ...project,
      repositoryOwner: existingIssue.repositoryOwner,
      repo: repository.repo,
    },
    description:
      'Link an explicit repository to an explicit project. Rejects an existing link; requires project update and repository write permission.',
  },
  createIssue: {
    ...
    properties: {
      ...repository,
      title: str('Exact issue title, 1–256 characters.'),
      body: str('Exact issue body, up to 20,000 characters; may be empty.'),
    },
    description:
      'Create one issue with the complete approved title and body in an explicit repository.',
  },
  addProjectItem: {
    name: 'github_add_project_item',
    required: ['owner', 'projectNumber', 'repositoryOwner', 'repo', 'issueNumber'],
    properties: { ...project, ...existingIssue },
    description:
      'Add an existing issue to a project. Rejects an existing membership, including archived items.',
  },
  setProjectItemField: {
    name: 'github_set_project_item_field',
    required: ['owner', 'projectNumber', 'itemId', 'fieldId', 'value'],
    properties: {
      ...project,
      itemId: str('Exact project item node ID, verified against the project.'),
      fieldId: str('Exact field ID from this project’s field definitions.'),
      value: {
        type: 'object',
        additionalProperties: false,
        properties: {
          text: str('TEXT value, up to 20,000 characters.'),
          number: { type: 'number', description: 'Finite NUMBER value.' },
          date: str('DATE value as a real YYYY-MM-DD calendar date.'),
          singleSelectOptionId: str(
            'Exact option ID in the project SINGLE_SELECT field, including Status.',
          ),
          iterationId: str('Exact active or completed ITERATION ID in this field.'),
        },
        description:
          'Exactly one typed value. TEXT, NUMBER, DATE, SINGLE_SELECT and ITERATION only; no clearing, multiselect, issue fields or other built-in field updates.',
      },
    },
    description:
      'Set one item field using the project’s actual type and option/iteration IDs, with its complete before/after value.',
  },
  addIssueDependency: {
    name: 'github_add_issue_dependency',
    required: [
      'owner',
      'repo',
      'issueNumber',
    ...
    properties: {
      ...repository,
      issueNumber: num('Issue that will be blocked.'),
      blockingOwner: str('Explicit blocking issue repository owner.'),
      blockingRepo: repository.repo,
      blockingIssueNumber: num('Issue that blocks the first issue.'),
    },
    description:
      'Add GitHub’s native blocked-by relationship between two explicit issues. Rejects self-dependencies and duplicate relationships.',
  },
})
const messages = {
  INVALID_ARGUMENT:
    'Invalid GitHub write arguments. Use explicit targets and the documented bounded values.',
  UNSAFE_CONTENT:
    'The exact content contains credential-looking or unsafe text. It cannot be shown or written by this tool; no mutation was dispatched.',
  APPROVAL_REQUIRED: 'This GitHub write has no unused preparation for this exact approved call.',
  CONTEXT_CHANGED:
    'The calling agent or working directory changed after preparation. Request new approval.',
  CONFLICT:
    'The authenticated identity, resolved target, permission or relevant remote state changed after approval. No mutation was dispatched. Request a new preview.',
  PERMISSION_DENIED:
    'Known GitHub permissions do not allow this operation. No mutation was dispatched.',
  NOT_FOUND:
    'A GitHub target is missing or inaccessible; these cases may be indistinguishable. No mutation was dispatched.',
  ALREADY_EXISTS: 'The requested relationship or value already exists. No mutation was dispatched.',
  BOUND_EXCEEDED:
    'The exact preview or relevant collection exceeds the safe bound. No mutation was dispatched; use a smaller resource or inspect it outside this write tool.',
  INVALID_RESPONSE:
    'GitHub returned an incomplete or malformed response. No mutation was dispatched.',
  AUTH_REQUIRED:
    'GitHub authentication is missing or invalid in the managed backend. No mutation was dispatched.',
  READ_FAILED:
    'GitHub preflight failed in the managed backend. No mutation was dispatched; no raw diagnostic is exposed.',
...
const fail = (code) => {
  throw new GitHubError(code, messages[code] ?? messages.READ_FAILED)
}
...
function clean(value) {
  ...
  if (typeof serialized !== 'string' || Buffer.byteLength(serialized) > 262144)
    fail('BOUND_EXCEEDED')
  const inspect = (entry) => {
...
function checkedEntity(value, numbered = false) {
  ...
  if (
    ...
    !/^https:\/\/github\.com\//.test(value.url) ||
    (numbered && (!Number.isSafeInteger(value.number) || value.number < 1))
  )
...
export function validateWriteArguments(operation, input) {
  ...
  for (const [key, value] of Object.entries(input)) {
    ...
    if (
      type === 'string' &&
      (value.length > 20000 || /[\u0000-\u0008\u000b\u000c\u000e-\u001f\u007f]/.test(value))
    )
      fail('INVALID_ARGUMENT')
    if (/Owner$/.test(key) || key === 'owner')
      if (!/^[A-Za-z0-9][A-Za-z0-9-]{0,99}$/.test(value)) fail('INVALID_ARGUMENT')
    if (key === 'repo' || key === 'blockingRepo')
      if (!/^[A-Za-z0-9_.-]{1,100}$/.test(value) || ['.', '..'].includes(value))
        fail('INVALID_ARGUMENT')
  ...
  if (operation === 'createProject') {
    if (
      (input.templateOwner === undefined) !== (input.templateNumber === undefined) ||
      (input.includeDraftIssues !== undefined && input.templateNumber === undefined)
    )
      fail('INVALID_ARGUMENT')
  }
  if (
    operation === 'updateProject' &&
    !['title', 'description', 'readme'].some((key) => Object.hasOwn(input, key))
  )
    fail('INVALID_ARGUMENT')
  if (operation === 'setProjectItemField') {
    ...
    if (
      keys.length !== 1 ||
      !['text', 'number', 'date', 'singleSelectOptionId', 'iterationId'].includes(keys[0])
    )
      fail('INVALID_ARGUMENT')
    const [key] = keys
    ...
    if (
      ...
      fail('INVALID_ARGUMENT')
    if (['singleSelectOptionId', 'iterationId'].includes(key) && !id(value))
      fail('INVALID_ARGUMENT')
    if (
      key === 'date' &&
...
export function renderWritePreview(value) {
  ...
  const json = JSON.stringify(value, null, 2).replace(/"(?:\\.|[^"\\])*"/g, (token) =>
    token
...
function readVariables(operation, args) {
  const allowed = {
    createProject: ['owner', 'templateOwner', 'templateNumber'],
    updateProject: ['owner', 'projectNumber'],
    linkProjectRepository: ['owner', 'projectNumber', 'repositoryOwner', 'repo'],
    createIssue: ['owner', 'repo'],
    addProjectItem: ['owner', 'projectNumber', 'repositoryOwner', 'repo', 'issueNumber'],
    setProjectItemField: ['owner', 'projectNumber', 'itemId'],
    addIssueDependency: [
      'owner',
      'repo',
      'issueNumber',
  ...
  return Object.fromEntries(
    allowed[operation].filter((key) => args[key] !== undefined).map((key) => [key, args[key]]),
  )
}
...
function repositoryTarget(data) {
  const target = checkedEntity(data.repository)
  if (typeof target.nameWithOwner !== 'string') fail('INVALID_RESPONSE')
  return target
}
function actor(data) {
  record(data.viewer)
  if (!id(data.viewer.id) || typeof data.viewer.login !== 'string' || !data.viewer.login)
    fail('AUTH_REQUIRED')
...
function fieldChange(project, item, args) {
  ...
  if (!key || field.isIssueField === true || !Object.hasOwn(args.value, key))
    fail('INVALID_ARGUMENT')
  let option = null
  if (key === 'singleSelectOptionId') {
    if (!Array.isArray(field.options) || field.options.length > 100) fail('BOUND_EXCEEDED')
    option = field.options.find((value) => value.id === args.value[key])
    if (!option) fail('INVALID_ARGUMENT')
  }
  if (key === 'iterationId') {
    if (
      !Array.isArray(field.configuration?.iterations) ||
      !Array.isArray(field.configuration?.completedIterations)
    )
      fail('INVALID_RESPONSE')
    const iterations = [
    ...
    option = iterations.find((value) => value.id === args.value[key])
    if (!option) fail('INVALID_ARGUMENT')
  }
...
function resolve(operation, args, data) {
  ...
  if (operation === 'createProject') {
    ...
    if (
      destination.__typename === 'User'
        ? destination.isViewer !== true
        : destination.__typename !== 'Organization' || destination.viewerIsAMember !== true
    )
    ...
    change = {
      title: args.title,
      creationPermission:
        'GitHub exposes no definitive owner project-creation capability; membership and identity are rechecked, and the server decides.',
    }
    if (args.templateNumber !== undefined) {
      ...
      change.copyBehavior = {
        includeDraftIssues: payload.includeDraftIssues,
        sourceTemplate: template.id,
        ordinaryNewProject: true,
        copied:
          'Source fields, views, insights and workflows except auto-add workflows; draft issues only when explicitly enabled.',
        notCopied:
          'Ordinary issues/pull requests, collaborators and linked repositories. Visibility follows GitHub copy defaults; this call does not set it.',
      }
    }
  } else if (operation === 'createIssue') {
    const target = repositoryTarget(data)
    if (
      target.viewerCanCreateIssues !== true ||
      target.hasIssuesEnabled !== true ||
      target.isArchived === true ||
      target.isDisabled === true
    )
      fail('PERMISSION_DENIED')
    targets = { repository: target }
    payload = { repositoryId: target.id, title: args.title, body: args.body }
    change = { title: args.title, body: args.body }
    ...
    if (target.viewerCanUpdate !== true || blocking.viewerCanUpdate !== true)
      fail('PERMISSION_DENIED')
    const before = complete(target.blockedBy)
    if (before.some((value) => value.id === blocking.id)) fail('ALREADY_EXISTS')
    targets = { blockedIssue: target, blockingIssue: blocking }
    payload = { issueId: target.id, blockingIssueId: blocking.id }
    change = { before, addBlockedBy: blocking }
  } else {
    const target = projectTarget(data)
    targets = { project: target }
    if (operation === 'updateProject') {
      payload = { projectId: target.id }
      change = {}
      for (const [arg, field] of [
        ...
        change[field] = { before: target[field], after: args[arg] }
      }
      if (Object.values(change).every((value) => value.before === value.after))
        fail('ALREADY_EXISTS')
      ...
      payload = { projectId: target.id, repositoryId: repository.id }
      change = { before, link: repository }
      ...
      targets.issue = issue
      payload = { projectId: target.id, contentId: issue.id }
      change = { addIssue: issue, existingProjectItems: before }
...
function readError(response) {
  ...
  if (/HTTP 403|forbidden|insufficient|scope|resource not accessible/i.test(text))
    fail('PERMISSION_DENIED')
  fail('READ_FAILED')
}
...
function confirmed(operation, prepared, data) {
  ...
  const roots = {
    ...
    copyProject: 'copyProjectV2',
    updateProject: 'updateProjectV2',
    linkProjectRepository: 'linkProjectV2ToRepository',
    createIssue: 'createIssue',
    addProjectItem: 'addProjectV2ItemById',
    setProjectItemField: 'updateProjectV2ItemFieldValue',
    addIssueDependency: 'addBlockedBy',
  }
  ...
  if (['createProject', 'updateProject'].includes(operation)) {
    ...
    if (operation === 'createProject') {
      if (
        resource.owner?.id !== prepared.payload.ownerId ||
        resource.title !== prepared.payload.title ||
        resource.template !== false
      )
        fail('INVALID_RESPONSE')
    } else if (resource.id !== prepared.payload.projectId) fail('INVALID_RESPONSE')
    if (operation === 'updateProject')
      for (const key of ['title', 'shortDescription', 'readme'])
        if (Object.hasOwn(prepared.payload, key) && resource[key] !== prepared.payload[key])
          fail('INVALID_RESPONSE')
  } else if (operation === 'linkProjectRepository') {
    ...
    if (
      resource.repository?.id !== prepared.payload.repositoryId ||
      resource.title !== prepared.payload.title ||
    ...
    if (
      !id(resource.id) ||
      resource.project?.id !== prepared.payload.projectId ||
      typeof resource.project?.url !== 'string'
    )
      fail('INVALID_RESPONSE')
    if (
      operation === 'setProjectItemField'
        ? resource.id !== prepared.payload.itemId
        : resource.content?.id !== prepared.payload.contentId
    )
      fail('INVALID_RESPONSE')
    resource = { ...resource, url: resource.project.url }
  } else {
    checkedEntity(result.issue, true)
    checkedEntity(result.blockingIssue, true)
    if (
      result.issue.id !== prepared.payload.issueId ||
      result.blockingIssue.id !== prepared.payload.blockingIssueId
    )
      fail('INVALID_RESPONSE')
    resource = result
  }
...
export function uncertainWriteResult(prepared, data) {
  ...
  if (data && typeof data === 'object' && !Array.isArray(data)) {
    for (const root of Object.values(data)) {
      ...
      for (const key of [
        ...
        if (!id(value?.id) || typeof url !== 'string' || !/^https:\/\/github\.com\//.test(url))
          continue
        ...
        try {
          ...
          /* Never expose unsafe partial response text. */
        }
      }
    }
  }
  return {
    ...
    message:
      'The mutation was dispatched and may have succeeded. Do not retry automatically. Inspect the explicit targets and any unconfirmed observed identities with read tools before requesting fresh approval. No rollback was attempted.',
  }
}
export function createGitHubWriteRuntime(subprocess, config = {}) {
  ...
  async function timed(exec, task) {
    ...
    try {
      return await task({ ...exec, signal: controller.signal })
    } catch (error) {
      if (timedOut && error?.code === 'CANCELLED') fail('TIMEOUT')
      throw error
    } finally {
      clearTimeout(timer)
  ...
  async function resolveGrantScope(input, exec = {}, { onAccount } = {}) {
    ...
    return timed(exec, async (current) => {
      const issues = []
      const projects = []
      for (const [kind, targets, results] of [
        ['issue', args.issues, issues],
  ...
  async function execute(
    prepared,
    exec = {},
    { onDispatch, beforeDispatch, onPreflight, onAccount } = {},
  ) {
    ...
    return timed(exec, async (current) => {
      ...
      const work = queue.then(async () => {
        ...
        let fresh
        try {
          fresh = await preflight(prepared.operation, prepared.args, current, onAccount)
        } catch (error) {
          if (
            ['ALREADY_EXISTS', 'NOT_FOUND', 'PERMISSION_DENIED', 'INVALID_ARGUMENT'].includes(
              error?.code,
            )
          )
            fail('CONFLICT')
        ...
        try {
          ...
          if (dispatched)
            return {
              ...uncertainWriteResult(prepared, observedData),
              ...(isGitHubBackendFenced(subprocess)
                ? {
                    backendFenced: true,
                    cleanupWarning:
                      'The managed process range could not be confirmed stopped. No further GitHub operation may run through this backend until it is verified and replaced or restarted.',
                  }
                : {}),
            }
          throw error
