...
const repo =
  'id nameWithOwner url isArchived isDisabled hasIssuesEnabled viewerCanCreateIssues viewerPermission'
const project =
  'id number title url shortDescription readme updatedAt template public closed viewerCanUpdate owner { ... on User { id login } ... on Organization { id login } }'
const issue =
  'id number title url updatedAt viewerCanUpdate repository { id nameWithOwner url owner { id login } }'
...
const projectOwner = (selection = project) =>
  `repositoryOwner(login:$owner) { id login ... on User { projectV2(number:$projectNumber) { ${selection} } } ... on Organization { projectV2(number:$projectNumber) { ${selection} } } }`
const resultProject =
  'id number title url template owner { ... on User { id login } ... on Organization { id login } }'
...
export const MUTATIONS = Object.freeze({
  ...
  linkProjectRepository:
    'mutation($input:LinkProjectV2ToRepositoryInput!) { linkProjectV2ToRepository(input:$input) { repository { id nameWithOwner url } } }',
  ...
  addProjectItem:
    'mutation($input:AddProjectV2ItemByIdInput!) { addProjectV2ItemById(input:$input) { item { id project { id url } content { ... on Issue { id number url } } } } }',
  setProjectItemField:
    'mutation($input:UpdateProjectV2ItemFieldValueInput!) { updateProjectV2ItemFieldValue(input:$input) { projectV2Item { id project { id url } } } }',
  addIssueDependency:
    'mutation($input:AddBlockedByInput!) { addBlockedBy(input:$input) { issue { id number url } blockingIssue { id number url } } }',
})
