...
export function validateInput(value, action) {
  ...
  if (
    ...
    throw new Error('Invalid GitHub card request.')
  return value
}
  ...
  return async (req, res) => {
    if (!allowedRequest(req, port))
      return reply(res, 403, {
        ok: false,
        error: { message: 'Manage GitHub access from the local DSH page.' },
      })
    if (req.method !== 'POST')
    ...
    try {
      ...
      return reply(res, 409, {
        ok: false,
        error: { message: 'GitHub card data is unavailable or expired. No access was granted.' },
      })
    } finally {
      clearTimeout(timer)
    }
    ...
    for (const action of ['status', 'revoke'])
      scope.effect(() =>
        scope.webServer.register({
          kind: 'exact',
          path: `${PREFIX}${action}`,
          handler: createHandler(presentation, action, scope.webServer.port),
        }),
      )
  })
}
