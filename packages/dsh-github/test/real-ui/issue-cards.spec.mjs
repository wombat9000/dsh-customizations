...
for (const [theme, narrow] of [['light', false], ['dark', false], ['light', true], ['dark', true]]) {
  test(`compact issue snapshots in real shell ${theme}${narrow ? ' narrow' : ''}`, async ({ app }, testInfo) => {
    let requests = 0
    await app.route('**/api/plugins/github/*', route => { requests++; return route.abort() })
    ...
    await sessions.getByRole('treeitem', { name: new RegExp(`^${githubItemsWorkspace}\\s`) }).click()
    for (const title of ['Issues', 'Issue search', 'Issue details']) {
      const card = app.getByRole('region', { name: `GitHub ${title}`, exact: true })
      await expect(card).toBeVisible()
      if (narrow) await card.evaluate(node => { node.style.width = '320px'; node.style.maxWidth = '100%' })
      ...
      await app.setViewportSize({ width: app.viewportSize().width, height: Math.ceil(bounds.height) + 600 })
      await card.scrollIntoViewIfNeeded()
      const path = testInfo.outputPath(`issues-${title.replaceAll(' ', '-')}-${theme}${narrow ? '-narrow' : ''}.png`)
      await card.screenshot({ path })
      await testInfo.attach('Issue snapshot diagnostic (not baseline)', { path, contentType: 'image/png' })
      if (title === 'Issue details') {
        const description = card.locator('summary').filter({ hasText: /description/i }).first()
        await description.focus(); await description.press('Enter')
