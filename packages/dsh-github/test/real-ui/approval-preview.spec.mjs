...
for (const [theme, narrow] of [
  ...
  test(`native GitHub approval detail in real shell ${theme}${narrow ? ' narrow' : ''}`, async ({
    app,
  }, testInfo) => {
...
for (const [theme, narrow] of [
  ...
  test(`membership resource cards in real shell ${theme}${narrow ? ' narrow' : ''}`, async ({
    app,
  }, testInfo) => {
    await openSession(app, theme)
...
test('unrelated native command approval retains the shipped command detail and controls', async ({
  app,
}) => {
...
test('all seven native previews, membership metadata exclusion, allow-once and malformed fallback', async ({
  app,
}) => {
  await openSession(app, 'light')
