...
import { githubItemsWorkspace, githubItemsPrompt } from '../../../../tests/real-ui/github-items-fixture.mjs'
for (const [theme, narrow] of [['light', false], ['dark', false], ['light', true], ['dark', true]]) {
  test(`historical compact project items in real shell ${theme}${narrow ? ' narrow' : ''}`, async ({ app }, testInfo) => {
    let requests = 0
    ...
    await expect(card.getByRole('link', { name: '#20 — Implement compact rows', exact: true })).toHaveAttribute('href', 'https://github.com/fixture-org/demo/pull/20')
    ...
    expect(await card.locator('.gh-item').evaluateAll(rows => rows.every(row => row.querySelectorAll('details[open]').length === 0))).toBe(true)
    const visible = await card.innerText()
    for (let number = 1; number <= 6; number++) expect(visible.split(`Compact project item ${number}`)).toHaveLength(2)
    for (const value of ['PVTI_', 'Board field', 'not archived', 'Value unavailable']) expect(visible).not.toContain(value)
    if (!narrow) expect(await card.locator('.gh-item').evaluateAll(rows => rows.reduce((sum, row) => sum + row.getBoundingClientRect().height, 0))).toBeLessThan(850)
    ...
    await card.screenshot({ path }); await testInfo.attach('compact-items', { path, contentType: 'image/png' })
    const first = card.locator('.gh-item').first()
    const fields = first.getByLabel('Additional fields for item #1', { exact: true })
    await fields.focus(); await fields.press('Enter')
