...
import { githubSessionId, githubCallId, githubWorkspaceName, githubToolName, githubPrompt } from '../../../../tests/real-ui/github-grants-fixture.mjs'
...
const scope = {
  ...
  issues: [43, 42].map(number => ({ id: `I_${number}`, repositoryId: 'R_fixture', repositoryOwnerId: 'O_fixture', nameWithOwner: 'fixture-org/demo', issueNumber: number,
    title: number === 43 ? 'Review session-scoped issue management grants' : 'Readable project field changes', url: `https://github.com/fixture-org/demo/issues/${number}` })),
  projects: [{ id: 'P_fixture', ownerId: 'O_fixture', owner: 'fixture-org', projectNumber: 7, title: 'Delivery board', url: 'https://github.com/orgs/fixture-org/projects/7' }],
  memberships: [{ id: 'PVTI_43', issueId: 'I_43', projectId: 'P_fixture' }, { id: 'PVTI_42', issueId: 'I_42', projectId: 'P_fixture' }],
}
const exactPreview = 'SYNTHETIC REVIEW ONLY — no authority\nAccount: fixture-maintainer (U_fixture)\nIssues: fixture-org/demo #43 (I_43), #42 (I_42)\nProject: fixture-org #7 (P_fixture)\nMemberships: PVTI_43, PVTI_42\nOperations: setProjectItemField, addIssueDependency\nOnly this live requesting session. No automatic renewal.'
function projection(phase = 'awaiting-approval', grants = [], history = []) {
  return { version: 1, toolName: githubToolName, callId: githubCallId, phase, scope, exactPreview, grants, history }
}
...
const history = ['confirmed', 'failed', 'uncertain', 'unattempted'].map((outcome, index) => ({
  id: `change-${index}`, operation: index % 2 ? 'addIssueDependency' : 'setProjectItemField', outcome,
...
async function mockProjection(page, initial) {
  ...
  await page.route('**/api/plugins/github/*', async route => {
    ...
    if (action === 'revoke') {
      expect(body).toEqual({ sessionId: githubSessionId, callId: githubCallId, grantId: activeGrant.id })
      value = { ...value, phase: 'revoked', grants: value.grants.map(grant => ({ ...grant, state: 'revoked' })) }
    } else {
  ...
  return { requests, set(valueNext) { value = valueNext } }
}
async function openGrant(page, theme = 'light', narrow = false) {
  ...
  await expect(card.getByRole('heading', { name: 'Manage selected issues for this session' })).toBeVisible()
  ...
  if (narrow) await card.evaluate(element => { element.style.width = '320px'; element.style.maxWidth = '100%' })
...
async function artifact(card, testInfo, name) {
  ...
  await page.setViewportSize({ width: page.viewportSize().width, height: Math.ceil(bounds.height) + 600 })
...
test('shared host mounts the real GitHub projection route without granting access', async ({ app }) => {
  ...
  const response = await app.request.post(new URL('/api/plugins/github/status', app.url()).href, {
    headers: { origin: new URL(app.url()).origin, 'x-dsh-github': '1', 'content-type': 'application/json' },
    data: { sessionId: githubSessionId, callId: githubCallId },
  })
  expect(response.status()).toBe(200)
  expect(await response.json()).toMatchObject({ ok: true, value: { version: 1, phase: 'expired', grants: [], history: [] } })
})

for (const [theme, narrow] of [['light', false], ['dark', false], ['light', true]]) {
  test(`GitHub grant review in real shell (${theme}${narrow ? ', narrow' : ''})`, async ({ app }, testInfo) => {
    ...
    await expect(card.getByRole('link', { name: /fixture-org\/demo #43/ })).toHaveAttribute('href', scope.issues[0].url)
    await expect(card.getByRole('link', { name: /fixture-org project #7/ })).toBeVisible()
    await expect(card.getByText('Update supported board fields for granted issue memberships', { exact: false })).toBeVisible()
    await expect(card.getByText('Add a dependency between two granted issues', { exact: false })).toBeVisible()
    ...
    await keyboardOpen(card.locator('summary').filter({ hasText: 'Exact existing project memberships (2)' }))
    await expect(card.getByText('Item ID: PVTI_43; Issue ID: I_43; Project ID: P_fixture')).toBeVisible()
    await keyboardOpen(card.locator('summary').filter({ hasText: 'Complete exact approval preview' }))
    await expect(card.locator('pre').filter({ hasText: 'SYNTHETIC REVIEW ONLY' })).toHaveText(exactPreview)
...
test('active grants, history, keyboard revoke, expired and renewal states', async ({ app }, testInfo) => {
  ...
  for (const outcome of ['confirmed', 'failed', 'uncertain', 'unattempted']) await expect(changes.getByRole('listitem').filter({ hasText: `: ${outcome}` })).toHaveCount(1)
  await expect(changes.getByRole('alert')).toHaveText('The write may have succeeded. Do not retry automatically. Inspect GitHub before requesting fresh approval.')
  ...
  const revoke = grants.getByRole('button', { name: `Revoke access ${activeGrant.id}`, exact: true })
  await revoke.focus()
  ...
  for (const [phase, label] of [['expired', 'Expired — fresh approval required'], ['renewal-required', 'Renewal required']]) {
