...
function fixture(options = {}) {
  ...
  const subprocess = fakeSubprocess((spec) => {
    ...
    if (query.startsWith('mutation')) {
      mutations++
      return options.mutation?.(request) ?? json({ data: mutationResult('setProjectItemField') })
    }
    let data
...
async function grant(f, value = input) {
  return f.grants.accept(await f.grants.prepare(value, f.exec), f.exec)
}
async function write(f, operation = 'setProjectItemField') {
  return f.runtime.prepare(operation, args[operation], f.exec)
}
function execute(f, prepared, permit) {
  return f.runtime.execute(prepared, f.exec, {
    onPreflight: (fresh) => f.grants.observe(fresh.actor, f.exec),
    beforeDispatch: () => f.grants.assert(permit, prepared, f.exec),
  })
}
...
test('explicit bounded issue/project/operation arguments reject wildcard, unknown, duplicate and unbounded targets', () => {
  assert.deepEqual(validateGrantArguments(input).operations, [
    'addIssueDependency',
    'setProjectItemField',
  ])
  for (const change of [
...
test('scope resolution binds immutable account, owner, repository, issue, project and membership identities', async () => {
  ...
  assert.deepEqual(token.scope.memberships, [
    { id: item.id, issueId: issue.id, projectId: project.id },
  ])
  assert.equal(token.scope.projects[0].ownerId, owner.id)
  assert.equal(
    token.scope.issues.find((value) => value.id === issue.id).repositoryOwnerId,
    'O_SOURCE',
  )
      ...
      const f = fixture({
        read(data, query) {
          ...
          if (query === GRANT_READS.issue && data.repository.issue.id === issue.id) {
            ...
            if (mode === 'membership')
              data.repository.issue.projectItems.nodes[0].id = 'PI_REPLACED'
          }
          if (query === GRANT_READS.project && mode === 'project')
            data.repositoryOwner.projectV2.id = 'P_REPLACED'
        },
      })
      let token = await f.grants.prepare(input, f.exec)
      changed = true
...
test('truncated memberships, inaccessible projects, unknown repository and account inconsistency fail closed', async (t) => {
  for (const mode of ['truncated', 'inaccessible', 'repo', 'account'])
    await t.test(mode, async () => {
      const f = fixture({
        read(data, query) {
          if (query === GRANT_READS.issue) {
...
test('only selected field memberships and both selected dependency endpoints receive permits', async () => {
  ...
  assert.ok(permit)
  f.grants.assert(permit, prepared, f.exec)
  ...
  for (const mutate of [
    (p) => {
      p.targets.item.id = 'OTHER'
    },
    (p) => {
      p.targets.item.content.id = 'OTHER'
    },
    (p) => {
      p.targets.item.content.repository.id = 'OTHER'
    },
    (p) => {
      p.targets.item.content.__typename = 'PullRequest'
    },
    (p) => {
      p.targets.project.id = 'OTHER'
    },
    (p) => {
      p.targets.project.owner.id = 'OTHER'
    },
  ]) {
  ...
  const outside = structuredClone(dependency)
  outside.targets.blockingIssue.id = 'OTHER'
...
test('reused repository owner login cannot substitute immutable owner identity in field or dependency permits', async () => {
  const f = fixture()
  await grant(f)
  const field = await write(f)
  const dependency = await write(f, 'addIssueDependency')
  assert.ok(f.grants.check(field, f.exec))
  assert.ok(f.grants.check(dependency, f.exec))
  for (const original of [field, dependency]) {
    const changed = structuredClone(original)
    const repository =
      original.operation === 'setProjectItemField'
        ? changed.targets.item.content.repository
        : changed.targets.blockingIssue.repository
    repository.owner.id = 'O_REPLACEMENT'
    assert.equal(
      f.grants.check(changed, f.exec),
      null,
      'same repo ID and name cannot replace repository owner ID',
    )
  }
})
...
test('native dependency add dispatches exactly once with both approved issue identities', async () => {
  ...
  const prepared = await write(f, 'addIssueDependency')
  const permit = f.grants.check(prepared, f.exec)
  assert.equal((await execute(f, prepared, permit)).outcome, 'confirmed')
...
test('exact live session, agent, cwd, no child authority and lifecycle disposal are enforced', async () => {
  ...
  const permit = f.grants.check(prepared, f.exec)
  assert.equal(
    f.grants.check(prepared, { ...f.exec, session: {} }),
    null,
    'restored same textual identity never inherits',
  )
  for (const change of [
    { agentId: 'other' },
    { cwd: '/other' },
    ...
    assert.throws(() => f.grants.assert(permit, prepared, { ...f.exec, ...change }))
  f.grants.disposeSession(f.exec.session)
  assert.throws(() => f.grants.assert(permit, prepared, f.exec))
  await assert.rejects(f.grants.prepare(input, f.exec))
  const g = fixture()
  await grant(g)
  const p = await write(g)
  const q = g.grants.check(p, g.exec)
  g.grants.dispose()
...
test('connection status and child preflight observations invalidate grants without granting child authority', async (t) => {
  for (const mode of ['connectionStatus', 'childPreflight'])
    await t.test(mode, async () => {
      let changed = false
      const f = fixture({
        read(data, query) {
          if (changed && query === WRITE_READS.setProjectItemField)
            data.viewer = { id: 'U_OTHER', login: 'other' }
        },
      })
      await grant(f)
      const prepared = await write(f)
      ...
      if (mode === 'connectionStatus') {
        const reads = createGitHubRuntime(
          fakeSubprocess([json({ data: { viewer: { id: 'U_OTHER', login: 'other' } } })]),
          { onAccount: (actor) => f.grants.observeAccount(actor) },
        )
        assert.equal((await reads.connectionStatus({}, child)).data.account, 'other')
      } else {
        changed = true
        await f.runtime.prepare('setProjectItemField', args.setProjectItemField, child, {
          onAccount: (actor) => f.grants.observeAccount(actor),
        })
      }
...
test('account change during dispatch recheck invalidates the grant before conflict rejection', async () => {
  ...
  const f = fixture({
    read(data, query) {
      if (changed && query === WRITE_READS.setProjectItemField) data.viewer.id = 'U_OTHER'
    },
  })
  await grant(f)
  const prepared = await write(f)
...
test('account observations invalidate grants even when scope resolution or write permissions fail', async () => {
  ...
  const f = fixture({
    read(data, query) {
      ...
      if (query === WRITE_READS.setProjectItemField) {
  ...
  await assert.rejects(
    f.runtime.prepare('setProjectItemField', args.setProjectItemField, f.exec, {
      onAccount: (actor) => f.grants.observe(actor, f.exec),
    }),
...
test('revocation after executable resolution but before spawn prevents actual mutation and dispatch notification', async () => {
  ...
  await assert.rejects(
    f.runtime.execute(prepared, f.exec, {
      beforeDispatch: () => f.grants.assert(permit, prepared, f.exec),
      onDispatch: () => {
        dispatched = true
      },
...
test('queued writes recheck revocation and disposal at the real predispatch boundary', async (t) => {
  for (const mode of ['revoke', 'dispose'])
    await t.test(mode, async () => {
      const f = fixture()
      const g = await grant(f)
      const first = await write(f)
      const second = await write(f)
      const a = f.grants.check(first, f.exec)
      const b = f.grants.check(second, f.exec)
      let release
      let entered
      ...
      const running = execute(f, first, a)
      const queued = execute(f, second, b)
      await ready
      if (mode === 'revoke') f.grants.revoke(g.id, f.exec)
      else f.grants.dispose()
      release()
...
test('history distinguishes unattempted, failed, confirmed and uncertain without retry', async () => {
  const f = fixture({ mutation: () => ({ stdout: 'lost response' }) })
  await grant(f)
  const prepared = await write(f)
  const permit = f.grants.check(prepared, f.exec)
  ...
  f.grants.outcome(record, result.outcome)
  f.grants.outcome(record, 'confirmed')
  assert.equal(
    f.grants.history(f.exec)[0].outcome,
    'uncertain',
  ...
  assert.equal(
    f.grants.check(await write(f), f.exec),
    null,
    'a fresh preparation cannot silently repeat an uncertain write',
  )
  assert.throws(() => f.grants.assert(permit, prepared, f.exec))
  ...
  assert.ok(
    f.grants.check(await write(f), f.exec),
    'fresh explicit grant acceptance renews authority',
  )
  for (const state of ['failed', 'confirmed']) {
    const token = f.grants.attempt(prepared, f.exec)
    f.grants.outcome(token, state)
  }
  assert.deepEqual(
    f.grants.history(f.exec).map((value) => value.outcome),
    ['uncertain', 'failed', 'confirmed'],
  )
