...
import { createGitHubPresentation, fieldPresentation, FIELD_TOOL_NAME } from '../src/presentation.js'
...
function presentationFor(agent, root = true) {
  return createGitHubPresentation({ agents: { get: id => id === agent.session.id ? agent : undefined, roots: () => root ? [agent] : [] }, grants: noGrants,
...
test('field projection copies only verified prepared leaves for all supported field types', async () => {
  const cases = [['F_TEXT', { text: 'A  multiline\nvalue' }], ['F_NUMBER', { number: 0 }], ['F_DATE', { date: '2024-02-29' }], ['F_STATUS', { singleSelectOptionId: 'OPT_READY' }], ['F_ITERATION', { iterationId: 'ITER_ACTIVE' }], ['F_ITERATION', { iterationId: 'ITER_OLD' }]]
  for (const [fieldId, value] of cases) {
    const subprocess = fakeSubprocess([json({ data: snapshot(operation) })])
    const prepared = await createGitHubWriteRuntime(subprocess).prepare(operation, { ...args[operation], fieldId, value }, { agentId: 'session', cwd: '/fixture' })
    ...
    if (fieldId === 'F_STATUS') { assert.equal(view.change.before.optionId, 'OPT_TODO'); assert.equal(view.change.selectedOption.name, 'Ready') }
...
test('missing previous value is not converted into an explicit unset value', () => {
  const prepared = { preview: 'complete', targets: { project: { id: 'P' }, item: { id: 'I', project: { id: 'P' }, content: null } }, change: { field: { id: 'F', dataType: 'TEXT' }, after: { text: 'after' } } }
  assert.equal(fieldPresentation(prepared).change.before, undefined)
  assert.equal(fieldPresentation({ ...prepared, change: { ...prepared.change, before: null } }).change.before, null)
})
test('native approval retains the exact payload and status reads never query GitHub', async t => {
  let presentation; let subprocess
  const host = await approvalHost(t, { answer: request => {
  ...
  subprocess = fakeSubprocess(spec => json({ data: JSON.parse(spec.stdio.stdin.data).query.startsWith('mutation') ? mutationResult(operation) : snapshot(operation) }))
...
test('trusted denial, preflight failure and uncertain dispatch have distinct bridge evidence', async t => {
  for (const variant of ['denied', 'failed', 'uncertain']) await t.test(variant, async t => {
    ...
    const presentation = presentationFor(host.agent)
    const subprocess = fakeSubprocess(spec => JSON.parse(spec.stdio.stdin.data).query.startsWith('mutation') ? { stdout: 'unusable result', exitCode: 1 } : json({ data: snapshot(operation) }))
    registerGitHubWriteTools(host.ctx, createGitHubWriteRuntime(subprocess), { presentation })
    await host.execute(FIELD_TOOL_NAME, variant === 'failed' ? { ...args[operation], value: { date: 'invalid date' } } : args[operation])
    const view = presentation.status({ sessionId: host.session.id, callId: 'write-call-1' })
    assert.equal(view.phase, variant)
    if (variant === 'uncertain') {
      assert.match(view.result.message, /may have succeeded/)
      presentation.settled({ agent: host.agent, name: FIELD_TOOL_NAME, callId: 'write-call-1' }, { outcome: 'confirmed', host: 'github.com' })
      assert.equal(presentation.status({ sessionId: host.session.id, callId: 'write-call-1' }).phase, 'uncertain')
    }
...
test('field status supports a live child without exposing or authorizing any root grant', () => {
  ...
  presentation.settled({ agent, name: FIELD_TOOL_NAME, callId: 'child-call' }, { outcome: 'failed', host: 'github.com' })
