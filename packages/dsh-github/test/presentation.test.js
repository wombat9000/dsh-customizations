...
function fixture() {
  ...
  const grants = {
    list: () => [{ id: 'grant', state: grantState }],
    history: () => [{ id: 'attempt', outcome: 'uncertain' }],
    historyTruncated: () => true,
    revoke: (id) => {
      assert.equal(id, 'grant')
      grantState = 'revoked'
    },
  }
  const presentation = createGitHubPresentation({
    agents: { get: (id) => (id === 'root' ? current : undefined), roots: () => [current] },
    grants,
    caller: (exec) => ({ session: exec.agent.session, isSubagent: false }),
  })
...
test('bounded presentation binds exact live session and never restores preview or authority', () => {
  ...
  assert.deepEqual(f.presentation.status(key), {
    version: 1,
    phase: 'expired',
    grants: [],
    history: [],
...
test('presentation uses exact correlated approval audit without equating approval with execution', () => {
  ...
  f.events.push({
    type: 'approval/asked',
    data: { id: 'ask', toolName: f.exec.name, callId: 'call' },
  })
...
test('native grant review is complete readable prose, with explicit exclusions and unavailable actions', () => {
  const scope = {
    ...
    memberships: [{ id: 'ITEM1', issueId: 'I1', projectId: 'P1' }],
    operations: ['setProjectItemField'],
  }
  ...
  for (const value of [
    ...
    assert.ok(preview.includes(value), value)
...
test('card routes reject cross-origin requests, injected actions and ambiguous identity bodies', () => {
  ...
  for (const patch of [
    ...
    { host: 'evil.example' },
    { 'x-dsh-github': undefined },
    { 'sec-fetch-site': 'cross-site' },
    { 'content-type': 'text/plain' },
  ])
    assert.equal(allowedRequest({ ...req, headers: { ...req.headers, ...patch } }, 3000), false)
  assert.equal(allowedRequest({ ...req, socket: { remoteAddress: '10.0.0.2' } }, 3000), false)
  assert.deepEqual(validateInput({ sessionId: 's', callId: 'c' }, 'status'), {
    sessionId: 's',
    callId: 'c',
  })
  for (const [input, action] of [
    [{ sessionId: 's', callId: 'c' }, 'approve'],
    [{ sessionId: 's', callId: 'c', grantId: 'g' }, 'status'],
    [{ sessionId: 's', callId: 'c' }, 'revoke'],
    [{ sessionId: '', callId: 'c' }, 'status'],
  ])
    assert.throws(() => validateInput(input, action))
})
