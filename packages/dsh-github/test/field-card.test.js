...
const result = (outcome) => ({
  ...
  content: [
    {
      type: 'text',
      text: JSON.stringify({ host: 'github.com', operation: 'setProjectItemField', outcome }),
    },
  ],
})
...
test('exact supported field values preserve empty, zero, fractional and multiline values', () => {
  for (const [change, before, after] of [
    ...
    [
      value(
        'SINGLE_SELECT',
        item.fieldValues.nodes[0],
        { singleSelectOptionId: 'OPT_READY' },
    ...
    [
      value(
        'ITERATION',
        { iterationId: 'OLD', title: 'Old sprint' },
        { iterationId: 'NEW' },
        { id: 'NEW', title: 'Current sprint', startDate: '2026-01-01', duration: 14 },
      ),
      'Old sprint',
      'Current sprint',
    ],
...
test('completed iterations and missing friendly names use verified selected identity', () => {
  ...
  assert.equal(
    plugin.fieldValueModel(
      { ...change, selectedOption: { id: 'WRONG', title: 'Misleading' } },
      'after',
    ).label,
    selected.id,
  )
...
test('null means unset while absent or malformed values never imply clearing or a previous value', () => {
  ...
  for (const change of [
    value('TEXT', undefined, { text: 'new' }),
    value('TEXT', { number: 4 }, { text: 'new' }),
    value('TEXT', { text: 'old', field: { id: 'FOREIGN' } }, { text: 'new' }),
  ])
    assert.equal(plugin.fieldValueModel(change, 'before').available, false)
  for (const change of [
    value('NUMBER', null, { number: Infinity }),
    value('DATE', null, { date: '2026-02-30' }),
    value('TEXT', null, { text: 'new', number: 1 }),
    value('NUMBER', null, { number: '2' }),
    { field: { id: 'F', dataType: 'LABELS' }, after: {} },
  ])
    assert.equal(plugin.fieldValueModel(change, 'after').available, false)
})
test('field DTO binds version, tool and call while accepting only explicit expired identity fallback', () => {
  ...
  assert.equal(
    plugin.validFieldStatus({ version: 1, phase: 'expired', grants: [], history: [] }, 'call'),
    true,
  )
  for (const wrong of [
    null,
    ...
    { version: 1, phase: 'confirmed' },
    { ...status, exactPreview: {} },
  ])
    assert.equal(plugin.validFieldStatus(wrong, 'call'), false)
})
test('explicit uncertainty wins over confirmation; approval never becomes confirmation', () => {
  ...
  assert.equal(
    plugin.fieldPhase({ phase: 'confirmed' }, { ...result('confirmed'), isError: true }),
    'uncertain',
  )
...
test('no-change requires its structured reason and explicit no-dispatch evidence', () => {
  ...
  for (const invalid of [
    ...
    { ...value, dispatched: undefined },
    { ...value, dispatched: true },
    { ...value, reason: 'already exists' },
  ]) {
    assert.equal(plugin.fieldResult(block(invalid)), undefined)
    assert.equal(
      plugin.fieldPhase({ phase: 'no-change', result: invalid }, block(invalid)),
      'unknown',
    )
  }
  assert.equal(
    plugin.fieldPhase(null, {
      kind: 'tool-result',
      isError: true,
      content: [
        {
...
test('malformed or foreign result envelopes never imply success', () => {
  for (const payload of [
    { outcome: 'confirmed' },
    { host: 'github.com', operation: 'createIssue', outcome: 'confirmed' },
    { host: 'github.com', outcome: 'confirmed' },
  ])
    assert.equal(
      plugin.fieldResult({
        kind: 'tool-result',
  ...
  assert.equal(
    plugin.fieldPhase({ phase: 'confirmed', result: { outcome: 'confirmed' } }, malformed),
    'unknown',
  )
  assert.equal(
    plugin.fieldPhase(
      {
        phase: 'confirmed',
        result: { host: 'github.com', operation: 'setProjectItemField', outcome: 'confirmed' },
      },
      malformed,
    ),
    'confirmed',
