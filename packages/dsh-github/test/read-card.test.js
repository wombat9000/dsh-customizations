...
test('six actual payload shapes render supplied identities without network access', () => {
  for (const [tool, data, kind] of [['github_list_projects', connection([project]), 'projects'], ['github_get_project', detailedProject, 'projects'], ['github_list_project_items', connection([projectItem]), 'items'], ['github_list_issues', connection([issue]), 'issues'], ['github_search_issues', { ...connection([issue]), issueCount: 1, searchLimit: 1000, exhaustive: true }, 'issues'], ['github_get_issue', detailedIssue, 'issues']]) {
...
test('empty template page preserves scanned/unfiltered counts and continuation', () => {
  const model = readCardModel('github_list_projects', block({ ...connection([], true, 'next'), totalCount: 100, scannedCount: 20, templateOnly: true, totalCountMeaning: 'Unfiltered owner projects' }))
...
test('search cap and outer truncation remain visible alongside nested cursor warnings', () => {
  const model = readCardModel('github_search_issues', block({ ...connection([issue]), issueCount: 2000, exhaustive: false }, { truncated: true, truncations: [{ path: 'data', kind: 'search-cap', reason: 'Narrow this query' }] }))
...
test('malformed arrays/entries/envelopes fail safely without invented counts', () => {
  for (const data of [null, [], { nodes: 'wrong' }, { nodes: [null] }, { nodes: [7] }, { nodes: [{ id: 'I', title: 'x', number: '1' }] }]) assert.notEqual(readCardModel('github_list_issues', block(data)).state, 'returned')
  assert.equal(readCardModel('github_list_issues', { kind: 'tool-result', content: [{ type: 'text', text: 'malformed' }] }).state, 'unknown')
...
test('nested UI bounds and missing metadata never imply completeness', () => {
  ...
  const data = { ...detailedProject, fields: { nodes: [{ id: 'F', options }], pageInfo: { hasNextPage: false } }, repositories: { nodes: Array.from({ length: 51 }, () => ({ id: 'R' })) } }
...
test('deep malformed data is bounded and produces an explicit inspection warning', () => {
  let data = {}; let cursor = data
  for (let i = 0; i < 30; i++) cursor = cursor.next = {}
  assert.ok(readWarnings({ data }, 'github_get_issue').some(value => value.includes('inspection bound')))
})
