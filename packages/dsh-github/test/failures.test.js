...
const secrets =
  'ghp_SYNTHETIC_SECRET https://user:synthetic-password@github.com Authorization: Bearer synthetic-bearer'
const safeError = (code) => (error) => {
  assert.ok(error instanceof GitHubError)
  assert.equal(error.code, code)
  assert.doesNotMatch(
    JSON.stringify({ ...error, message: error.message }),
    /SYNTHETIC_SECRET|synthetic-password|synthetic-bearer|\/private\//,
  )
  return true
}
...
test('missing CLI and resolver exceptions report fixed sanitized diagnostics', async () => {
  for (const options of [
    { missing: 'gh' },
    { resolveError: Object.assign(new Error(`/private/gh ${secrets}`), { code: 'ENOENT' }) },
  ]) {
...
test('authentication, permissions, not-found and rate-limit errors do not retry or expose raw diagnostics', async () => {
  for (const [stderr, code] of [
    ...
    await assert.rejects(
      createGitHubRuntime(subprocess, { maxRetries: 2 }).getRepository(target, exec),
      safeError(code),
    )
    assert.equal(subprocess.specs.length, 1)
  }
  const runtime = createGitHubRuntime(
    fakeSubprocess([{ stderr: `HTTP 401 ${secrets}`, exitCode: 1 }]),
  )
  const status = await runtime.connectionStatus({}, exec)
...
test('GraphQL errors override partial data and remain sanitized', async () => {
  const subprocess = fakeSubprocess([
    json({ data: { repository }, errors: [{ message: `Resource not accessible ${secrets}` }] }),
  ])
  await assert.rejects(
    createGitHubRuntime(subprocess).getRepository(target, exec),
...
test('network failures retry identical read-only argv within the configured hard cap', async () => {
  const subprocess = fakeSubprocess([
    { stderr: `HTTP 503 ${secrets}`, exitCode: 1 },
  ...
  for (const [maxRetries, expectedCalls] of [
    [0, 1],
    [1, 2],
    [2, 3],
    [99, 3],
  ]) {
    const failed = fakeSubprocess(() => ({
      stderr: 'HTTP 502 temporarily unavailable',
      exitCode: 1,
    }))
    await assert.rejects(
      createGitHubRuntime(failed, { maxRetries }).getRepository(target, exec),
      safeError('NETWORK_ERROR'),
    )
    assert.equal(failed.specs.length, expectedCalls)
  }
  const graphql = fakeSubprocess([
    json({ errors: [{ message: 'network connection temporarily unavailable' }] }),
    json({ data: { repository } }),
  ])
...
test('spawn errors, malformed JSON, invalid connection payloads and absent resources fail closed', async () => {
  for (const [run, code] of [
    ...
    [json({ data: { repository: {} } }), 'INVALID_RESPONSE'],
    [
      json({ data: { repository: { ...repository, url: 'javascript:alert(1)' } } }),
      'INVALID_RESPONSE',
    ],
  ]) {
  ...
  for (const value of [
    ...
    const subprocess = fakeSubprocess([
      json({ data: { repositoryOwner: { repositories: value } } }),
    ])
    await assert.rejects(
      createGitHubRuntime(subprocess).listRepositories({ owner: 'acme' }, exec),
...
test('lossy or oversized subprocess stdout never returns partial JSON or retries', async () => {
  for (const [run, config] of [
    ...
    const subprocess = fakeSubprocess([run])
    await assert.rejects(
      createGitHubRuntime(subprocess, config).getRepository(target, exec),
...
test('timeouts abort managed execution and do not retry', async () => {
  ...
  await assert.rejects(
    createGitHubRuntime(subprocess, { timeoutMs: 5, maxRetries: 2 }).getRepository(target, exec),
    safeError('TIMEOUT'),
  )
  assert.equal(subprocess.specs.length, 1)
...
test('caller cancellation aborts active execution and pre-cancelled calls never resolve a CLI', async () => {
  ...
  await assert.rejects(
    createGitHubRuntime(subprocess).getRepository(target, { ...exec, signal: controller.signal }),
    safeError('CANCELLED'),
  )
  assert.equal(subprocess.specs.length, 1)
  assert.equal(subprocess.specs[0].signal.aborted, true)
  ...
  await assert.rejects(
    createGitHubRuntime(untouched).getRepository(target, { ...exec, signal: controller.signal }),
    safeError('CANCELLED'),
  )
...
test('CLI resolution itself receives timeout and caller cancellation signals', async () => {
  for (const cancel of [false, true]) {
    ...
    const subprocess = {
      ...
      spawn() {
        assert.fail('no spawn after failed resolution')
      },
    }
    await assert.rejects(
      createGitHubRuntime(subprocess, { timeoutMs: 5 }).getRepository(target, {
        ...exec,
        signal: controller.signal,
      }),
      safeError(cancel ? 'CANCELLED' : 'TIMEOUT'),
    )
...
test('large text and non-paginated project options expose precise non-continuable truncation', async () => {
  const payload = {
    ...
    fields: connection([
      {
        ...detailedProject.fields.nodes[0],
        options: Array.from({ length: 55 }, (_, i) => ({ id: `O_${i}`, name: `Option ${i}` })),
      },
    ]),
    repositories: connection([]),
  }
  const subprocess = fakeSubprocess([json({ data: { repositoryOwner: { projectV2: payload } } })])
  const result = await createGitHubRuntime(subprocess).getProject(
  ...
  assert.ok(
    result.truncations.some(
      (row) => row.path === 'data.readme' && row.kind === 'text' && row.continuation === null,
    ),
  )
  assert.ok(
    result.truncations.some(
...
test('result byte bounds fail closed and issue body secrets are sanitized', async () => {
  ...
  const subprocess = fakeSubprocess([
    json({ data: { repository: { issue: { ...detailedIssue, body: secrets } } } }),
  ])
  const result = await createGitHubRuntime(subprocess).getIssue(
    { ...target, issueNumber: 33 },
    exec,
  )
  assert.doesNotMatch(
    JSON.stringify(result),
    /SYNTHETIC_SECRET|synthetic-password|synthetic-bearer/,
  )
})
...
test('discovery handles no remotes and rejects malformed or oversized remote inventories', async () => {
  ...
  for (const [stdout, code] of [
    ...
    await assert.rejects(
      createGitHubRuntime(subprocess).detectRepositories({}, exec),
      safeError(code),
    )
  ...
  await assert.rejects(
    createGitHubRuntime(missing).detectRepositories({}, exec),
    safeError('CLI_UNAVAILABLE'),
  )
})

test('tool wrappers never expose arbitrary runtime exception properties or credentials', async () => {
  for (const error of [
    Object.assign(new Error(secrets), { stdout: secrets, stderr: secrets, path: '/private/gh' }),
    new GitHubError('AUTH_REQUIRED', 'Authentication required.'),
  ]) {
    ...
    const value = JSON.parse(
      await tools.find((tool) => tool.name === 'github_get_repository').execute(target, {}),
    )
