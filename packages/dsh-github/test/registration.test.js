...
test('eleven plain definitions forward only the calling workspace and cancellation', async () => {
  ...
  for (const tool of definitions) {
    ...
    await tool.execute(args, {
      cwd: '/wrong',
      signal,
      agent: { session: { header: { cwd: '/session/a' } } },
    })
    assert.deepEqual(calls.at(-1), {
      method: camel(tool.name),
      args,
      exec: { cwd: '/session/a', signal },
    })
  }
})
...
test('real pinned DSH registry exposes host tools across preset/session scopes and disposes them', async (t) => {
  ...
  for (const agent of [a, b]) {
    ...
    const result = JSON.parse(
      await tool.execute({}, { agent, signal: new AbortController().signal }),
    )
  ...
  assert.deepEqual(
    [...new Set(subprocess.specs.map((spec) => spec.cwd))],
    ['/session/a', '/session/b'],
  )
