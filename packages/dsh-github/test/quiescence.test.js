...
test('command completion does not finish a write or release its queue before managed range quiescence', async () => {
  ...
  const first = runtime.execute(a, exec).then((result) => {
    firstReturned = true
    return result
  })
  ...
  assert.equal(
    ...
    4,
    'second execute cannot run its recheck while the first mutation range survives',
  )
...
test('cleanup uses a separate live signal even after caller cancellation', async () => {
  ...
  await assert.rejects(runCollected(subprocess, { ...runOptions, signal: controller.signal }), {
    code: 'CANCELLED',
  })
  assert.equal(terminated, 1)
  ...
  for (const [label, waitForExit] of [
    ...
    await t.test(label, async () => {
      ...
      await assert.rejects(
        createGitHubRuntime(subprocess).getRepository({ owner: 'source', repo: 'example' }, exec),
        { code: 'CLEANUP_FAILED' },
      )
      await assert.rejects(
...
test('mutation cleanup failure reports uncertainty and prevents all subsequent work on that backend', async () => {
  const subprocess = fakeSubprocess([
    json({ data: snapshot('createIssue') }),
    json({ data: snapshot('createIssue') }),
  ...
  assert.equal(isGitHubBackendFenced(subprocess), true)
  await assert.rejects(runtime.prepare('createIssue', args.createIssue, exec), {
    code: 'CLEANUP_FAILED',
  })
