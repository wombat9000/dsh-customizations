...
const scope = { operations: ['setProjectItemField', 'addIssueDependency'], issues: [{ owner: 'source', repo: 'example', issueNumber: 33 }, { owner: 'blockers', repo: 'other', issueNumber: 34 }], projects: [{ owner: 'destination', projectNumber: 7 }] }
const fieldTool = 'github_set_project_item_field'
const fieldArgs = args.setProjectItemField
const deferred = () => { let resolve; const promise = new Promise(done => { resolve = done }); return { promise, resolve } }
function content(result) { return result.value ? JSON.parse(result.value) : JSON.parse(result.content.find(part => part.type === 'text').text) }

async function fixture(t, options = {}) {
  const host = await approvalHost(t, options)
  let live = host.agent; let roots = [host.agent]
  ...
  const events = []
  const presentation = Object.fromEntries(['request', 'phase', 'prepared', 'settled'].map(name => [name, (...args) => events.push({ name, args })]))
  let mutations = 0
  const subprocess = fakeSubprocess(spec => {
    const request = JSON.parse(spec.stdio.stdin.data); const { query, variables } = request
    if (query.startsWith('mutation')) {
      mutations++
      const operation = query.includes('AddBlockedByInput') ? 'addIssueDependency' : query.includes('CreateIssueInput') ? 'createIssue' : 'setProjectItemField'
  ...
  const writeRegistration = registerGitHubWriteTools(host.ctx, runtime, { grants, grantCaller: caller, presentation })
  return { ...host, runtime, grants, caller, events, subprocess, grantRegistration, writeRegistration,
    mutations: () => mutations,
    owner: () => caller({ agent: host.agent }),
    registry: { replace(agent) { live = agent; roots = [agent] }, removeRoot() { roots = [] }, removeOwner() { live = undefined } },
  }
}
...
test('trusted approval creates finite authority once; matching field/dependency writes omit only the second prompt', async t => {
  ...
  assert.equal(content(await host.execute('github_add_issue_dependency', args.addIssueDependency)).outcome, 'confirmed')
  assert.equal(host.requests.length, 1)
  assert.equal(host.mutations(), 2)
  assert.deepEqual(host.audit().map(event => event.type), ['approval/asked', 'approval/decided'])
  assert.deepEqual(host.grants.history(host.owner()).map(row => row.outcome), ['confirmed', 'confirmed'])
  assert.ok(host.events.some(event => event.name === 'prepared' && event.args[2] === 'authorized-by-grant'))
...
test('history marks dispatched writes running before a final confirmed outcome', async t => {
  const host = await fixture(t, { answer: 'allowed-once', mutation: () => {
...
test('late cancellation after grant acceptance revokes authority before final tool output', async t => {
  ...
  assert.deepEqual(host.grants.list(host.owner()).map(grant => grant.state), ['revoked'])
  assert.equal(host.mutations(), 0)
})

test('rejected, unavailable, missing, malformed and policy-never approval grant no authority', async t => {
  for (const options of [
    { label: 'rejected', answer: 'rejected' }, { label: 'unavailable', answer: 'unavailable' },
    { label: 'missing answerer' }, { label: 'bad answer', answer: 'yes' },
    { label: 'throwing answerer', answer() { throw new Error('synthetic') } },
    { label: 'never', policy: 'never', answer: 'allowed-once' },
    { label: 'missing service', approval: false }, { label: 'no open turn', openTurn: false, answer: 'allowed-once' },
...
test('direct grant execution and replay never create a grant or dispatch mutations', async t => {
  ...
  const direct = JSON.parse(await tool.execute(scope, { name: GRANT_TOOL_NAME, arguments: scope, token: {}, agent: host.agent }))
...
test('changed scope or live agent after approval cannot replace a prepared grant', async t => {
  for (const mode of ['arguments', 'agent', 'root']) await t.test(mode, async t => {
    const host = await fixture(t, { answer: 'allowed-once' })
    host.ctx.on('tools/execute', async (exec, next) => {
...
test('registry root ownership denies child and stale agents; same-id restoration receives no authority', async t => {
  for (const mode of ['child', 'not-owner']) await t.test(mode, async t => {
    const host = await fixture(t, { answer: 'allowed-once' })
    if (mode === 'child') host.registry.removeRoot(); else host.registry.removeOwner()
    const result = await host.execute(GRANT_TOOL_NAME, scope)
    assert.equal(result.isError, true)
    assert.equal(host.requests.length, 0)
    assert.equal(host.subprocess.specs.length, 0)
  ...
  await t.test('restored same id', async t => {
    const host = await fixture(t, { answer: 'allowed-once' }); await allow(host)
...
test('registry ownership is rechecked after executable resolution immediately before dispatch', async t => {
  const host = await fixture(t, { answer: 'allowed-once' }); await allow(host)
  const original = host.subprocess.resolveExecutable; let calls = 0
...
test('cancelling a trusted pending grant request consumes preparation without creating authority', async t => {
  const controller = new AbortController()
  const host = await fixture(t, { answer() { controller.abort(); return new Promise(() => {}) } })
  const result = await host.execute(GRANT_TOOL_NAME, scope, { signal: controller.signal })
  assert.equal(result.isError, true)
...
test('another guard deny or ask remains authoritative with a matching grant', async t => {
  for (const kind of ['deny', 'ask']) await t.test(kind, async t => {
    const host = await fixture(t, { answer: 'allowed-once' }); await allow(host)
    host.ctx.on('tools/pre-execute', async () => ({ kind, reason: 'Independent policy requires review' }))
    const result = await host.execute(fieldTool, fieldArgs)
    if (kind === 'deny') {
      assert.equal(result.isError, true)
      assert.equal(host.requests.length, 1)
...
test('grant-request downstream denial and pending disposal never create authority', async t => {
  for (const mode of ['policy', 'dispose']) await t.test(mode, async t => {
    let host
    host = await fixture(t, { answer: () => { if (mode === 'dispose') host.grantRegistration.dispose(); return 'allowed-once' } })
    if (mode === 'policy') host.ctx.on('tools/pre-execute', async () => ({ kind: 'deny', reason: 'Independent deny' }))
    const result = await host.execute(GRANT_TOOL_NAME, scope)
...
test('queued revoke and cancellation prevent dispatch and retain accurate session history', { timeout: 10000 }, async t => {
  for (const mode of ['revoke', 'cancel']) await t.test(mode, async t => {
    const gate = deferred(); const rechecking = deferred(); const queued = deferred()
    let reads = 0; let executions = 0
    const host = await fixture(t, { answer: 'allowed-once', read(data, query) {
      if (query === WRITE_READS.setProjectItemField && ++reads === 2) {
        rechecking.resolve()
        return { ...json({ data }), waitForExit: async () => { await gate.promise; return true } }
      }
    } })
    ...
    if (mode === 'revoke') host.grants.revoke(grant.id, host.owner()); else controller.abort()
    gate.resolve()
    await Promise.all([first, second])
    assert.equal(host.requests.length, 1)
    assert.equal(host.mutations(), mode === 'cancel' ? 1 : 0)
    assert.deepEqual(host.grants.history(host.owner()).map(row => row.outcome), mode === 'cancel' ? ['confirmed', 'failed'] : ['failed', 'failed'])
  })
})

test('post-dispatch cancellation remains uncertain in real Tools content/history and quarantines grants', async t => {
  for (const late of [false, true]) await t.test(late ? 'after response' : 'during mutation', async t => {
    const controller = new AbortController()
    const host = await fixture(t, { answer: 'allowed-once', mutation() {
    ...
    await allow(host)
    if (late) host.ctx.on('tools/execute', async (exec, next) => { const result = await next(); if (exec.name === fieldTool) controller.abort(); return result })
