...
export async function approvalHost(
  t,
  { policy = 'ask', approval = true, answer, cwd = '/fixture/session-a', openTurn = true } = {},
) {
  ...
  return {
    ...
    audit() {
      return callerSession.snapshotEvents().filter((event) => event.type.startsWith('approval/'))
    },
  }
}
