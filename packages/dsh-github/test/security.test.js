...
const noCalls = () =>
  fakeSubprocess(() => assert.fail('invalid input must not execute a subprocess'))
...
test('explicit targets are required and discovery cannot establish an implicit selection', async () => {
  ...
  for (const [method, args] of [
    ['listRepositories', {}],
    ['getRepository', {}],
    ['getRepository', { owner: 'acme' }],
    ['listProjects', {}],
    ['getProject', { owner: 'acme' }],
    ['listProjectItems', { owner: 'acme' }],
    ['listIssues', { owner: 'acme' }],
    ['searchIssues', { query: 'test' }],
    ['getIssue', { owner: 'acme', repo: 'example' }],
...
test('invalid owner, repository, identifier, cursor, and limit inputs fail before subprocess execution', async () => {
  ...
  for (const owner of [
    ...
    await assert.rejects(
      runtime.getRepository({ owner, repo: 'example' }, exec),
      GitHubError,
      owner,
    )
  }
  ...
  for (const value of [0, -1, 1.5, '1', NaN, Infinity]) {
    await assert.rejects(
      runtime.getIssue({ owner: 'acme', repo: 'example', issueNumber: value }, exec),
      GitHubError,
    )
    await assert.rejects(
      runtime.getProject({ owner: 'acme', projectNumber: value }, exec),
      GitHubError,
    )
  }
...
test('search cannot override explicit owner/repository scope or select pull requests', async () => {
  ...
  for (const query of [
    ...
    'is:pr secret',
    'type:pr secret',
    'test OR repo:other/private',
    'test OR secret',
    'REPO:other/private',
    '-repo:acme/example',
  ])
    await assert.rejects(
      runtime.searchIssues({ owner: 'acme', repo: 'example', query }, exec),
      GitHubError,
      query,
    )
  assert.equal(subprocess.specs.length, 0)
...
test('remote normalization omits embedded credentials and rejects unsafe protocols, hosts, and shell forms', () => {
  for (const remote of [
    ...
    'https://ghp_supersecret@github.com/acme/example.git',
  ]) {
  ...
  for (const remote of [
    'https://github.com.evil.test/acme/example.git',
    'https://evil.test/acme/example.git',
    'http://github.com/acme/example.git',
    'file:///private/secret',
    'ext::sh -c echo',
    'git://github.com/acme/example.git',
    'ssh://git@evil.test/acme/example.git',
    'git@github.com:acme/example.git;echo unsafe',
...
test('untrusted @file strings use raw gh fields and never trigger typed-field file expansion', async () => {
  ...
  await createGitHubRuntime(subprocess).listIssues(
    {
      owner: 'acme',
      repo: 'example',
      cursor: '@/private/token',
      labels: ['@/private/credentials', '$(touch injected); `echo unsafe`'],
    },
    exec,
  )
...
test('unknown API/shell/mutation fields and ambiguous nested continuations are rejected', async () => {
  ...
  for (const extra of [
    { endpoint: '/user' },
    { query: 'mutation { unsafe }' },
    { method: 'DELETE' },
    { hostname: 'evil.test' },
    { argv: ['auth', 'token'] },
  ]) {
    await assert.rejects(
      runtime.getRepository({ owner: 'acme', repo: 'example', ...extra }, exec),
      { code: 'INVALID_ARGUMENT' },
    )
  }
  for (const extra of [
    { fieldValuesCursor: 'cursor' },
    { valueCursor: 'cursor' },
    { itemId: 'PI_1', valueCursor: 'cursor', fieldValuesLimit: 2 },
    { itemId: 'PI_1', cursor: 'cursor' },
    { fieldValuesLimit: 21 },
    { nestedLimit: 51 },
  ]) {
    await assert.rejects(
      runtime.listProjectItems({ owner: 'acme', projectNumber: 7, ...extra }, exec),
      { code: 'INVALID_ARGUMENT' },
    )
  }
...
test('sanitization preserves normal token prose but removes authorization credential values', () => {
  ...
  assert.doesNotMatch(
    sanitize('Authorization: token synthetic-private-credential'),
