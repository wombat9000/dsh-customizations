...
const issue = {
  ...
  state: 'OPEN',
  url: 'https://github.com/acme/example/issues/33',
}
const project = {
  id: 'P_1',
  number: 7,
...
const detailedIssue = {
  ...
  body: 'Synthetic body',
  parent: issue,
  labels: connection([{ name: 'bug' }], true),
  blockedBy: connection([issue], true),
  blocking: connection([issue], true),
}
  ...
  fields: connection(
    [
      ...
      },
      {
        ...
        name: 'Sprint',
        dataType: 'ITERATION',
        configuration: { iterations: [{ id: 'IT_1', title: 'Sprint 1' }], completedIterations: [] },
      },
    ],
    true,
  ),
...
const projectItem = {
  ...
  project: { id: 'P_1' },
  isArchived: false,
  content: { ...issue, __typename: 'Issue' },
  fieldValues: connection(
    [
      { field: { name: 'Status' }, name: 'Ready' },
      { field: { name: 'Labels' }, labels: connection([{ name: 'bug' }], true) },
    ],
    true,
  ),
}
...
test.each([
  ...
  'supported tool %s renders readable entries with zero HTTP calls',
...
test.each(['github_list_issues', 'github_search_issues'])(
  '%s compact rows preserve shared and mixed repository context',
  async (tool) => {
    ...
    expect(container.querySelector('.gh-issue-state').getAttribute('aria-label')).toBe(
      'Issue state: Open',
    )
    ...
    await mount(
      tool,
      connection([one, { ...one, id: 'I_2', repository: { nameWithOwner: 'acme/two' } }]),
    )
    expect(
      [...container.querySelectorAll('.gh-issue small')].map((node) => node.textContent),
...
test('empty template page warns that filtering and totals apply to different scopes', async () => {
  await mount('github_list_projects', {
    ...connection([], true),
    totalCount: 99,
    scannedCount: 20,
    templateOnly: true,
  ...
  expect(container.textContent).toContain(
    '0 entries returned; 99 total reported (Unfiltered owner projects)',
  )
...
test('outer truncation keeps otherwise empty sections visibly uncertain', async () => {
  await mount('github_get_issue', emptyIssue, {
    block: block(emptyIssue, {
      truncated: true,
      truncations: [{ path: 'data.labels', reason: 'bounded' }],
    }),
  })
  const notes = [...container.querySelectorAll('[role="note"]')]
  expect(
    notes.some((node) =>
      node.textContent.includes('Labels: no entries returned; completeness unknown'),
    ),
  ).toBe(true)
...
test.each(['github_list_issues', 'github_search_issues'])(
  '%s handles empty results and unknown issue states without board status',
  async (tool) => {
...
test('partial labels and body-only bounds do not unhide independently complete empty collections', async () => {
  ...
  await mount('github_get_issue', value, {
    block: block(value, {
      truncated: true,
      truncations: [
        { path: 'data.labels', kind: 'connection' },
        { path: 'data.body', kind: 'text' },
      ],
    }),
  })
  const notes = [...container.querySelectorAll('[role="note"]')]
  ...
  for (const name of ['Assignees:', 'Sub-issues:', 'Blocked by:', 'Blocking:'])
    expect(container.querySelector('.gh-grant').innerText).not.toContain(name)
...
test('populated issue collections show compact chips and directed safe links', async () => {
  await mount('github_get_issue', {
    ...emptyIssue,
    state: 'CLOSED',
    labels: connection([{ name: '<img src=x>' }]),
    assignees: connection([{ login: 'alice', url: 'https://github.com/alice' }]),
    blockedBy: connection([issue]),
    blocking: connection([{ ...issue, title: 'Unsafe link', url: 'javascript:alert(1)' }]),
  })
  expect(container.querySelector('.gh-issue-state').getAttribute('aria-label')).toBe(
    'Issue state: Closed',
  )
...
test('search cap and malformed payload never present exhaustive success', async () => {
  await mount('github_search_issues', {
    nodes: [null],
    issueCount: 1001,
    exhaustive: false,
...
test('all client-side nested truncation warnings remain outside collapsed bodies', async () => {
  ...
  await mount('github_get_project', {
    ...project,
    fields: connection([{ id: 'F', name: 'Status', options }]),
  })
