...
const scope = {
  ...
  issues: [
    ...
    },
    {
      ...
      nameWithOwner: 'fixture/repo',
      issueNumber: 2,
      url: 'https://evil.example/unsafe',
    },
  ],
  projects: [{ id: 'P_fixture', ownerId: 'U_fixture', owner: 'fixture', projectNumber: 3 }],
  memberships: [{ id: 'ITEM_fixture', issueId: 'I_one', projectId: 'P_fixture' }],
}
const pending = {
  version: 1,
  ...
  phase: 'pending',
  scope,
  exactPreview: 'Exact scope: fixture account U_fixture; issue I_one.  Preserve spaces.\nEnd.',
  grants: [],
  history: [],
}
const active = {
...
test('readable complete scope supplements native approval without custom allow controls', async () => {
  ...
  await expect
    .element(page.getByRole('heading', { name: 'Manage selected issues for this session' }))
    .toBeVisible()
...
test('revoke uses exact grant identity and observes revoked status; renewal never dispatches', async () => {
  ...
  const fixture = await mount((action) => {
    if (action === 'revoke') {
      revoked = true
      return { id: 'grant-one', state: 'revoked' }
    }
  ...
  await expect
    .element(page.getByRole('button', { name: 'Revoke access grant-one' }))
    .not.toBeInTheDocument()
  expect(container.textContent).toContain('No access renews automatically')
  expect(fixture.calls.find((call) => call.action === 'revoke').body).toEqual({
    sessionId: 'session',
    callId: 'call',
    grantId: 'grant-one',
  })
...
test('uncertain and unattempted history remain explicit without retry buttons', async () => {
  await mount(() => ({
    ...active,
    grants: [{ ...active.grants[0], state: 'renewal-required' }],
    history: [
      {
        id: 'h1',
        operation: 'setProjectItemField',
        outcome: 'uncertain',
        targets: { project: { id: 'P_fixture' } },
      },
  ...
  expect(
    Array.from(container.querySelectorAll('button')).some((button) =>
      /retry|apply|allow|approve/i.test(button.textContent),
    ),
  ).toBe(false)
})
test('lost revoke response does not claim revoked or send another revoke', async () => {
  const fixture = await mount((action) => {
    if (action === 'revoke') throw new Error('lost')
    return active
  })
  await click(page.getByRole('button', { name: 'Revoke access grant-one' }))
...
test.each([
  ...
  ['unrecognized', 'Status unknown — access is not confirmed'],
])('lifecycle %s never invents active authority', async (phase, label) => {
