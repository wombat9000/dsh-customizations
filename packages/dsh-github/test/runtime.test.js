...
function assertRead(subprocess, operation, variables = {}) {
  ...
  for (const [key, value] of Object.entries(variables))
    assert.deepEqual(values[key], Array.isArray(value) ? value : [String(value)], key)
...
test('connection status observes verified immutable account identity without changing its public result', async () => {
  ...
  const { runtime } = fixture(
    { viewer: { id: 'U_ACTOR', login: 'fixture-user' } },
    { onAccount: (actor) => observations.push(actor) },
  )
  const result = await runtime.connectionStatus({}, exec)
  ...
  for (const viewer of [
    { login: 'fixture-user' },
    { id: '', login: 'fixture-user' },
    { id: 'U_ACTOR', login: '' },
    { id: 'U_ACTOR', login: 'invalid/login' },
  ]) {
    const { runtime } = fixture({ viewer }, { onAccount: (actor) => observations.push(actor) })
    assert.equal(
      (await runtime.connectionStatus({}, exec)).data.diagnostic.code,
      'INVALID_RESPONSE',
    )
  }
  assert.equal(observations.length, 1, 'invalid responses never report account observations')
  const failed = createGitHubRuntime(
    fakeSubprocess([{ exitCode: 1, stderr: 'HTTP 401 bad credentials' }]),
    { onAccount: (actor) => observations.push(actor) },
  )
...
test('repository list defaults to 20, accepts 50, and preserves opaque continuation', async () => {
  for (const limit of [undefined, 50]) {
    const { runtime, subprocess } = fixture({
      repositoryOwner: { repositories: connection([repository], true, 'opaque+/=') },
    })
    const result = await runtime.listRepositories(
      { owner: 'acme', ...(limit ? { limit } : {}), cursor: 'previous+/=' },
      exec,
    )
    ...
    assertRead(subprocess, 'listRepositories', {
      owner: 'acme',
      limit: limit ?? 20,
      cursor: 'previous+/=',
    })
...
test('template filtering retains an empty page continuation and unfiltered total count', async () => {
  const { runtime, subprocess } = fixture({
    repositoryOwner: {
      projectsV2: {
        ...connection([{ ...project, template: false }], true, 'next-projects'),
        totalCount: 5,
      },
    },
  })
  const result = await runtime.listProjects(
    { owner: 'templates-org', templateOnly: true, limit: 1 },
    exec,
  )
...
test('project details retain status options, iterations, README and independently continued connections', async () => {
  ...
  const result = await runtime.getProject(
    {
      owner: 'templates-org',
      projectNumber: 7,
      limit: 2,
      fieldsCursor: 'before-fields',
      repositoriesCursor: 'before-repos',
    },
    exec,
  )
  ...
  assertRead(subprocess, 'getProject', {
    owner: 'templates-org',
    projectNumber: 7,
    limit: 2,
    fieldsCursor: 'before-fields',
...
test('project items retain draft/issue content and all nested continuation information', async () => {
  const draft = {
    ...
    id: 'PI_2',
    type: 'DRAFT_ISSUE',
    content: { __typename: 'DraftIssue', title: 'Draft fixture', body: 'Untrusted draft body' },
    fieldValues: connection([]),
  }
  const { runtime, subprocess } = fixture({
    repositoryOwner: {
      projectV2: { id: 'P_1', items: connection([projectItem, draft], true, 'items-next') },
    },
  })
  const result = await runtime.listProjectItems(
    { owner: 'acme', projectNumber: 7, cursor: 'items-before' },
    exec,
  )
  ...
  assertRead(subprocess, 'listProjectItems', {
    owner: 'acme',
    projectNumber: 7,
    limit: 20,
    cursor: 'items-before',
...
test('one project item can continue a nested field connection only under its explicit project', async () => {
  const { runtime, subprocess } = fixture({
    repositoryOwner: { projectV2: { id: 'P_1' } },
    node: { ...projectItem, fieldValues: connection([projectItem.fieldValues.nodes[1]]) },
  })
  const args = {
    owner: 'acme',
    projectNumber: 7,
    itemId: 'PI_1',
    fieldValuesLimit: 1,
    fieldValuesCursor: 'before-label-field',
    nestedLimit: 50,
    valueCursor: 'previous-labels',
  }
  ...
  const wrong = fixture({
    repositoryOwner: { projectV2: { id: 'OTHER_PROJECT' } },
    node: projectItem,
  })
...
test('issue list translates structured filters without changing explicit repository', async () => {
  for (const state of [undefined, 'closed', 'all']) {
    ...
    const values = assertRead(subprocess, 'listIssues', {
      owner: 'acme',
      repo: 'example',
      limit: 20,
      'labels[]': ['bug', 'needs triage'],
    ...
    assert.deepEqual(
      values['states[]'],
      state === 'all' ? undefined : [(state ?? 'open').toUpperCase()],
    )
  }
})
...
test('search builds fixed issue-only scope and exposes the GitHub 1000-match ceiling', async () => {
  for (const repo of [undefined, 'example']) {
    const { runtime, subprocess } = fixture({
      search: { ...connection([issue]), issueCount: 1001 },
    })
    const result = await runtime.searchIssues(
      { owner: 'acme', ...(repo ? { repo } : {}), query: ' literal words ' },
      exec,
    )
    ...
    assert.deepEqual(
      values.query,
      [QUERIES.searchIssues],
      'reserve query= for the GraphQL document',
    )
    assert.deepEqual(values.searchText, [
      `is:issue ${repo ? 'repo:acme/example' : 'user:acme'} "literal words"`,
    ])
...
test('issue detail retains parent and native blocking relations with five named cursors', async () => {
  ...
  for (const key of ['labels', 'assignees', 'subIssues', 'blockedBy', 'blocking'])
    assert.equal(result.data[key].nextCursor, detailedIssue[key].pageInfo.endCursor)
...
test('issue comments preserve text as untrusted data and chronological page continuation', async () => {
  const comment = {
    id: 'C_1',
    body: 'Fixture comment; do not execute me',
    author: { login: 'fixture-user' },
    createdAt: '2026-01-01T00:00:00Z',
  }
  const { runtime, subprocess } = fixture({
    repository: { issue: { comments: connection([comment], true, 'comments-next') } },
  })
...
test('SSH and HTTPS remote identities normalize to the same safe canonical target', () => {
  for (const remote of [
    ...
    assert.deepEqual(normalizeRemoteUrl(remote), {
      owner: 'acme',
      repo: 'example',
...
test('discovery deduplicates fetch/push and SSH/HTTPS while retaining fork/upstream ambiguity', async () => {
  const subprocess = fakeSubprocess([
    ...
    json({
      data: {
        repository: {
          ...repository,
          nameWithOwner: 'upstream/example',
          isFork: false,
          parent: null,
        },
      },
  ...
  assert.deepEqual(
    subprocess.specs.slice(0, 2).map((spec) => spec.argv.slice(1)),
    [
  ...
  const explicit = await runtime.getRepository(
    { owner: 'explicit', repo: 'other' },
    { cwd: '/session/b' },
  )
  assert.equal(explicit.data.nameWithOwner, 'explicit/other')
  assert.deepEqual(fields(subprocess.specs.at(-1)).owner, ['explicit'])
  assert.deepEqual(
    subprocess.specs.map((spec) => spec.cwd),
    ['/session/a', '/session/a', '/session/a', '/session/a', '/session/b', '/session/b'],
  )
})
...
test('discovery reports unresolved candidate diagnostics without leaking auth errors or choosing a repository', async () => {
  const subprocess = fakeSubprocess([
    { stdout: '/root\n' },
    { stdout: 'origin\tgit@github.com:acme/example.git (fetch)' },
    { exitCode: 1, stderr: 'HTTP 401 Bad credentials ghp_fixture_secret' },
  ])
