...
function fixture(
  name,
  { before = snapshot(name), after = before, result = mutationResult(name), lastRun, config } = {},
) {
  const subprocess = fakeSubprocess([
    json({ data: before }),
    json({ data: after }),
    lastRun ?? json({ data: result }),
  ])
...
const expectedInputs = {
  ...
  createIssue: {
    ...
    title: 'New issue',
    body: 'Complete issue specification',
  },
...
test('complete Markdown content, whitespace, quotes and normal token prose survive preparation and mutation', async () => {
  ...
  assert.equal(
    preview.exactPayload.body,
    body,
    'preview includes the complete body, not a truncated sample',
  )
...
test('template preview distinguishes source and destination and declares draft-copy behavior', async () => {
  ...
  for (const value of ['templates', 'destination', 'P_TEMPLATE', 'includeDraftIssues', 'false'])
    assert.ok(prepared.preview.includes(value), value)
})
...
test('invalid or missing explicit write targets and unknown mutation inputs fail before subprocess work', async () => {
  ...
  for (const operation of Object.keys(args).filter((name) => name !== 'copyProject'))
    await assert.rejects(runtime.prepare(operation, {}, exec), { code: 'INVALID_ARGUMENT' })
  for (const input of [
    { ...args.createIssue, owner: '../escape' },
    { ...args.createIssue, repo: 'a/b' },
    { ...args.createIssue, title: '' },
    { ...args.createIssue, title: 'x'.repeat(257) },
    { ...args.createIssue, body: 'x'.repeat(20001) },
    { ...args.createIssue, method: 'DELETE' },
    { ...args.createIssue, query: 'mutation { unsafe }' },
    { ...args.createIssue, host: 'evil.test' },
  ])
    await assert.rejects(runtime.prepare('createIssue', input, exec), { code: 'INVALID_ARGUMENT' })
  for (const input of [
    { ...args.createProject, templateOwner: 'templates' },
    { ...args.createProject, templateNumber: 9 },
    { ...args.createProject, includeDraftIssues: true },
  ])
    await assert.rejects(runtime.prepare('createProject', input, exec), {
      code: 'INVALID_ARGUMENT',
    })
  await assert.rejects(
    runtime.prepare('updateProject', { owner: 'destination', projectNumber: 7 }, exec),
    { code: 'INVALID_ARGUMENT' },
  )
  await assert.rejects(runtime.prepare('createIssue', args.createIssue, { cwd: exec.cwd }), {
  ...
  for (const [fieldId, value] of [
    ...
    await t.test(`${fieldId}:${Object.values(value)[0]}`, async () => {
      ...
      const prepared = await runtime.prepare(
        'setProjectItemField',
        { ...args.setProjectItemField, fieldId, value },
        exec,
      )
      const preview = JSON.parse(prepared.preview.match(/```json\n([\s\S]*)\n```$/)[1])
...
test('invalid field dates, shapes and non-finite numbers fail before target reads', async () => {
  ...
  for (const value of [
    ...
    { text: 'x', number: 1 },
    { clear: true },
    { singleSelectOptionId: '../other' },
  ]) {
    await assert.rejects(
      runtime.prepare('setProjectItemField', { ...args.setProjectItemField, value }, exec),
      { code: 'INVALID_ARGUMENT' },
    )
  }
  assert.equal(subprocess.specs.length, 0)
...
test('field ownership, type, option membership and archival state fail closed', async (t) => {
  const cases = [
    ...
    { label: 'wrong type', patchArgs: { value: { text: 'Ready' } }, code: 'INVALID_ARGUMENT' },
    {
      ...
      patchArgs: { value: { singleSelectOptionId: 'OPT_FOREIGN' } },
      code: 'INVALID_ARGUMENT',
    },
    {
      label: 'foreign iteration',
      patchArgs: { fieldId: 'F_ITERATION', value: { iterationId: 'ITER_FOREIGN' } },
      code: 'INVALID_ARGUMENT',
    },
    {
      label: 'foreign item project',
      alter: (data) => {
        data.node.project.id = 'P_OTHER'
      },
      code: 'NOT_FOUND',
    },
    {
      label: 'archived item',
      alter: (data) => {
        data.node.isArchived = true
      },
      code: 'PERMISSION_DENIED',
    },
    {
      label: 'issue-owned field',
      alter: (data) => {
        data.repositoryOwner.projectV2.fields.nodes[3].isIssueField = true
      },
      code: 'INVALID_ARGUMENT',
    },
    {
      ...
      alter: (data) => {
        data.repositoryOwner.projectV2.fields.nodes[3].dataType = 'ASSIGNEES'
      },
      code: 'INVALID_ARGUMENT',
    },
  ]
  for (const row of cases)
    await t.test(row.label, async () => {
      ...
      await assert.rejects(
        runtime.prepare(
          'setProjectItemField',
          { ...args.setProjectItemField, ...row.patchArgs },
          exec,
        ),
        { code: row.code },
      )
...
test('duplicate project memberships, repository links and dependencies are rejected without mutation', async (t) => {
  for (const name of ['linkProjectRepository', 'addProjectItem', 'addIssueDependency'])
    await t.test(name, async () => {
      const before = snapshot(name)
      if (name === 'linkProjectRepository')
        before.repositoryOwner.projectV2.repositories.nodes.push(repository)
      if (name === 'addProjectItem')
        before.repository.issue.projectItems.nodes.push({
          id: 'PI_EXISTING',
          isArchived: true,
  ...
  await assert.rejects(
    fixture('addIssueDependency', { before }).runtime.prepare(
      'addIssueDependency',
      args.addIssueDependency,
      exec,
    ),
    { code: 'INVALID_ARGUMENT' },
  )
...
test('incomplete relevant collections fail closed instead of assuming absent duplicates or fields', async (t) => {
  for (const name of [
    ...
    await t.test(name, async () => {
      const before = snapshot(name)
      const collection =
        name === 'linkProjectRepository'
          ? before.repositoryOwner.projectV2.repositories
          : name === 'addProjectItem'
            ? before.repository.issue.projectItems
            : name === 'setProjectItemField'
              ? before.repositoryOwner.projectV2.fields
              : before.repository.issue.blockedBy
  ...
  const cases = [
    {
      ...
      change: (data) => {
        data.viewer.id = 'U_OTHER'
        data.viewer.login = 'other-user'
      },
    },
    {
      name: 'createIssue',
      label: 'repository access lost',
      change: (data) => {
        data.repository.viewerCanCreateIssues = false
      },
    },
    {
      name: 'createIssue',
      label: 'repository identity changed',
      change: (data) => {
        data.repository.id = 'R_OTHER'
      },
    },
    {
      name: 'updateProject',
      label: 'remote update',
      change: (data) => {
        data.repositoryOwner.projectV2.readme = 'Concurrent remote content'
      },
    },
    {
      name: 'updateProject',
      label: 'project permission lost',
      change: (data) => {
        data.repositoryOwner.projectV2.viewerCanUpdate = false
      },
    },
    {
      name: 'setProjectItemField',
      label: 'field renamed',
      change: (data) => {
        data.repositoryOwner.projectV2.fields.nodes[3].name = 'Renamed status'
      },
    },
    {
      name: 'setProjectItemField',
      ...
      change: (data) => {
        data.node.fieldValues.nodes[0].optionId = 'OPT_READY'
      },
    },
    {
      name: 'addProjectItem',
      label: 'membership appeared',
      change: (data) => {
        data.repository.issue.projectItems.nodes.push({
          id: 'PI_LATE',
          project: { id: project.id },
        })
      },
    ...
    {
      name: 'addIssueDependency',
      label: 'dependency appeared',
      change: (data) => {
        data.repository.issue.blockedBy.nodes.push(blocker)
      },
    },
  ]
  for (const row of cases)
    await t.test(row.label, async () => {
      const before = snapshot(row.name),
...
test('unknown permission, invalid authentication and unavailable CLI fail before mutation with sanitized errors', async () => {
  for (const [before, name, code] of [
    ...
    [
      { ...snapshot('createIssue'), repository: { ...repository, viewerCanCreateIssues: null } },
      'createIssue',
      'PERMISSION_DENIED',
    ],
    [
  ...
  for (const [stderr, code] of [
    ...
    ['HTTP 503 network error', 'READ_FAILED'],
  ]) {
    const subprocess = fakeSubprocess([
      { exitCode: 1, stderr: `${stderr} ghp_SYNTHETIC_SECRET Authorization: Bearer private-value` },
    ])
    await assert.rejects(
      createGitHubWriteRuntime(subprocess).prepare('createIssue', args.createIssue, exec),
      (error) => {
...
test('credential-looking user or remote content never reaches previews or mutation dispatch', async () => {
  for (const body of [
    ...
    'github_pat_SYNTHETIC_SECRET',
    'Authorization: Bearer private-value',
    'https://user:private-value@github.com/repo',
    '-----BEGIN PRIVATE KEY-----\nprivate-value',
    '\u202ehidden text',
  ]) {
    const subprocess = fakeSubprocess([])
  ...
  await assert.rejects(
    fixture('updateProject', { before }).runtime.prepare('updateProject', args.updateProject, exec),
    { code: 'UNSAFE_CONTENT' },
  )
})
...
test('cancellation before dispatch fails closed while cancellation after dispatch reports uncertainty', async () => {
  ...
  cancelled.abort()
  await assert.rejects(before.runtime.execute(prepared, { ...exec, signal: cancelled.signal }), {
    code: 'CANCELLED',
  })
  ...
  const result = await after.runtime.execute(
    second,
