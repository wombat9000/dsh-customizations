...
export type SettingsDraft = Omit<Settings, 'inactivityMinutes'> & {
  inactivityMinutes: number | string
}
...
function SettingsRow({ label, children }: SettingsRowProps) {
  return (
    <label className="dsh-session-recap-settings__row">
      {label}
      {children}
    </label>
  )
    ...
    >
      ...
      >
        <span className="dsh-session-recap-settings__heading">
          ...
          <span className="dsh-session-recap-settings__description">
            Show a short recap above the composer.
          </span>
        </span>
        <svg
      ...
      {open ? (
        <div className="dsh-session-recap-settings__body">
          <p style={{ margin: 0 }}>
            Recaps use the selected model and never change the transcript.
          </p>
          {draft ? (
            <>
              ...
              <small id={`${ID}-jev-privacy`} className="dsh-session-recap-settings__hint">
                Optional; requires the Jev plugin. Sends the same bounded history through OpenRouter
                  ...
                  {providers.map((provider) => (
                    <optgroup key={provider.id} label={provider.name || provider.id}>
                      {provider.models.map((model) => (
                        <option key={model.id} value={JSON.stringify([provider.id, model.id])}>
                          {model.name || model.id}
                        </option>
                      ))}
                    </optgroup>
              ...
              <small className="dsh-session-recap-settings__hint">
                The catalog is advisory. You can enter an exact provider and model route. Saving
                validates the route without generating a recap.
              </small>
