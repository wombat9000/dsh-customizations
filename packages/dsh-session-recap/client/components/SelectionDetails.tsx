...
export interface SelectionDetailsProps {
  diagnostics?: SelectionDiagnostics | null | undefined
}
...
export function SelectionDetailsView({
  diagnostics,
  json,
  copyStatus,
  onCopy,
}: SelectionDetailsViewProps) {
  ...
  return (
    <details className="dsh-session-recap-card__diagnostics">
      ...
      <p>
        From this recap request. Opening or copying these details makes no model call. No
        conversation text or credentials are included.
      </p>
      ...
      {diagnostics.status === 'unavailable' ? (
        ...
        <p>
          Table values are rounded to three decimals. JSON keeps full precision. Selected categories
          go to the writer, which can still omit a card.
        </p>
      )}
      ...
      <details>
        ...
        <p>
          Question wording and rubric levels for this request, with full-precision values when
          evaluation succeeded:
        </p>
        <pre tabIndex={0} aria-label="Selection diagnostics JSON">
          {json}
        </pre>
      </details>
    </details>
