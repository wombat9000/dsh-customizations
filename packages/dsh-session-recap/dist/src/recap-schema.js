...
export function parseRecap(text) {
    if (typeof text !== 'string')
        throw invalidRecap('response-type');
    if (text.length > LIMITS.outputChars)
        throw invalidRecap('output-limit', null, text.length);
    let value;
    try {
    ...
    const bullets = value.bullets.map((bullet, index) => {
        if (typeof bullet !== 'string')
            throw invalidRecap('bullet-type', index, count);
        const normalized = bullet.replace(/\s+/gu, ' ').trim();
        if (!normalized)
            throw invalidRecap('empty-bullet', index, count);
        return normalized;
    });
    ...
    if (Object.hasOwn(value, 'headline')) {
        if (typeof value.headline !== 'string')
            throw invalidRecap('headline-type');
        headline = value.headline.replace(/\s+/gu, ' ').trim();
        if (!headline)
            throw invalidRecap('empty-headline');
    }
    ...
    if (oversized !== -1)
        throw invalidRecap('bullet-length', oversized, bullets[oversized].length);
...
export function parseCards(text, labels) {
    if (typeof text !== 'string')
        throw invalidRecap('response-type');
    if (text.length > LIMITS.outputChars)
        throw invalidRecap('output-limit');
    let value;
    try {
        value = JSON.parse(text.trim());
    }
    ...
    if (labels.some((label) => !Object.hasOwn(values, label)))
        throw invalidRecap('shape');
    ...
    const cards = labels.flatMap((label) => {
        ...
        if (!text)
            throw invalidRecap('card-type');
        return [Object.freeze({ label, text })];
    });
    if (!cards.length)
        throw invalidRecap('card-count');
    if (headline.length > CARD_LIMITS.headline)
        throw invalidRecap('headline-length');
    if (cards.some((card) => card.text.length > CARD_LIMITS.text))
        throw invalidRecap('card-length');
