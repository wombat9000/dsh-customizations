...
export function evaluateCardSelection(result, status = 'evaluated') {
    ...
    const categories = rows.map((row) => {
        ...
        const usefulness = typeof row.score === 'number' &&
            Number.isFinite(row.score) &&
            row.score >= 0 &&
            row.score <= 3
            ? row.score
        ...
        if (status === 'evaluated' && !selected) {
            ...
            if (support !== null && support < CARD_THRESHOLDS.support)
                reasons.push('support');
            if (usefulness !== null && usefulness < CARD_THRESHOLDS.usefulness)
                reasons.push('usefulness');
            if (confidence !== null && confidence < CARD_THRESHOLDS.confidence)
                reasons.push('confidence');
            if (!reasons.length)
                reasons.push('ranked-out');
        }
