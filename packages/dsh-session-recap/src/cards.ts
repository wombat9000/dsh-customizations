...
const meanings = {
  direction:
    'The current goal or exploratory direction, including user corrections. A proposal can describe exploration but is not an agreed decision. Exclude superseded directions.',
  decision:
    'An explicitly agreed choice or constraint. Require user agreement or an explicit user decision; an assistant proposal, silence, or superseded agreement is not a decision. Honor later user corrections.',
  insight:
    'A substantive explanation or finding actually stated in the conversation that changes understanding. Distinguish tentative findings from established facts; exclude superseded or corrected claims.',
  question:
    'A meaningful question or uncertainty that remains unresolved. Exclude answered questions, rhetorical questions, and uncertainty inferred from missing history.',
  next_step:
    'An explicitly stated, still-pending intended action. Distinguish proposals from commitments and never turn an assistant suggestion into user agreement. Exclude completed or superseded steps.',
  paused:
    'The concrete discussion point where the visible conversation stopped. Describe its actual unresolved or exploratory state, not an invented next action or inferred completion.',
}
const safety =
...
function questionsForCategory(label: CardLabel): [string, SelectionQuestion][] {
  ...
  return [
    [`support_${label}`, support],
    [`usefulness_${label}`, usefulness],
  ]
}

export const CARD_QUESTIONS = Object.freeze(
  Object.fromEntries(CARD_LABELS.flatMap(questionsForCategory)),
)
export const CARD_THRESHOLDS = Object.freeze({
  support: 0.75,
  usefulness: 2,
  confidence: 0.3,
  maxCards: 3,
...
function cardRows(result: unknown): CardRow[] {
  ...
  return CARD_LABELS.map((label, index) => {
    ...
    return {
      ...
      probabilities:
        own(usefulness, 'type') === 'score' ? own(usefulness, 'probabilities') : undefined,
    }
  })
}
