...
test('history excludes tools, reasoning, attachments and injected instructions', () => {
  const messages = [
    message('secret', 'system', 'plugin'),
    message('tool secret', 'user', 'tool'),
    message('injected', 'user', 'plugin'),
    {
      ...message('visible'),
      content: [
        { type: 'reasoning', text: 'private' },
        { type: 'image', attachment: 'secret' },
...
test('history bounds transmitted text and selected messages', () => {
  const rows = boundedHistory(
    Array.from({ length: 1000 }, (_, i) => message(`${i} ${'😀'.repeat(30000)}`)),
  )
...
test('history redistributes short-message allowances and keeps fitting reports whole', () => {
  ...
  assert.deepEqual(
    ...
    messages.map((item) => item.content[0].text),
  )
...
test('shortened history keeps both ends across text blocks and marks the gap', () => {
  const input = message('')
  input.content = [
    { type: 'text', text: 'Opening decision. ' + 'x'.repeat(100000) },
    { type: 'tool-call', arguments: 'PRIVATE' },
    { type: 'text', text: 'y'.repeat(100000) + ' Final correction.' },
  ]
...
test('serialized budgets include escaping, Unicode, omission metadata, and block caps', () => {
  for (const text of ['😀漢字', '\\"\n\t\\', 'é', 'word. ']) {
    const messages = Array.from({ length: 93 }, (_, i) =>
      message(`Start ${i} ` + text.repeat(3000) + ` End ${i}`),
    )
    const rows = boundedHistory(messages)
  ...
  input.content = Array.from({ length: LIMITS.blocks + 1 }, (_, i) => ({
    type: 'text',
    text: i === LIMITS.blocks ? 'EXCLUDED' : `Block ${i}`,
  }))
  const [row] = boundedHistory([input])
  assert.equal(row.truncated, true)
...
test('history retains opening, middle, and recent context with explicit gaps', () => {
  const rows = boundedHistory(
    Array.from({ length: 1000 }, (_, i) =>
      message(`${i} discussion`, i % 2 ? 'assistant' : 'user', i % 2 ? 'model' : 'user'),
    ),
  )
  ...
  assert.ok(rows.some((row) => row.omittedBefore > 0))
  assert.deepEqual(
    rows.map((row) => Number.parseInt(row.text)),
    rows.map((row) => Number.parseInt(row.text)).sort((a, b) => a - b),
  )
})
test('long reports do not crowd out user intent or corrections', () => {
  const rows = boundedHistory([
    message('Make this a memory refresh.'),
    message('Report '.repeat(20000), 'assistant', 'model'),
    message('No status report. Keep it short.'),
  ])
...
test('injected and tool traffic cannot displace the visible conversation', () => {
  const rows = boundedHistory([
    ...
    ...Array.from({ length: 500 }, () => message('Excluded data', 'user', 'tool')),
    message('Still exploring.', 'assistant', 'model'),
  ])
  assert.deepEqual(rows, [
    { role: 'user', text: 'Original intent' },
    { role: 'assistant', text: 'Still exploring.' },
  ])
})
test('short exploratory conversations retain all visible text without gaps', () => {
  assert.deepEqual(
    boundedHistory([
      message('Could we explore two approaches?'),
      message('Both remain open.', 'assistant', 'model'),
    ]),
    [
      { role: 'user', text: 'Could we explore two approaches?' },
