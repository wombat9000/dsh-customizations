...
export async function mountSlot(
  name,
  {
    blank = false,
    narrow = false,
    ...
    selection,
    recapHeadline,
    recapTopic = 'Add reliable screenshot coverage for Session recap.',
  } = {},
) {
  ...
  const rpc = {
    call: vi.fn(async (channel, method, payload) => {
      ...
      switch (method) {
        ...
        case 'activity':
          return { ok: true, value: { ready: true, running: false, latestActivity: 1 } }
        case 'models':
          return {
            ok: true,
            value: {
        ...
        case 'recap':
          if (deferRecap) await new Promise((resolve) => pending.set(payload.sessionId, resolve))
          if (recapError)
            return {
              ok: false,
              error: {
                code: 'fixture-error',
                details: {},
                message: 'The fixture provider is unavailable. Try again.',
              },
            }
          return {
            ok: true,
            value: {
              ...
              recap: {
                ...
                ...(recapCards === undefined
                  ? {
                      bullets: [
                        typeof recapTopic === 'function'
                          ? recapTopic(payload.sessionId)
                          : recapTopic,
  ...
  const chat = () => ({
    timeline: {
      turnOrder: blank ? [] : [1, turn],
      turns: new Map([
        [1, { status: 'closed' }],
        [turn, { status: closed ? 'closed' : 'running' }],
      ]),
    },
    ...
    nodes: {
      get: (key) => ({
        kind: 'turn-tail',
        data: {
          closing: { status: 'settled', finalNode: { messageId: `message-${key.slice(5)}` } },
        },
      }),
    },
  })
  const render = () =>
    act(async () => {
      ...
      const hooks = {
        ...
        useChat: (select) => select(snapshot),
        useConversation: (select) =>
          select({ timeline: snapshot.timeline, activeTargets: new Set(loaded ? ['chat'] : []) }),
      }
      root.render(
        name === 'conversation.input.dock'
          ? h(
                ...
                blank
                  ...
                  : h(
                      ...
                      ...[1, turn].map((id) =>
                        h(
                          'section',
                          { key: id, 'data-testid': `assistant-${id}` },
                          h('p', null, `Assistant message ${id}.`),
                          h(
                            'div',
                            { 'data-testid': `assistant-actions-${id}` },
                            h('button', { type: 'button' }, 'Copy'),
                            h(action.Component, {
                              ...action.options.inject(sessionId),
                              ...hooks,
              ...
              h(
                ...
                h(Component, { ...options.inject(sessionId), ...hooks, session }),
              ),
  ...
  return {
    rpc,
    async sent(id = sessionId) {
      await act(async () => events.get('api-session/activity')(id, Date.now()))
    },
    async switchSession(id, isBlank = false) {
      sessionId = id
      blank = isBlank
      await render()
    },
