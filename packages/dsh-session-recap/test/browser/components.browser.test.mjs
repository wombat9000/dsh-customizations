...
const diagnostics = () => ({
  version: 1, model: '<img src=x onerror=alert(1)>', questionSetVersion: 'v1', status: 'complete',
  thresholds: { support: .6, usefulness: 1.5, confidence: .3, maxCards: 3 },
  categories: [
    { label: 'direction', support: .912345, usefulness: 2.4567, confidence: .7, selected: true, reasons: [] },
    { label: '<script>bad()</script>', support: null, usefulness: null, confidence: null, selected: false, reasons: ['support', 'unknown'] },
  ],
})
...
test('action button props control labels, loading, unread, open and error without disabling callbacks', async () => {
  ...
  for (const [state, label] of [
    [{}, 'Generate recap'], [{ recap: {}, unread: true }, 'Show recap'],
    [{ recap: {}, open: true }, 'Hide recap'], [{ error: '<b>failure</b>' }, 'Retry recap'],
    ...
    expect(button().title).toBe(state.busy ? 'Generating recap…' : state.error ? `Retry recap: ${state.error}` : label)
...
test('panel renders escaped bullets, headline, errors, loading and only known fallback captions', async () => {
  ...
  await render(RecapPanel, { busy: true, error: '<b>failure</b>', recap, selection: { mode: 'standard', reason: 'unavailable' } })
  ...
  for (const [selection, caption] of [
    ...
    expect(container.querySelector('.dsh-session-recap-card__caption')?.textContent ?? null).toBe(caption)
  }
...
test('panel filters visual cards, fixes titles and icons, clips text and hides bullet fallback', async () => {
  await render(RecapPanel, { recap: { bullets: ['Hidden fallback'], cards: [
    { label: 'alien', text: 'Unknown' }, { label: 'direction', text: '<img>'.repeat(50) },
    { label: 'direction', text: 'Duplicate' }, { label: 'decision', text: 'Decision body' },
    { label: 'insight', text: 'Insight body' }, { label: 'paused', text: 'Fourth' },
  ] } })
  expect([...container.querySelectorAll('li')].map(node => node.dataset.recapCard)).toEqual(['direction', 'decision', 'insight'])
...
test('all six card labels use fixed icons and titles; invalid-only cards fall back to bullets', async () => {
  const titles = {
    direction: 'Direction', decision: 'Decision', insight: 'Key insight',
    question: 'Open question', next_step: 'Next step', paused: 'Where we paused',
  }
  for (const [label, title] of Object.entries(titles)) {
    await render(RecapPanel, { recap: { cards: [{ label, text: 'Body', title: 'MODEL TITLE', style: { color: 'red' }, path: 'MODEL PATH', accent: 'red' }] } })
  ...
  await render(RecapPanel, { recap: { cards: [
    { label: '__proto__', text: 'Prototype' }, { label: 'alien', text: 'Unknown' },
    { label: 'direction', text: ' ' }, { label: 'decision', text: 42 }, null,
  ], bullets: ['Fallback'] } })
...
test('selection view invokes supplied copy callback and shows supplied status', async () => {
  const onCopy = vi.fn()
  await render(SelectionDetailsView, { diagnostics: diagnostics(), json: '<script>json</script>', copyStatus: 'Supplied status', onCopy })
...
test('selection details copies only its snapshot, reports failure safely and resets status on new diagnostics', async () => {
  ...
  expect(container.textContent).toContain('Jev evaluation was unavailable; no category scores were retained.')
  copied.mockRejectedValue(Error('PRIVATE ERROR'))
  await click(page.getByRole('button', { name: 'Copy diagnostics JSON' }))
  expect(container.textContent).toContain('Clipboard unavailable. Expand the JSON and copy it manually.')
  expect(container.textContent).not.toContain('PRIVATE ERROR')
  await render(SelectionDetails, { diagnostics: { version: 2 } })
  expect(container.children).toHaveLength(0)
})

const draft = { autoRecap: false, useJev: false, inactivityMinutes: 30, provider: 'route', model: 'model' }
test('settings form is prop driven and delivers control callbacks without RPC', async () => {
  const onToggle = vi.fn(), onChange = vi.fn(), onChooseModel = vi.fn(), onSave = vi.fn()
  const props = { open: true, draft, providers: [{ id: 'p', name: '<provider>', models: [{ id: 'm', name: '<model>' }] }], onToggle, onChange, onChooseModel, onSave }
  ...
  expect([...container.querySelectorAll('input, select')].every(input => input.disabled)).toBe(true)
...
function sessionProps(state = {}) {
  ...
  const chat = { timeline: { turnOrder: [1], turns: new Map([[1, { status: 'closed' }]]) }, locations: { getTurn: () => ['tail'] }, nodes: new Map([['tail', { kind: 'turn-tail', data: { closing: { status: 'settled', finalNode: { messageId: 'closing' } } } }]]) }
  const cleanup = vi.fn()
  const unsubscribe = vi.fn()
  const controller = { getSnapshot: () => state, subscribe: vi.fn(() => unsubscribe), click: vi.fn(), observeSession: vi.fn(), mount: vi.fn(() => cleanup) }
  return { session, chat, cleanup, unsubscribe, props: { sessionId: 'session', messageId: 'closing', controller, useSession: select => select(session), useChat: select => select(chat), useConversation: select => select({ activeTargets: new Set(['chat']) }) } }
}
...
test('source action container guards blank, unavailable and non-closing seats and disposes styles', async () => {
  ...
  expect(props.controller.subscribe).toHaveBeenLastCalledWith('second-session', expect.any(Function))
  await render(() => null)
...
test('source dock observes and mounts while hidden, gates visibility, and cleans up', async () => {
  ...
  await render(RecapCard, props)
  expect(container.children).toHaveLength(0)
  expect(props.controller.observeSession).toHaveBeenCalledWith('session', { ready: true, running: false, latestTurn: 1 })
  ...
  await render(() => null)
  expect(unsubscribe).toHaveBeenCalledOnce()
  expect(document.querySelector('style[data-plugin-css="wombat9000-session-recap/recap"]')).toBeNull()
})

test('source settings container saves original payload and disposes its stylesheet', async () => {
  const rpc = { call: vi.fn(async (_, method, payload) => method === 'models'
  ...
  expect(document.querySelector('style[data-plugin-css="wombat9000-session-recap/settings"]')).not.toBeNull()
  await render(() => null)
  expect(document.querySelector('style[data-plugin-css="wombat9000-session-recap/settings"]')).toBeNull()
})
