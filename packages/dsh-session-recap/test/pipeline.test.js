...
test('shared credentials -> Jev -> constrained writer works through real Cordis and falls back on removal', async t => {
  ...
  ctx.provide('settings', {
    ...
    async update(ns, value) { sections.set(ns, { ...sections.get(ns), ...value }) },
  })
  const credentialReads = []
  ctx.provide('credentials', {
    async readRecord(key) { credentialReads.push(key); return { kind: 'api-key', key: 'test-only-shared-key' } },
    async describeRecord() { return { configured: true, kind: 'api-key', writable: true } },
    async describe() { return { configured: false, writable: true } },
  })
  const routes = new Map()
  ctx.provide('connection', { rpc: { handle(channel, handler) { routes.set(channel, handler); return () => routes.delete(channel) } } })
  ctx.provide('webServer', {})
  ctx.provide('sessions', { get: () => ({ seq: 1, deriveMessages: () => [] }) })
  const session = { seq: 1, deriveMessages: () => [{ role: 'user', source: { kind: 'user' }, content: [{ type: 'text', text: 'Use fixed labels. How should cards be arranged?' }] }] }
  ...
  const fetchMock = t.mock.method(globalThis, 'fetch', async (url, options) => {
    ...
    const answers = Object.fromEntries(Object.entries(body.questions).map(([id, q]) => {
      ...
      return [id, q.type === 'noul' ? { type: 'noul', noul: useful ? .95 : .1 }
        : { type: 'score', score: useful ? 3 : 0, confidence: .95,
          probabilities: Object.fromEntries(q.criteria.map((_, i) => [i, i === (useful ? 3 : 0) ? 1 : 0])),
          legend: Object.fromEntries(q.criteria.map((v, i) => [i, v])) }]
    }))
    return new Response(JSON.stringify({ model: body.model, answers }))
  })
  ctx.provide('llm', { async prepareCall(config) {
    return { config, async *stream(request) {
      calls.push('writer')
      const value = request.system.includes('cards is an object')
        ? { headline: 'Visual recap cards', cards: { decision: 'Use fixed labels.', question: 'How should the cards be arranged?' } }
        : { headline: 'Visual recap cards', bullets: ['Use fixed labels; layout remains open.'] }
      yield { type: 'text-delta', text: JSON.stringify(value) }
      yield { type: 'finish', reason: { kind: 'stop' } }
    } }
  } })
  const routerMount = ctx.plugin(openrouter); await routerMount.await()
  const jevMount = ctx.plugin(jev); await jevMount.await()
  const recapMount = ctx.plugin(recap); await recapMount.await()
  ...
  assert.deepEqual(first.value.recap.cards.map(card => card.label), ['decision', 'question'])
  ...
  assert.deepEqual(fallback.value.selection, { mode: 'standard', reason: 'unavailable' })
  assert.deepEqual(calls, ['jev', 'writer', 'writer'])
  assert.equal(fetchMock.mock.callCount(), 1)
  await recapMount.dispose(); await routerMount.dispose()
