...
const answers = (labels = ['direction']) => ({
  answers: Object.fromEntries(
    CARD_LABELS.flatMap((label) => [
      [`support_${label}`, { type: 'noul', noul: labels.includes(label) ? 0.9 : 0.1 }],
  ...
  const session = {
    ...
    deriveMessages: () =>
      Array.from({ length: 80 }, (_, i) => ({
        role: 'user',
        source: { kind: 'user' },
        content: [{ type: 'text', text: `${i} ${'context '.repeat(300)}` }],
      })),
  }
...
test('Jev runs first on the identical bounded excerpt; cards cache and in-flight requests deduplicate', async () => {
  ...
  assert.deepEqual(a.recap, {
    headline: draft.headline,
    cards: [{ label: 'direction', text: draft.cards.direction }],
  })
  assert.deepEqual(f.order, ['jev', 'prepare', 'write'])
  assert.deepEqual(f.evaluations[0].state, {
    conversation: boundedHistory(f.session.deriveMessages()),
  })
  assert.deepEqual(
    JSON.parse(f.calls[0].messages[0].content[0].text),
    f.evaluations[0].state.conversation,
  )
...
test('card length repair uses selected schema once; malformed shape never repairs', async () => {
  for (const value of [
    { ...draft, headline: 'x'.repeat(121) },
    { ...draft, cards: { direction: 'x'.repeat(181) } },
  ]) {
    const f = fixture()
    f.state.output = (n) => (n === 1 ? value : draft)
  ...
  const f = fixture()
  f.state.output = () => ({ ...draft, cards: { direction: 'x'.repeat(181), unknown: 1 } })
  await assert.rejects(f.runtime.recap({ sessionId: 's' }), { reason: 'shape' })
  assert.equal(f.calls.length, 1)
  const g = fixture()
  g.state.output = () => ({ ...draft, cards: { direction: 'x'.repeat(181) } })
...
test('disabled, absent, getter errors, failed and no-label Jev fall back; enabled fallback never caches and recovers', async () => {
  for (const mode of ['disabled', 'absent', 'getter', 'failure', 'no-labels']) {
    const f = fixture()
    f.state.output = () => ({ bullets: ['Legacy recap.'] })
    ...
    assert.deepEqual(selection, {
      mode: 'standard',
      ...(mode === 'disabled' ? {} : { reason: mode === 'no-labels' ? mode : 'unavailable' }),
    })
    assert.equal(
      diagnostics?.status,
      mode === 'disabled' ? undefined : mode === 'no-labels' ? 'evaluated' : 'unavailable',
    )
    assert.equal(f.calls.length, 1)
...
test('changes after draft prevent repair and final publication', async () => {
  for (const repair of [false, true]) {
    ...
    f.state.output = () => {
      f.setModel('typesafe/jev-2')
      return repair ? { ...draft, headline: 'x'.repeat(121) } : draft
    }
