...
test('real backend RPC owns registration, calls, settings persistence, disposal and remount', { timeout: 15000 }, async t => {
  ...
  const saved = success(await host.rpc('configure', { ...route, inactivityMinutes: 91, autoRecap: false }))
  ...
  assert.equal(fresh.llm.calls.length, 0, 'saving and restoring settings makes no generation call')
})

test('real persisted session history restores, recaps, rejects running turns and invalidates cache', { timeout: 15000 }, async t => {
  const directory = await state(t)
  ...
  assert.deepEqual(success(await host.rpc('activity', { sessionId: session.id })), {
    ready: true, running: false, latestActivity: originalEvents.at(-1).time,
  })
...
test('real session and settings changes invalidate pending recaps; disposal aborts and remount recovers', { timeout: 15000 }, async t => {
  const directory = await state(t)
  const llm = model()
  ...
  userMessage(session)
  for (const change of [
    async () => { userMessage(session, 'The plan changed.'); await host.ctx.sessionPersistence.flush() },
    async () => { success(await host.rpc('configure', { inactivityMinutes: 92 })) },
  ]) {
