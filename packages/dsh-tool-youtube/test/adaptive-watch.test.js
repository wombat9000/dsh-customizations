...
test('distinguishes live, upcoming, and ended-live metadata', () => {
  const live = parseYoutubeVideoMetadata(
    htmlFor({
  ...
  const upcoming = parseYoutubeVideoMetadata(
    htmlFor({
  ...
  const ended = parseYoutubeVideoMetadata(
    htmlFor({
      videoDetails: { isLiveContent: true, lengthSeconds: '3600' },
      microformat: {
        playerMicroformatRenderer: {
          liveBroadcastDetails: { endTimestamp: '2025-01-01T01:00:00Z' },
        },
      },
    }),
    URL,
...
test('selects direct, low, agentic, and chunked strategies from explicit capabilities', () => {
  ...
  const chunked = planAdaptiveWatch(
    vod(3600),
    { intent: 'exhaustive' },
    {
      capabilities: { clipping: true },
      maximumCoreSeconds: 900,
    },
  )
...
test('normalizes and verifies known-duration evidence timestamps', () => {
  ...
  assert.throws(
    () =>
      normalizeWatchEvidence(
        { timestamp: '2:01', description: 'Impossible', modality: 'visual' },
        120,
      ),
...
test('offsets chunk-local evidence, applies core ownership, and deduplicates boundary drift', () => {
    ...
    [
      {
        ...
        evidence: [
          {
            start_seconds: 599,
            description: 'Save button turns green',
            modality: 'visual',
            basis: 'observation',
          },
      ...
      },
      {
        ...
        evidence: [
          {
            start_seconds: 15,
            description: 'The save button turns green',
            modality: 'visual',
            basis: 'observation',
          },
  ...
  assert.deepEqual(
    merged.map((item) => item.startSeconds),
    [600, 605],
  )
...
test('calculates coverage from successful non-overlap cores and reports gaps', () => {
  ...
  const complete = calculateWatchCoverage(
    1800,
    chunks,
