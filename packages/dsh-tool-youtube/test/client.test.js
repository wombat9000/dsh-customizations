...
test('client bundle registers the expected module and Settings section', async () => {
  ...
  assert.deepEqual(
    toolviews.map((entry) => entry.options.key),
...
test('Gemini card uses status-only describe and does not retain returned secrets', async () => {
  ...
  const credentialUpdate = react.updates[0].find(
...
test('Settings describe failures remain visible instead of escaping the effect', async () => {
  for (const describe of [
    ...
    assert.ok(
      react.updates[3].some((value) => String(value).includes('Credential service unavailable')),
    )
...
test('shared card renders honest accessible transcript progress', async () => {
  ...
  const tree = exports.YoutubeToolCard({
    ...
    block: {
      ...
      argsRaw: JSON.stringify({ url: 'https://youtu.be/dQw4w9WgXcQ' }),
    },
    rpc: {},
  })
  ...
  const [bar] = findElements(
    tree,
    (element) => element.props?.['aria-label'] === 'Transcript progress',
  )
  assert.equal(bar.props['aria-valuenow'], 13)
  const intervals = findElements(
    tree,
    (element) => element.props?.['data-chunk-status'] !== undefined,
  )
  assert.deepEqual(
    intervals.map((element) => element.props['data-chunk-status']),
    ['complete', 'fallback', 'pending'],
  )
  assert.ok(
    intervals.every(
      (element) =>
        element.props.tabIndex === undefined && element.props['aria-hidden'] === undefined,
    ),
  )
})
...
test('shared card uses disclosure and indeterminate or degraded progress honestly', async () => {
  ...
  const collapsed = (await loadClient(collapsedReact)).exports.YoutubeToolCard({
    toolName: 'youtube_transcript',
    block,
    callId: 'call',
    rpc: {},
  ...
  assert.equal(
    findElements(collapsed, (element) => element.props?.role === 'progressbar')[0].props[
      'aria-valuenow'
    ],
    undefined,
  )
  ...
  const degraded = (await loadClient(degradedReact)).exports.YoutubeToolCard({
    toolName: 'youtube_transcript',
    block,
    callId: 'call',
    rpc: {},
...
test('transcript card polls the existing progress RPC and leads with final outcome', async () => {
  ...
  assert.deepEqual(plain(react.updates[1][0]), {
    phase: 'complete',
    totalChunks: 1,
    completedChunks: 1,
  })
  ...
  const final = (await loadClient(finalReact)).exports.YoutubeToolCard({
    ...
    block: {
      ...
      call: {
        name: 'youtube_transcript',
        argsRaw: JSON.stringify({ url: 'https://www.youtube.com/watch?v=dQw4w9WgXcQ' }),
      },
      meta: {
        phase: 'complete',
        durationSeconds: 3_600,
        truncated: true,
        result: { totalSegments: 842, nextCursor: 240 },
      },
      content: [
        {
          type: 'text',
...
test('watch, read, and search cards render outcome summaries and timestamp links', async () => {
  const cases = [
    ...
    },
    {
      ...
      args: { transcriptId: 'tr-1', cursor: 20 },
    ...
    },
    {
      ...
      args: { transcriptId: 'tr-1', query: 'quota' },
      text: '[2:10] Host: The quota is bounded.',
      expected: /1 match for “quota”/,
    },
  ]
  for (const item of cases) {
    ...
    const tree = exports.YoutubeToolCard({
      ...
      block: {
        kind: 'tool-result',
        isError: false,
        call: { name: item.toolName, argsRaw: JSON.stringify(item.args) },
    ...
    if (item.toolName === 'youtube_watch') {
      const [link] = findElements(
        tree,
        (element) => element.type === 'a' && element.children?.includes('0:42'),
      )
