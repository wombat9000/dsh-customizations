...
| `pnpm run format:check`                                   | Verified pnpm executable and installed pinned Prettier; checks maintained files without writes. Honors `.prettierignore`; not a type or behavior check.                                                                                                       |
| `pnpm run format`                                         | Same formatter prerequisites; rewrites maintained files across the repository. Use explicit paths with `pnpm exec prettier --write <paths>` for focused changes. Rebuild affected generated artifacts afterward.                                              |
