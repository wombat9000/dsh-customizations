...
- `pnpm-workspace.yaml` sets `minimumReleaseAge: 10080` (7 days in minutes), supported by the pinned pnpm 11.9.0, with no release-age exceptions. Preserve this policy and inspect overrides before dependency work. The delay is not a safety guarantee. If an urgent security fix needs an exception, obtain explicit approval for a narrow package-and-version exclusion, document its reason and review/removal condition, and keep the global delay enabled. This does not authorize an install or upgrade.
